...
const ChartView = ({ years, regions, baseYear, inflation }: ChartViewProps) => {
  ...
  const createData = (values: number[]) =>
    values.map((v) =>
      inflation ? Number(((v - values[baseYear]) / values[baseYear]) * 100).toFixed(0) : Number((v / values[baseYear]) * 100).toFixed(0)
    );
  ...
  return (
    <Card>
      <CardContent>{data && <Line data={data} height={350} options={options as any} />}</CardContent>
    </Card>
  );
};
