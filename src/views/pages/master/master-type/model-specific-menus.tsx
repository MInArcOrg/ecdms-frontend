...
const ModelSpecificMenus = ({
  ...
  const { data: typeModels, isLoading: isTypeLoading } = useQuery({
    ...
    queryFn: () =>
      modelMenuApiService.getByTypeId(typeId, {
  ...
  useEffect(() => {
    if (moduleModels && typeModels) {
      const initialStates = moduleModels.map((model: string) => {
        const typeModel = typeModels.find(
          (item: ModelMenu) => item.model === model,
        );
        return {
          model: model,
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newStates = switchStates.map((state) =>
      state.model === name ? { ...state, status: checked } : state,
    );
  ...
  return (
    <FormGroup>
      <Typography variant="h6">
        {t(`master-data.form.${module}-type-specific-models`)}
      </Typography>
      {moduleModels?.map((model: string) => {
        ...
        return (
          <FormControlLabel
            ...
            control={
              <Switch
                checked={switchState ? switchState.status : false}
