...
const GeneralMasterDrawer = (props: GeneralMasterDrawerType) => {
  ...
  const onActionSuccess = async (
    response: IApiResponse<ResourceGeneralMaster>,
    payload: IApiPayload<ResourceGeneralMaster>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        resourceMasterModel.fileType,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`master-data.general-master.${
        isEdit ? resourceMasterModel.editTitle : resourceMasterModel.createTitle
      }`}
