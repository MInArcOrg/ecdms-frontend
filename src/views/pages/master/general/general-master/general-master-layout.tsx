...
function GeneralMasterLayout({ children }: { children: ReactElement }) {
  ...
  const { activeModule, masterDataItemsByFlag, isMasterDataSelected } = useMemo(() => {
    ...
    const masterDataItemsByFlag = (activeModule.flags || []).reduce((acc: Record<string, any[]>, flag) => {
        ...
        {!activeModule.id ? (
          <Grid xs={12}>
            <Box sx={{ p: 4, display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', height: '100%' }}>
              ...
              <Typography sx={{ mt: 2, textAlign: 'center' }}>{t('master-data.general-master.select-module')}</Typography>
            </Box>
          </Grid>
        ) : (
          <Fragment>
            ...
            <Grid xs={12} md={9}>
              {isMasterDataSelected ? (
                ...
                <Box
                  sx={{ p: 4, display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', height: '100%' }}
                >
                  <Icon icon="mdi:gesture-tap-button" fontSize="2rem" />
                  <Typography sx={{ mt: 2, textAlign: 'center' }}>{t('master-data.general-master.select-item')}</Typography>
                </Box>
              )}
            </Grid>
          </Fragment>
