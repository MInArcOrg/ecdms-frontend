      ...
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.other.study-period-cost.details.title')}: {studyPeriodCost.title || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.other.study-period-cost.details.description')}: {studyPeriodCost.description || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.other.study-period-cost.details.studyfield-id')}: {studyPeriodCost.stakeholderstudyfield?.studyfield.id || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.other.study-period-cost.details.studyfield-title')}: {studyPeriodCost.stakeholderstudyfield?.studyfield.title || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.other.study-period-cost.details.total-month')}: {studyPeriodCost.total_month || 'N/A'} months
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.other.study-period-cost.details.study-cost')}: {studyPeriodCost.study_cost?.toFixed(2) || 'N/A'} {t('common.currency')}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.other.study-period-cost.details.revision-no')}: {studyPeriodCost.revision_no?.toString() || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('common.table-columns.created-at')}: {studyPeriodCost.created_at ? formatCreatedAt(studyPeriodCost.created_at) : 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('common.table-columns.updated-at')}: {studyPeriodCost.updated_at ? formatCreatedAt(studyPeriodCost.updated_at) : 'N/A'}
          </Typography>
        </Box>
      </CardContent>
