...
const TotalEmployeeDrawer = (props: TotalEmployeeDrawerType) => {
  ...
  const createTotalEmployee = async (body: IApiPayload<TotalEmployee>) =>
    totalEmployeeApiService.create(body);

  const editTotalEmployee = async (body: IApiPayload<TotalEmployee>) =>
    totalEmployeeApiService.update(totalEmployee?.id || "", body);
  ...
  return (
    <CustomSideDrawer
      title={`stakeholder.total-employee.${
        isEdit ? `edit-total-employee` : `create-total-employee`
      }`}
