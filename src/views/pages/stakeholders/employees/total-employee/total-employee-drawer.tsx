...
const TotalEmployeeDrawer = (
  props: TotalEmployeeDrawerType
) => {
  const { open, toggle, refetch, totalEmployee, stakeholderId } =
    props;
  ...
  const createTotalEmployee = async (
    body: IApiPayload<TotalEmployee>
  ) => totalEmployeeApiService.create(body);

  const editTotalEmployee = async (
    body: IApiPayload<TotalEmployee>
  ) =>
    totalEmployeeApiService.update(
      totalEmployee?.id || "",
      body
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<TotalEmployee>,
    payload: IApiPayload<TotalEmployee>
  ) => {
  ...
  return (
    <CustomSideDrawer
      title={`stakeholder.total-employee.${
        isEdit
          ? `edit-total-employee`
        ...
        <FormPageWrapper
          ...
          initialValues={{
            ...(totalEmployee as TotalEmployee),
              ...
              .find(
                (country) =>
                  country.value === totalEmployee.nationality
              ),
          }}
          createActionFunc={
            isEdit
