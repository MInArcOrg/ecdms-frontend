...
const BranchManagerDrawer = (props: BranchManagerDrawerType) => {
  ...
  const editBranchManager = async (
    body: IApiPayload<StakeholderBranchManager>,
  ): Promise<IApiResponse<StakeholderBranchManager>> => {
    return stakeholderBranchManagerApiService.update(
      branchManager?.id || "",
      body,
    );
  };
  ...
  const onActionSuccess = async (
    response: IApiResponse<StakeholderBranchManager>,
  ) => {
    refetch();
    handleClose();
  };
  ...
  return (
    <CustomSideDrawer
      title={`stakeholder.stakeholder-branch-manager.${
        isEdit ? "edit" : "create"
      }`}
