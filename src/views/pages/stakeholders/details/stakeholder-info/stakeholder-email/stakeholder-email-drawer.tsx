...
const StakeholderEmailDrawer = (
  props: StakeholderEmailDrawerType
) => {
  const { open, toggle, refetch, stakeholderEmail, stakeholderId } =
    props;
  ...
  const createStakeholderEmail = async (
    body: IApiPayload<StakeholderEmail>
  ) => stakeholderEmailApiService.create(body);

  const editStakeholderEmail = async (
    body: IApiPayload<StakeholderEmail>
  ) =>
    stakeholderEmailApiService.update(
      stakeholderEmail?.id || "",
      body
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<StakeholderEmail>,
    payload: IApiPayload<StakeholderEmail>
  ) => {
  ...
  return (
    <CustomSideDrawer
      title={`stakeholder.stakeholder-email.${
        isEdit
          ? `edit-stakeholder-email`
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editStakeholderEmail
              : createStakeholderEmail
          }
