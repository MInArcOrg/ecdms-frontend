...
const StakeholderPhoneDrawer = (
  props: StakeholderPhoneDrawerType
) => {
  const { open, toggle, refetch, stakeholderPhone, stakeholderId } =
    props;
  ...
  const createStakeholderPhone = async (
    body: IApiPayload<StakeholderPhone>
  ) => stakeholderPhoneApiService.create(body);

  const editStakeholderPhone = async (
    body: IApiPayload<StakeholderPhone>
  ) =>
    stakeholderPhoneApiService.update(
      stakeholderPhone?.id || "",
      body
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<StakeholderPhone>,
    payload: IApiPayload<StakeholderPhone>
  ) => {
  ...
  return (
    <CustomSideDrawer
      title={`stakeholder.stakeholder-phone.${
        isEdit
          ? `edit-stakeholder-phone`
          : `create-stakeholder-phone`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editStakeholderPhone
              : createStakeholderPhone
          }
