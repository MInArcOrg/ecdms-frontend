...
const EmployeeCard: React.FC<EmployeeCardProps> = ({ employee, refetch, onEdit, onDelete, onDetail, departments, positions }) => {
      ...
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.employee.department')}: {getDepartmentName(employee.stakeholder_department_id)}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.employee.position')}: {getPositionName(employee.stakeholder_position_id)}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.employee.email')}: {employee.email || t('common.not-available')}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.employee.phone')}: {employee.phone || t('common.not-available')}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('stakeholder.employee.nationalIdNo')}: {employee.national_id_no || t('common.not-available')}
          </Typography>
        </Box>
      </CardContent>
