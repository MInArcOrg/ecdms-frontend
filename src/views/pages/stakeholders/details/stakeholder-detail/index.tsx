...
const StakeholderDetailComponent: React.FC<StakeholderDetailComponentProps> = ({
  stakeholder,
  refetch,
  typeId,
}) => {
  ...
  const changeProfilePicture = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.addEventListener("load", () => {
        uploadImage(
          e.target.files![0],
          uploadablePhotoTypes.stakeholder_profile_photo,
          stakeholder.id,
        )
          .then(() => {
  ...
  return (
    <Fragment>
      ...
      <Card>
        ...
        <Tooltip
          title={t("project.general-information.upload-cover-profile")}
          placement="top"
          arrow
        >
        ...
        >
          ...
          <label htmlFor="upload-avatar-pic">
            <Tooltip
              title={t("project.general-information.upload-profile-picture")}
              placement="top"
              arrow
            >
              <Avatar
          ...
          <Box ml={6}>
            ...
            <Typography variant="subtitle1">
              {stakeholder.stakeholdertype?.title}
            </Typography>
          </Box>
        </CardContent>
        <CardContent>
          ...
          >
            <Typography variant="body1">
              <strong>{t("Ownership Type")}:</strong>{" "}
              {stakeholder?.ownership?.title}
            </Typography>
            <Typography variant="body1">
              <strong>{t("Field of Business")}:</strong>{" "}
              {stakeholder?.businessfield?.title}
            </Typography>
          </Box>
          ...
          >
            <Typography variant="body1">
              ...
              {stakeholder?.stakeholdercategory?.title}
            </Typography>
            <Typography variant="body1">
              <strong>{t("Subcategory")}:</strong>{" "}
              {stakeholder?.stakeholdersubcategory?.title}
            </Typography>
          </Box>
        </CardContent>
        <CardActions style={{ justifyContent: "flex-end" }}>
          <Fragment>
            <Box>
              <FileDrawer
                id={stakeholder.id}
                type={uploadableStakeholderFileTypes.stakeholder}
              />{" "}
              &nbsp;
              <Box sx={{ display: "flex" }}>
