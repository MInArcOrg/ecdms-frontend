...
const SafetyEquipmentDrawer = (props: SafetyEquipmentDrawerType) => {
  ...
  const createSafetyEquipment = async (body: IApiPayload<SafetyEquipment>) =>
    safetyEquipmentApiService.create(body);

  const editSafetyEquipment = async (body: IApiPayload<SafetyEquipment>) =>
    safetyEquipmentApiService.update(equipment?.id || "", body);
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editSafetyEquipment : createSafetyEquipment
          }
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<SafetyEquipment>) => (
            <SafetyEquipmentForm formik={formik} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
