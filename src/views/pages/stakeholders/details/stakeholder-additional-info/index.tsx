...
const AdditionalInformationList: React.FC<AdditionalInformationListProps> = ({ stakeholderId }) => {
  ...
  const [selectedRow, setSelectedRow] = useState<StakeholderAdditionalInformation | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const { t } = useTranslation();

  const fetchAdditionalInformation = (params: GetRequestParam): Promise<IApiResponse<StakeholderAdditionalInformation[]>> => {
  ...
  const handleClickDetail = (additionalInfo: StakeholderAdditionalInformation) => {
    toggleDetailDrawer();
    setSelectedRow(additionalInfo);
  };
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapAdditionalInfoToDetailItems(selectedRow as StakeholderAdditionalInformation)}
          id={selectedRow?.id || ''}
          hasReference={true}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: additionalInformationColumns(handleClickDetail, handleEdit, handleDelete, t)
        }}
        isLoading={isLoading}
