...
const JointVentureCompanyDrawer = (props: JointVentureCompanyDrawerType) => {
  ...
  const createJointVentureCompany = async (
  ...
  const editJointVentureCompany = async (
    body: IApiPayload<JointVentureCompany>,
  ): Promise<IApiResponse<JointVentureCompany>> => {
    return jointVentureCompanyApiService.update(
      jointVentureCompany?.id || "",
      body,
    );
  };
  ...
  const onActionSuccess = async (
    response: IApiResponse<JointVentureCompany>,
  ) => {
    refetch();
    handleClose();
  };
  ...
  return (
    ...
    >
      {() => (
        <FormPageWrapper
          edit={isEdit}
          title={`stakeholder.joint-venture-company.${
            isEdit ? "edit" : "create"
          }`}
          ...
          createActionFunc={
            isEdit ? editJointVentureCompany : createJointVentureCompany
          }
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<JointVentureCompany>) => (
            <JointVentureCompanyForm
              formik={formik}
