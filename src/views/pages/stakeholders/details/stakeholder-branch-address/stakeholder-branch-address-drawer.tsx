...
const BranchAddressDrawer = (props: BranchAddressDrawerType) => {
  const { open, toggle, refetch, branchAddress, stakeholderId, stakeholderBranches } = props;
  ...
  const createBranchAddress = async (body: IApiPayload<StakeholderBranchAddress>): Promise<IApiResponse<StakeholderBranchAddress>> => {
    return stakeholderBranchAddressApiService.create(body);
  };

  const editBranchAddress = async (body: IApiPayload<StakeholderBranchAddress>): Promise<IApiResponse<StakeholderBranchAddress>> => {
    return stakeholderBranchAddressApiService.update(branchAddress?.id || '', body);
  };
  ...
  const onActionSuccess = async (response: IApiResponse<StakeholderBranchAddress>) => {
    refetch();
    handleClose();
  };

  return (
    <CustomSideDrawer title={`stakeholder.stakeholder-branch-address.${isEdit ? 'edit' : 'create'}`} handleClose={handleClose} open={open}>
      {() => (
        <FormPageWrapper
          edit={isEdit}
          title={`stakeholder.stakeholder-branch-address.${isEdit ? 'edit' : 'create'}`}
        ...
        >
          {(formik: FormikProps<StakeholderBranchAddress>) => (
            <BranchAddressForm formik={formik} stakeholderBranches={stakeholderBranches} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
