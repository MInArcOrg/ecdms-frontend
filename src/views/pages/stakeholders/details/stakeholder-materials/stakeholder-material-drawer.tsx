...
const MaterialDrawer = (props: MaterialDrawerType) => {
  const { open, toggle, refetch, material, stakeholderId, materialCategories } =
    props;
  ...
  const createMaterial = async (body: IApiPayload<StakeholderMaterial>) =>
    stakeholderMaterialApiService.create(body);

  const editMaterial = async (body: IApiPayload<StakeholderMaterial>) =>
    stakeholderMaterialApiService.update(material?.id || "", body);
