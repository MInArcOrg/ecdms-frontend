...
const MainContractPriceForm: React.FC<MainContractPriceFormProps> = ({
  ...
  const calculatePriceAfterRebate = useCallback(
    (mainContractPrice: number, rebate: number) => {
      ...
      if (name === "main_contract_price_amount" || name === "rebate") {
        const mainContractPrice =
          name === "main_contract_price_amount"
            ? value
            : formik.values.main_contract_price_amount || 0;
        const rebate = name === "rebate" ? value : formik.values.rebate || 0;
        const priceAfterRebate = calculatePriceAfterRebate(
          mainContractPrice,
          rebate,
        );
    ...
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <CustomTextBox
          ...
          placeholder={t(
            "project.main-contract-price.form.main-contract-price",
          )}
          name="main_contract_price_amount"
          size="small"
          type="number"
          onValueChange={(value: string | number) =>
            handleValueChange("main_contract_price_amount", Number(value))
          }
          sx={{ mb: 2 }}
        />
      </Grid>
      <Grid item xs={12}>
        <CustomTextBox
          ...
          type="number"
          onValueChange={(value: string | number) =>
            handleValueChange("rebate", Number(value))
          }
          sx={{ mb: 2 }}
        />
      </Grid>
