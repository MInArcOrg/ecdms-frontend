          ...
          >
            {projectReport.type}
              ...
              <Typography variant="body2" color="text.secondary" sx={{ marginTop: 0.5 }}>
                <span>{t('project.report.form.financial-year')}:</span> <strong>{String(projectReport.year) ?? t('N/A')}</strong>, {t('Quarter')}: <strong>{projectReport.quarter ?? t('N/A')}</strong>
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ marginTop: 0.5 }}>
                <span>{t('project.report.form.physical-performance')}:</span> <strong>{formatCurrency(Number(projectReport.physical_performance))}</strong>
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ marginTop: 0.5 }}>
                <span>{t('project.report.form.financial-performance')}:</span> <strong>{formatCurrency(Number(projectReport.financial_performance))}</strong>
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ marginTop: 0.5 }}>
                <span>{t('project.report.form.project-expense')}:</span> <strong>{formatCurrency(Number(projectReport.project_expense)) ?? t('N/A')}</strong>
              </Typography>
            </Box>
          </Grid>
        </Grid>
