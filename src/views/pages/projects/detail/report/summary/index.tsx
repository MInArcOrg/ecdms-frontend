...
function ReportSummary({ projectId }: ReportSummaryProps) {
  ...
  const sumData = {
    financial: {
      ...
      percent: percentOf(
        data?.Report?.totalFinancialPerformance,
        data?.Plan?.totalFinancialPerformance,
      ).toFixed(2),
    },
    physical: {
      ...
      percent: percentOf(
        data?.Report?.totalPhysicalPerformance,
        data?.Plan?.totalPhysicalPerformance,
      ).toFixed(2),
    },
    expense: {
      ...
      percent: percentOf(
        data?.Report?.totalProjectExpense,
        data?.Plan?.totalProjectExpense,
      ).toFixed(2),
    },
    loss: {
      ...
      percent: percentOf(
        data?.Report?.totalProfitOrLoss,
        data?.Plan?.totalProfitOrLoss,
      ).toFixed(2),
    },
  };
  ...
  return (
    <Box display="flex" flexDirection="column" gap={3}>
      ...
      <Card>
        {!isLoading && data && (
          <ReportSummaryViewCard data={sumData} detailData={data?.Report} />
        )}
      </Card>
    </Box>
  );
