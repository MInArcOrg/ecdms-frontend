...
function ProjectResourceList({ projectId }: { projectId: string }) {
  ...
  ): Promise<IApiResponse<ProjectResource[]>> => {
  ...
  const handleClickDetail = (projectResource: ProjectResource) => {
    router.push(
      `/resources/${projectResource.resource?.resourcetype_id}/details/${projectResource.resource?.id}/`,
    );
  };
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: projectResourceColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
