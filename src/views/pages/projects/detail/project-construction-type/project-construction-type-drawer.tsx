...
const ProjectConstructionTypeDrawer: React.FC<
  ProjectConstructionTypeDrawerProps
> = (props) => {
  ...
  const createProjectConstructionType = async (
    body: IApiPayload<ProjectConstructionType>,
  ) => {
    return await projectConstructionTypeApiService.create(body);
  };

  const editProjectConstructionType = async (
    body: IApiPayload<ProjectConstructionType>,
  ) => {
    return await projectConstructionTypeApiService.update(
  ...
  const onActionSuccess = async (
    response: IApiResponse<ProjectConstructionType>,
    payload: IApiPayload<ProjectConstructionType>,
  ) => {
    if (payload.files.length > 0) {
      if (response.payload.id) {
        uploadFile(
          payload.files[0],
          uploadableResourceFileTypes.project_construction_type,
          response.payload.id,
          "",
          "",
        );
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editProjectConstructionType : createProjectConstructionType
          }
