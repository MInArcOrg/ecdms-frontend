...
const RailwayVehicleInteriorAndPassengerAmenityList: React.FC<
  RailwayVehicleInteriorAndPassengerAmenityListProps
> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayVehicleInteriorAndPassengerAmenity | null>(null);
  const { t } = useTranslation();

  const fetchInteriorAndAmenity = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayVehicleInteriorAndPassengerAmenity[]>> => {
    return projectOtherApiSecondService<RailwayVehicleInteriorAndPassengerAmenity>().getAll(
  ...
  const handleEdit = (
    specs: RailwayVehicleInteriorAndPassengerAmenity,
  ) => {
    toggleDrawer();
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayVehicleInteriorAndPassengerAmenity>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
  ...
  const mapInteriorAndAmenityToDetailItems = (
    specs: RailwayVehicleInteriorAndPassengerAmenity,
      ...
      {
        title: t(
          "project.other.railway-vehicle-interior-and-passenger-amenity.details.railway_vehicle_identification_id",
        ),
      ...
      {
        title: t(
          "project.other.railway-vehicle-interior-and-passenger-amenity.details.seating_capacity",
        ),
        value: specs?.seating_capacity?.toString() || "N/A",
      },
      {
        title: t(
          "project.other.railway-vehicle-interior-and-passenger-amenity.details.passenger_amenities_availability",
        ),
      ...
      },
      {
        ...
        value: specs?.accessibility_features_for_passengers_with_disabilities
          ? t("common.yes")
          : t("common.no"),
      },
      {
        title: t(
          "project.other.railway-vehicle-interior-and-passenger-amenity.details.remark",
        ),
    ...
    <Box>
      {showDrawer && (
        <RailwayVehicleInteriorAndPassengerAmenityDrawer
          ...
          railwayVehicleInteriorAndPassengerAmenity={
            selectedRow as RailwayVehicleInteriorAndPassengerAmenity
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapInteriorAndAmenityToDetailItems(
            selectedRow as RailwayVehicleInteriorAndPassengerAmenity,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          ...
          title={t(
            "project.other.railway-vehicle-interior-and-passenger-amenity.detail",
          )}
        />
      )}
      ...
      <ItemsListing
        title={t(
          "project.other.railway-vehicle-interior-and-passenger-amenity.title",
        )}
        pagination={pagination}
        type={ITEMS_LISTING_TYPE.table.value}
        tableProps={{
          headers: railwayVehicleInteriorAndPassengerAmenityColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
        ...
        ItemViewComponent={({ data }) => (
          <RailwayVehicleInteriorAndPassengerAmenityCard
            ...
            railwayVehicleInteriorAndPassengerAmenity={
              data as RailwayVehicleInteriorAndPassengerAmenity
            }
            onEdit={handleEdit}
            refetch={refetch}
            otherSubMenu={otherSubMenu}
