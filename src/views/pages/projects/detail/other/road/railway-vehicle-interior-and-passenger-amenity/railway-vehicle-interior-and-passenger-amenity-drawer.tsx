...
const RailwayVehicleInteriorAndPassengerAmenityDrawer = ({
  ...
  const validationSchema = yup.object().shape({
    ...
    accessibility_features_for_passengers_with_disabilities: yup
      .boolean()
      .nullable(),
    remark: yup.string().nullable(),
  });

  const createInteriorAndAmenity = async (
    body: IApiPayload<RailwayVehicleInteriorAndPassengerAmenity>,
  ) =>
    projectOtherApiSecondService<RailwayVehicleInteriorAndPassengerAmenity>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editInteriorAndAmenity = async (
    body: IApiPayload<RailwayVehicleInteriorAndPassengerAmenity>,
  ) =>
  ...
  const getPayload = (
    values: RailwayVehicleInteriorAndPassengerAmenity,
  ): IApiPayload<RailwayVehicleInteriorAndPassengerAmenity> => {
    return {
      data: {
        ...values,
  ...
  const onActionSuccess = async (
    response: IApiResponse<RailwayVehicleInteriorAndPassengerAmenity>,
  ) => {
    try {
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editInteriorAndAmenity
              : createInteriorAndAmenity
          }
        ...
        >
          {(
            ...
            <RailwayVehicleInteriorAndPassengerAmenityForm
              formik={formik}
