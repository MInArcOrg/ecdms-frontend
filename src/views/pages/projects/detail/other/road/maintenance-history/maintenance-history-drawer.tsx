...
const MaintenanceHistoryDrawer = (props: MaintenanceHistoryDrawerType) => {
  const { open, toggle, refetch, maintenanceHistory, projectId, otherSubMenu } =
    props;
  ...
  const createMaintenanceHistory = async (
    body: IApiPayload<MaintenanceHistory>,
  ) =>
    projectOtherApiSecondService<MaintenanceHistory>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editMaintenanceHistory = async (
    body: IApiPayload<MaintenanceHistory>,
  ) =>
    projectOtherApiSecondService<MaintenanceHistory>().update(
      otherSubMenu?.apiRoute || "",
  ...
  const onActionSuccess = async (
    response: IApiResponse<MaintenanceHistory>,
    payload: IApiPayload<MaintenanceHistory>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.maintenanceHistory,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.maintenance-history.${
        isEdit ? `edit-maintenance-history` : `create-maintenance-history`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editMaintenanceHistory : createMaintenanceHistory
          }
