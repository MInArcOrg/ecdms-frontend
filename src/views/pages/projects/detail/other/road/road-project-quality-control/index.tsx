...
const RoadProjectQualityControlList: React.FC<RoadProjectQualityControlListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<RoadProjectQualityControl | null>(null);
  const { t } = useTranslation();

  const fetchRoadProjectQualityControls = (params: GetRequestParam): Promise<IApiResponse<RoadProjectQualityControl[]>> => {
  ...
  const handleDelete = async (roadProjectQualityControlId: string) => {
    await projectOtherApiSecondService<RoadProjectQualityControl>().delete(otherSubMenu?.apiRoute || '', roadProjectQualityControlId);
    refetch();
  };
