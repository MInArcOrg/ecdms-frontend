...
const RailwayVehicleMaintenanceAndInspectionList: React.FC<RailwayVehicleMaintenanceAndInspectionListProps> = ({
  otherSubMenu,
  projectId
}) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<RailwayVehicleMaintenanceAndInspection | null>(null);
  const { t } = useTranslation();

  const fetchMaintenanceAndInspection = (params: GetRequestParam): Promise<IApiResponse<RailwayVehicleMaintenanceAndInspection[]>> => {
    return projectOtherApiSecondService<RailwayVehicleMaintenanceAndInspection>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleEdit = (specs: RailwayVehicleMaintenanceAndInspection) => {
    toggleDrawer();
    setSelectedRow(specs);
  };

  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayVehicleMaintenanceAndInspection>().delete(otherSubMenu?.apiRoute || '', id);
    refetch();
  };

  const handleClickDetail = (specs: RailwayVehicleMaintenanceAndInspection) => {
    toggleDetailDrawer();
    setSelectedRow(specs);
  };

  const mapMaintenanceAndInspectionToDetailItems = (specs: RailwayVehicleMaintenanceAndInspection): { title: string; value: any }[] => [
    {
    ...
    {
      title: t('project.other.railway-vehicle-maintenance-and-inspection.details.maintenance_history_records'),
      value: specs?.maintenance_history_records || 'N/A'
    },
    {
      title: t('project.other.railway-vehicle-maintenance-and-inspection.details.vehicle_weight_and_load_capacity'),
      value: specs?.vehicle_weight_and_load_capacity || 'N/A'
    },
    {
      title: t('project.other.railway-vehicle-maintenance-and-inspection.details.maximum_speed'),
    ...
    <Box>
      {showDrawer && (
        <RailwayVehicleMaintenanceAndInspectionDrawer
          ...
          open={showDrawer}
          toggle={toggleDrawer}
          railwayVehicleMaintenanceAndInspection={selectedRow as RailwayVehicleMaintenanceAndInspection}
          refetch={refetch}
          projectId={projectId}
        />
      )}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapMaintenanceAndInspectionToDetailItems(selectedRow as RailwayVehicleMaintenanceAndInspection)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={'RAILWAY_VEHICLE_MAINTENANCE_AND_INSPECTION'}
          title={t('project.other.railway-vehicle-maintenance-and-inspection.detail')}
        />
      )}

      <ItemsListing
        title={t('project.other.railway-vehicle-maintenance-and-inspection.title')}
        pagination={pagination}
        type={ITEMS_LISTING_TYPE.table.value}
        tableProps={{
          headers: railwayVehicleMaintenanceAndInspectionColumns(handleClickDetail, handleEdit, handleDelete, t, refetch, otherSubMenu)
        }}
        isLoading={isLoading}
        ItemViewComponent={({ data }) => (
          <RailwayVehicleMaintenanceAndInspectionCard
            onDetail={handleClickDetail}
            railwayVehicleMaintenanceAndInspection={data as RailwayVehicleMaintenanceAndInspection}
            onEdit={handleEdit}
            refetch={refetch}
            otherSubMenu={otherSubMenu}
