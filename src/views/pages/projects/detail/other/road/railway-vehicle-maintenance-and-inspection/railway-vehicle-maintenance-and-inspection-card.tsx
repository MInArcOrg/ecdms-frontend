...
interface RailwayVehicleMaintenanceAndInspectionCardProps {
  ...
  refetch: () => void;
  onEdit: (specs: RailwayVehicleMaintenanceAndInspection) => void;
  onDelete: (id: string) => void;
  onDetail: (specs: RailwayVehicleMaintenanceAndInspection) => void;
  otherSubMenu?: DetailSubMenuItemChild;
}

const RailwayVehicleMaintenanceAndInspectionCard: React.FC<RailwayVehicleMaintenanceAndInspectionCardProps> = ({
    ...
    <Card sx={{ p: 2 }}>
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Typography variant="h6" fontWeight="bold">
            <Typography
              noWrap
              component={Button}
              onClick={() => onDetail(railwayVehicleMaintenanceAndInspection)}
              sx={{
                fontWeight: 500,
                textDecoration: 'none',
            ...
            >
              {railwayVehicleMaintenanceAndInspection?.id?.toString().slice(0, 5)}
              ...
            </Typography>
          </Typography>
        </Box>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('project.other.railway-vehicle-maintenance-and-inspection.details.maintenance_history_records')}:{' '}
            {railwayVehicleMaintenanceAndInspection.maintenance_history_records || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('project.other.railway-vehicle-maintenance-and-inspection.details.vehicle_weight_and_load_capacity')}:{' '}
            {railwayVehicleMaintenanceAndInspection.vehicle_weight_and_load_capacity || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('project.other.railway-vehicle-maintenance-and-inspection.details.maximum_speed')}:{' '}
            {railwayVehicleMaintenanceAndInspection.maximum_speed || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
          ...
          {railwayVehicleMaintenanceAndInspection.created_at && (
            <Typography variant="body2" color="text.secondary">
              {t('common.table-columns.created-at')}: {railwayVehicleMaintenanceAndInspection.created_at}
            </Typography>
          )}
        </Box>
      </CardContent>
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        {railwayVehicleMaintenanceAndInspection.id && (
          <FileDrawer id={railwayVehicleMaintenanceAndInspection.id} type={otherSubMenu?.fileType || ''} />
        )}
        ...
        <RowOptions
          ...
          onDelete={() => onDelete(railwayVehicleMaintenanceAndInspection.id as string)}
