...
const RailwayVehicleMaintenanceAndInspectionDrawer = ({
  ...
  const createMaintenanceAndInspection = async (body: IApiPayload<RailwayVehicleMaintenanceAndInspection>) =>
    projectOtherApiSecondService<RailwayVehicleMaintenanceAndInspection>().create(otherSubMenu?.apiRoute || '', body);

  const editMaintenanceAndInspection = async (body: IApiPayload<RailwayVehicleMaintenanceAndInspection>) =>
  ...
  const getPayload = (values: RailwayVehicleMaintenanceAndInspection): IApiPayload<RailwayVehicleMaintenanceAndInspection> => {
    return {
      data: {
        ...values,
  ...
  const onActionSuccess = async (response: IApiResponse<RailwayVehicleMaintenanceAndInspection>) => {
    try {
      ...
      if (defaultFile) {
        await uploadFile(defaultFile, fileType, recordId, 'maintenance_document', '');
      }
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editMaintenanceAndInspection : createMaintenanceAndInspection}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwayVehicleMaintenanceAndInspection>) => (
            <RailwayVehicleMaintenanceAndInspectionForm formik={formik} defaultFile={defaultFile} onDefaultFileChange={setDefaultFile} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
