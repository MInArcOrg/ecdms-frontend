...
const RailwayTrackRehabilitationOrRenewalList: React.FC<
  RailwayTrackRehabilitationOrRenewalListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayTrackRehabilitationOrRenewal | null>(null);
  const { t } = useTranslation();

  const fetchRailwayTrackRehabilitationOrRenewal = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayTrackRehabilitationOrRenewal[]>> => {
    return projectOtherApiSecondService<RailwayTrackRehabilitationOrRenewal>().getAll(
  ...
  const handleDelete = async (
    railwayTrackRehabilitationOrRenewalId: string,
  ) => {
    ...
    <Box>
      {showDrawer && (
        <RailwayTrackRehabilitationOrRenewalDrawer
          ...
          railwayTrackRehabilitationOrRenewal={
            selectedRow as RailwayTrackRehabilitationOrRenewal
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapRailwayTrackRehabilitationOrRenewalToDetailItems(
            selectedRow as RailwayTrackRehabilitationOrRenewal,
          )}
          hasReference={false}
          id={selectedRow?.id || ""}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwayTrackRehabilitationOrRenewalColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
