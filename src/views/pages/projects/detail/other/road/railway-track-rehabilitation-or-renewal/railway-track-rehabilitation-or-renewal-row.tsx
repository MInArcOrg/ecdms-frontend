...
export const railwayTrackRehabilitationOrRenewalColumns = (
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.track_renewal_history || t('common.not-available')}</Typography>
    )
  },
  {
    flex: 0.15,
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.plans_or_schedules || t('common.not-available')}</Typography>
    )
  },
  {
    flex: 0.15,
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.rehabilitation_renewal_methods_used_id || t('common.not-available')}</Typography>
    )
  },
  {
    flex: 0.15,
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.rehabilitation_renewal_types || t('common.not-available')}</Typography>
    )
  },
  {
    flex: 0.15,
  ...
  },
  {
    ...
    field: 'created_at',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{formatCreatedAt(row?.created_at) || t('common.not-available')}</Typography>
    )
  },
