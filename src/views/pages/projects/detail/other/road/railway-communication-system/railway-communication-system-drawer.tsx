...
const RailwayCommunicationSystemDrawer = ({
  ...
  const validationSchema = yup.object().shape({
    ...
    communication_system_type_id: yup.string().nullable(),
  ...
  const createRailwayCommunicationSystem = async (body: IApiPayload<RailwayCommunicationSystem>) =>
    projectOtherApiSecondService<RailwayCommunicationSystem>().create(otherSubMenu?.apiRoute || '', body);

  const editRailwayCommunicationSystem = async (body: IApiPayload<RailwayCommunicationSystem>) =>
  ...
  const getPayload = (values: RailwayCommunicationSystem): IApiPayload<RailwayCommunicationSystem> => {
    return {
      data: {
        ...values,
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editRailwayCommunicationSystem : createRailwayCommunicationSystem}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwayCommunicationSystem>) => (
            <RailwayCommunicationSystemForm formik={formik} defaultFile={defaultFile} onDefaultFileChange={setDefaultFile} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
