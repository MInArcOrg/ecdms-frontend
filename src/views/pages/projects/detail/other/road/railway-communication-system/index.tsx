...
const RailwayCommunicationSystemList: React.FC<
  RailwayCommunicationSystemListProps
> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayCommunicationSystem | null>(null);
  const { t } = useTranslation();

  const fetchRailwayCommunicationSystem = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayCommunicationSystem[]>> => {
    return projectOtherApiSecondService<RailwayCommunicationSystem>().getAll(
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayCommunicationSystem>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
  ...
  const mapRailwayCommunicationSystemToDetailItems = (
    ...
    },
    {
      ...
      value:
        communicationSystem?.communication_system_protocols_or_standards ||
        "N/A",
    },
    {
      title: t(
    ...
    },
    {
      ...
      value: communicationSystem?.created_at
        ? formatCreatedAt(communicationSystem.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: communicationSystem?.updated_at
        ? formatCreatedAt(communicationSystem.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapRailwayCommunicationSystemToDetailItems(
            selectedRow as RailwayCommunicationSystem,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwayCommunicationSystemColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
