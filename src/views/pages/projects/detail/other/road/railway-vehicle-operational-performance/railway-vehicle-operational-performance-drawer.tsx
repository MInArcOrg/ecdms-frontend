...
const RailwayVehicleOperationalPerformanceDrawer = ({
  ...
  const createOperationalPerformance = async (
    body: IApiPayload<RailwayVehicleOperationalPerformance>,
  ) =>
    projectOtherApiSecondService<RailwayVehicleOperationalPerformance>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editOperationalPerformance = async (
    body: IApiPayload<RailwayVehicleOperationalPerformance>,
  ) =>
  ...
  const getPayload = (
    values: RailwayVehicleOperationalPerformance,
  ): IApiPayload<RailwayVehicleOperationalPerformance> => {
    return {
      data: {
        ...values,
  ...
  const onActionSuccess = async (
    response: IApiResponse<RailwayVehicleOperationalPerformance>,
  ) => {
    try {
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editOperationalPerformance
              : createOperationalPerformance
          }
        ...
        >
          {(
            ...
            <RailwayVehicleOperationalPerformanceForm
              formik={formik}
