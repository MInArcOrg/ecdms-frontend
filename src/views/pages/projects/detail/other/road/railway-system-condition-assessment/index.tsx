...
const RailwaySystemConditionAssessmentList: React.FC<RailwaySystemConditionAssessmentListProps> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<RailwaySystemConditionAssessment | null>(null);
  const { t } = useTranslation();

  const fetchRailwaySystemConditionAssessment = (params: GetRequestParam): Promise<IApiResponse<RailwaySystemConditionAssessment[]>> => {
    return projectOtherApiSecondService<RailwaySystemConditionAssessment>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwaySystemConditionAssessment>().delete(otherSubMenu?.apiRoute || '', id);
    refetch();
  };
  ...
  const mapRailwaySystemConditionAssessmentToDetailItems = (
    ...
    {
      title: t('project.other.railway-system-condition-assessment.details.railway_line_section_name'),
      value: systemConditionAssessment?.railway_line_section_name || 'N/A'
    },
    {
      title: t('project.other.railway-system-condition-assessment.details.system_condition_rating_or_assessment'),
      value: systemConditionAssessment?.system_condition_rating_or_assessment || 'N/A'
    },
    {
      title: t('project.other.railway-system-condition-assessment.details.defect_presence'),
      value: systemConditionAssessment?.defect_presence ? 'Yes' : 'No'
    },
    {
      title: t('project.other.railway-system-condition-assessment.details.system_performance_indicators'),
      value: systemConditionAssessment?.system_performance_indicators || 'N/A'
    },
    {
      title: t('project.other.railway-system-condition-assessment.details.power_supply_systems_and_communication'),
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwaySystemConditionAssessmentColumns(handleClickDetail, handleEdit, handleDelete, t, refetch, otherSubMenu)
        }}
        isLoading={isLoading}
