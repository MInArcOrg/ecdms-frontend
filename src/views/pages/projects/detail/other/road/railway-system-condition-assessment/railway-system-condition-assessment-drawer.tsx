...
const RailwaySystemConditionAssessmentDrawer = ({
  ...
  const createRailwaySystemConditionAssessment = async (body: IApiPayload<RailwaySystemConditionAssessment>) =>
    projectOtherApiSecondService<RailwaySystemConditionAssessment>().create(otherSubMenu?.apiRoute || '', body);

  const editRailwaySystemConditionAssessment = async (body: IApiPayload<RailwaySystemConditionAssessment>) =>
  ...
  const getPayload = (values: RailwaySystemConditionAssessment): IApiPayload<RailwaySystemConditionAssessment> => {
    return {
      data: {
        ...values,
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editRailwaySystemConditionAssessment : createRailwaySystemConditionAssessment}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwaySystemConditionAssessment>) => (
            <RailwaySystemConditionAssessmentForm formik={formik} defaultFile={defaultFile} onDefaultFileChange={setDefaultFile} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
