...
const RailwaySignalingSystemList: React.FC<RailwaySignalingSystemListProps> = ({
  otherSubMenu,
  projectId,
}) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<RailwaySignalingSystem | null>(
    null,
  );
  const { t } = useTranslation();

  const fetchRailwaySignalingSystem = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwaySignalingSystem[]>> => {
    return projectOtherApiSecondService<RailwaySignalingSystem>().getAll(
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwaySignalingSystem>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
  ...
  const mapRailwaySignalingSystemToDetailItems = (
    signalingSystem: RailwaySignalingSystem,
    ...
    },
    {
      ...
      value:
        signalingSystem?.signaling_system_manufacturer_or_supplier_name ||
        "N/A",
    },
    {
      title: t(
      ...
      value:
        signalingSystem?.signaling_system_manufacturer_or_supplier_phone ||
        "N/A",
    },
    {
      title: t(
    ...
    },
    {
      ...
      value: signalingSystem?.created_at
        ? formatCreatedAt(signalingSystem.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: signalingSystem?.updated_at
        ? formatCreatedAt(signalingSystem.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapRailwaySignalingSystemToDetailItems(
            selectedRow as RailwaySignalingSystem,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwaySignalingSystemColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
