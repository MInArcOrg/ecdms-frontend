...
const RailwaySignalingSystemDrawer = ({
  ...
  const createRailwaySignalingSystem = async (body: IApiPayload<RailwaySignalingSystem>) =>
    projectOtherApiSecondService<RailwaySignalingSystem>().create(otherSubMenu?.apiRoute || '', body);

  const editRailwaySignalingSystem = async (body: IApiPayload<RailwaySignalingSystem>) =>
    projectOtherApiSecondService<RailwaySignalingSystem>().update(otherSubMenu?.apiRoute || '', railwaySignalingSystem.id as string, body);

  const getPayload = (values: RailwaySignalingSystem): IApiPayload<RailwaySignalingSystem> => {
    return {
      data: {
        ...values,
  ...
  return (
    <CustomSideDrawer title={`project.other.railway-signaling-system.${isEdit ? 'edit' : 'create'}`} handleClose={handleClose} open={open}>
      {() => (
        <FormPageWrapper
          edit={isEdit}
          ...
          createActionFunc={isEdit ? editRailwaySignalingSystem : createRailwaySignalingSystem}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwaySignalingSystem>) => (
            <RailwaySignalingSystemForm formik={formik} defaultFile={defaultFile} onDefaultFileChange={setDefaultFile} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
