...
const RailwaySubBallastMaterialList: React.FC<
  RailwaySubBallastMaterialListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwaySubBallastMaterial | null>(null);
  const { t } = useTranslation();

  const fetchData = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwaySubBallastMaterial[]>> => {
    return projectOtherApiSecondService<RailwaySubBallastMaterial>().getAll(
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwaySubBallastMaterial>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwaySubBallastMaterialColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
