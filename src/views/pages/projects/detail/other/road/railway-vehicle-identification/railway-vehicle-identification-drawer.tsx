...
const RailwayVehicleIdentificationDrawer = ({
  ...
  const createVehicleIdentification = async (
    body: IApiPayload<RailwayVehicleIdentification>,
  ) =>
    projectOtherApiSecondService<RailwayVehicleIdentification>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editVehicleIdentification = async (
    body: IApiPayload<RailwayVehicleIdentification>,
  ) =>
  ...
  const getPayload = (
    values: RailwayVehicleIdentification,
  ): IApiPayload<RailwayVehicleIdentification> => {
    return {
      data: {
        ...values,
        project_id: projectId,
        manufacture_year: values.manufacture_year
          ? Number(values.manufacture_year)
          : null,
      },
      files: [],
    };
  ...
  const onActionSuccess = async (
    response: IApiResponse<RailwayVehicleIdentification>,
  ) => {
    try {
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editVehicleIdentification
              : createVehicleIdentification
          }
        ...
        >
          {(
            ...
            <RailwayVehicleIdentificationForm
              formik={formik}
