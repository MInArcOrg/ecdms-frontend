...
const SafetyAndHealthList: React.FC<SafetyAndHealthListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<SafetyAndHealth | null>(null);
  const { t } = useTranslation();

  const fetchSafetyAndHealths = (params: GetRequestParam): Promise<IApiResponse<SafetyAndHealth[]>> => {
    return projectOtherApiSecondService<SafetyAndHealth>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleDelete = async (safetyAndHealthId: string) => {
    await projectOtherApiSecondService<SafetyAndHealth>().delete(otherSubMenu?.apiRoute || '', safetyAndHealthId);
    refetch();
  };
  ...
  const mapSafetyAndHealthToDetailItems = (safetyAndHealth: SafetyAndHealth): { title: string; value: string }[] => [
    {
    ...
    },
    {
      ...
      value: safetyAndHealth?.incident_time ? formatCreatedAt(safetyAndHealth.incident_time) : 'N/A'
    },
    {
      title: t('project.other.safety-and-health.details.medicare-required'),
      value: safetyAndHealth?.medicare_required ? t('common.yes') : t('common.no')
    },
    {
    ...
    },
    {
      ...
      value: safetyAndHealth?.trained_on_equipment_usage ? t('common.yes') : t('common.no')
    },
    {
    ...
    },
    {
      ...
      value: safetyAndHealth?.created_at ? formatCreatedAt(safetyAndHealth.created_at) : 'N/A'
    },
    {
      title: t('common.table-columns.updated-at'),
      value: safetyAndHealth?.updated_at ? formatCreatedAt(safetyAndHealth.updated_at) : 'N/A'
    }
  ];
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: safetyAndHealthColumns(handleClickDetail, handleEdit, handleDelete, t, refetch)
        }}
        isLoading={isLoading}
