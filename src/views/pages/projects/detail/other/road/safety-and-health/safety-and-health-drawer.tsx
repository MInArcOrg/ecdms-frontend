...
const SafetyAndHealthDrawer = (props: SafetyAndHealthDrawerType) => {
  const { open, toggle, refetch, safetyAndHealth, projectId, otherSubMenu } =
    props;
  ...
  const validationSchema = yup.object().shape({
    ...
    total_injury_number: yup
      .number()
      .min(0, "Total injuries must be non-negative"),
    incident_reported_by: yup.string(),
    personal_protective_equipment_type_id: yup
      .string()
      .required("PPE type is required"),
    personal_protective_equipment_condition_id: yup
      .string()
      .required("PPE condition is required"),
    trained_on_equipment_usage: yup.boolean(),
    training_hours_number: yup
      .number()
      .min(0, "Training hours must be non-negative"),
    weather_condition_during_incident_id: yup
      .string()
      .required("Weather condition is required"),
    injury_severity_id: yup.string().required("Injury severity is required"),
    fatality_number: yup
      .number()
      .min(0, "Fatality number must be non-negative"),
  ...
  const createSafetyAndHealth = async (body: IApiPayload<SafetyAndHealth>) =>
    projectOtherApiSecondService<SafetyAndHealth>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editSafetyAndHealth = async (body: IApiPayload<SafetyAndHealth>) =>
    projectOtherApiSecondService<SafetyAndHealth>().update(
      otherSubMenu?.apiRoute || "",
      safetyAndHealth?.id || "",
      body,
    );
  ...
  const getPayload = (values: SafetyAndHealth) => ({
    data: {
      ...
      personal_protective_equipment_type_id:
        values.personal_protective_equipment_type_id,
      personal_protective_equipment_condition_id:
        values.personal_protective_equipment_condition_id,
      ...
      weather_condition_during_incident_id:
        values.weather_condition_during_incident_id,
  ...
  const onActionSuccess = async (
    response: IApiResponse<SafetyAndHealth>,
    payload: IApiPayload<SafetyAndHealth>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.safetyAndHealth,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.safety-and-health.${
        isEdit ? `edit-safety-and-health` : `create-safety-and-health`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...safetyAndHealth,
            incident_time: formatInitialDateDate(
              safetyAndHealth?.incident_time,
            ),
            medicare_required: safetyAndHealth.medicare_required || false,
            trained_on_equipment_usage:
              safetyAndHealth.trained_on_equipment_usage || false,
          }}
          createActionFunc={
            isEdit ? editSafetyAndHealth : createSafetyAndHealth
          }
