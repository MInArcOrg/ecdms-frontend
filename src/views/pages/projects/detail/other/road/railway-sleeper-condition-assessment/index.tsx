...
const RailwaySleeperConditionAssessmentList: React.FC<RailwaySleeperConditionAssessmentListProps> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<RailwaySleeperConditionAssessment | null>(null);
  const { t } = useTranslation();

  const fetchRailwaySleeperConditionAssessment = (params: GetRequestParam): Promise<IApiResponse<RailwaySleeperConditionAssessment[]>> => {
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwaySleeperConditionAssessmentColumns(handleClickDetail, handleEdit, handleDelete, t, refetch)
        }}
        isLoading={isLoading}
