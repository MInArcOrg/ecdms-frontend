...
const BridgeFoundationList: React.FC<BridgeFoundationListProps> = ({
  otherSubMenu,
  projectId,
  typeId,
}) => {
  ...
  const fetchBridgeFoundations = (
    params: GetRequestParam,
  ): Promise<IApiResponse<BridgeFoundation[]>> => {
    return projectOtherApiSecondService<BridgeFoundation>().getAll(
  ...
  const handleDelete = async (bridgeFoundationId: string) => {
    await projectOtherApiSecondService<BridgeFoundation>().delete(
      otherSubMenu?.apiRoute || "",
      bridgeFoundationId,
    );
    refetch();
  ...
  const mapBridgeFoundationToDetailItems = (
    ...
    },
    {
      ...
      value: bridgeFoundation?.created_at
        ? formatCreatedAt(bridgeFoundation.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: bridgeFoundation?.updated_at
        ? formatCreatedAt(bridgeFoundation.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapBridgeFoundationToDetailItems(
            selectedRow as BridgeFoundation,
          )}
          hasReference={false}
          id={selectedRow?.id || ""}
          fileType=""
