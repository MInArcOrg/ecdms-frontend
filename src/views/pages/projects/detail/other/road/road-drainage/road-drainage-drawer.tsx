...
const RoadDrainageDrawer = (props: RoadDrainageDrawerType) => {
  const { open, toggle, refetch, roadDrainage, projectId, otherSubMenu } =
    props;
  ...
  const createRoadDrainage = async (body: IApiPayload<RoadDrainage>) =>
    projectOtherApiSecondService<RoadDrainage>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editRoadDrainage = async (body: IApiPayload<RoadDrainage>) =>
    projectOtherApiSecondService<RoadDrainage>().update(
      otherSubMenu?.apiRoute || "",
      roadDrainage?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<RoadDrainage>,
    payload: IApiPayload<RoadDrainage>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.roadDrainage,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.road-drainage.${
        isEdit ? "edit-road-drainage" : "create-road-drainage"
      }`}
