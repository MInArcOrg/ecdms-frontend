...
const RailwayBallastMaintenanceAndRenewalList: React.FC<
  RailwayBallastMaintenanceAndRenewalListProps
> = ({
  // Renamed component
  otherSubMenu,
  projectId,
  ...
  const [selectedRow, setSelectedRow] =
    useState<RailwayBallastMaintenanceAndRenewal | null>(null); // Updated type
  const { t } = useTranslation();

  const fetchData = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayBallastMaintenanceAndRenewal[]>> => {
    // Updated type
    return projectOtherApiSecondService<RailwayBallastMaintenanceAndRenewal>().getAll(
  ...
  const { data, isLoading, pagination, handlePageChange, refetch } =
    usePaginatedFetch<RailwayBallastMaintenanceAndRenewal[]>({
      // Updated type
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayBallastMaintenanceAndRenewal>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    ); // Updated type
    refetch();
  ...
  const mapToDetailItems = (
    row: RailwayBallastMaintenanceAndRenewal,
    ...
    {
      title: t(
        "project.other.railway-ballast-maintenance-and-renewal.details.railway-line-section-name",
      ), // Updated translation key
      value: row?.railway_line_section_name || "N/A",
    },
    {
      title: t(
        "project.other.railway-ballast-maintenance-and-renewal.details.scheduled-maintenance-activities",
      ), // New field
      value: row?.scheduled_maintenance_activities || "N/A",
    },
    {
      title: t(
        "project.other.railway-ballast-maintenance-and-renewal.details.recent-maintenance-dates",
      ), // New field
      value: row?.recent_maintenance_dates
        ? formatCreatedAt(row.recent_maintenance_dates)
        : "N/A",
    },
    {
      title: t(
        "project.other.railway-ballast-maintenance-and-renewal.details.inspection-reports-findings",
      ), // New field
      value: row?.inspection_reports_findings || "N/A",
    },
    {
      title: t(
        "project.other.railway-ballast-maintenance-and-renewal.details.remark",
      ), // Updated translation key
    ...
    <Box>
      {showDrawer && (
        <RailwayBallastMaintenanceAndRenewalDrawer
          ...
          railwayBallastMaintenanceAndRenewal={
            selectedRow as RailwayBallastMaintenanceAndRenewal
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          fileType=""
          title={t(
            "project.other.railway-ballast-maintenance-and-renewal.detail",
          )}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwayBallastMaintenanceAndRenewalColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
