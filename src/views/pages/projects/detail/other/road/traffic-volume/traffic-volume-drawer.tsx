...
const TrafficVolumeDrawer = (props: TrafficVolumeDrawerType) => {
  const { open, toggle, refetch, trafficVolume, projectId, otherSubMenu } =
    props;
  ...
  const createTrafficVolume = async (body: IApiPayload<TrafficVolume>) =>
    projectOtherApiSecondService<TrafficVolume>().create(
  ...
  const onActionSuccess = async (
    response: IApiResponse<TrafficVolume>,
    payload: IApiPayload<TrafficVolume>,
  ) => {
    refetch();
    handleClose();
  };
  ...
  return (
    <CustomSideDrawer
      title={`project.other.traffic-volume.${
        isEdit ? `edit-traffic-volume` : `create-traffic-volume`
      }`}
