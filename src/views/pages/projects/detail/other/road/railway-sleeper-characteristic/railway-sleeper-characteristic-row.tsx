...
export const railwaySleeperCharacteristicColumns = (
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.sleeper_distance_between_pairs || 'N/A'}</Typography>
    )
  },
  {
    flex: 0.2,
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.sleeper_material_specification || 'N/A'}</Typography>
    )
  },
  {
    flex: 0.2,
