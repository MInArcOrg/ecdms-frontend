...
const RailwaySleeperCharacteristicDrawer = ({
  ...
  const createRailwaySleeperCharacteristic = async (body: IApiPayload<RailwaySleeperCharacteristic>) =>
    projectOtherApiSecondService<RailwaySleeperCharacteristic>().create(otherSubMenu?.apiRoute || '', body);
  ...
  const getPayload = (values: RailwaySleeperCharacteristic): IApiPayload<RailwaySleeperCharacteristic> => ({
    data: {
      ...values,
  ...
  const onActionSuccess = async (response: IApiResponse<RailwaySleeperCharacteristic>, payload: IApiPayload<RailwaySleeperCharacteristic>) => {
    refetch();
    handleClose();
  };
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-sleeper-characteristic.${isEdit ? 'edit' : 'create'}`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editRailwaySleeperCharacteristic : createRailwaySleeperCharacteristic}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwaySleeperCharacteristic>) => <RailwaySleeperCharacteristicForm formik={formik} />}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
  );
