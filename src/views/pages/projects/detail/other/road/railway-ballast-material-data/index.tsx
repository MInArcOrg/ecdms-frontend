...
const RailwayBallastMaterialDataList: React.FC<
  railwayBallastMaterialDataProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayBallastMaterialData | null>(null);
  const { t } = useTranslation();

  const fetchRailwayBallastMaterialData = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayBallastMaterialData[]>> => {
    return projectOtherApiSecondService<RailwayBallastMaterialData>().getAll(
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayBallastMaterialData>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
  ...
  const mapRailwayBallastMaterialDataToDetailItems = (
    ...
    },
    {
      ...
      value: railwayBallastMaterialData?.created_at
        ? formatCreatedAt(railwayBallastMaterialData.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: railwayBallastMaterialData?.updated_at
        ? formatCreatedAt(railwayBallastMaterialData.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapRailwayBallastMaterialDataToDetailItems(
            selectedRow as RailwayBallastMaterialData,
          )}
          hasReference={false}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwayBallastMaterialDataColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
