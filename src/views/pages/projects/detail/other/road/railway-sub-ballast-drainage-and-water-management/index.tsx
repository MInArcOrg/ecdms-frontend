...
const RailwaySubBallastDrainageAndWaterManagementList: React.FC<RailwaySubBallastDrainageAndWaterManagementListProps> = ({
  // Renamed component
  otherSubMenu,
  projectId
}) => {
  ...
  const fetchData = (params: GetRequestParam): Promise<IApiResponse<RailwaySubBallastDrainageAndWaterManagement[]>> => {
    // Updated generic type
    return projectOtherApiSecondService<RailwaySubBallastDrainageAndWaterManagement>().getAll(otherSubMenu?.apiRoute || '', {
      // Updated generic type
      ...params,
  ...
  const { data, isLoading, pagination, handlePageChange, refetch } = usePaginatedFetch<RailwaySubBallastDrainageAndWaterManagement[]>({
    // Updated generic type
  ...
  const handleClickDetail = (row: RailwaySubBallastDrainageAndWaterManagement) => {
    // Updated type
    setSelectedRow(row);
