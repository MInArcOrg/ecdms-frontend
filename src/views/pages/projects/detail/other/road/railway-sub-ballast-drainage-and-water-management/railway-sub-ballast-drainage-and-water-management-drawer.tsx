...
interface RailwaySubBallastDrainageAndWaterManagementDrawerProps {
  // Renamed interface
  open: boolean;
  toggle: () => void;
  refetch: () => void;
...
const RailwaySubBallastDrainageAndWaterManagementDrawer = ({
  // Renamed component
  open,
  toggle,
  refetch,
  ...
  projectId,
  otherSubMenu,
}: RailwaySubBallastDrainageAndWaterManagementDrawerProps) => {
  ...
  const validationSchema = yup.object().shape({
    ...
    drainage_infrastructure_length: yup
      .number()
      .nullable()
      .typeError("Drainage infrastructure length must be a number")
      .optional(),
    remark: yup.string().nullable().optional(),
  });

  const createRailwaySubBallastDrainageAndWaterManagement = async (
    body: IApiPayload<RailwaySubBallastDrainageAndWaterManagement>, // Renamed function and updated type
  ) =>
    projectOtherApiSecondService<RailwaySubBallastDrainageAndWaterManagement>().create(
      otherSubMenu?.apiRoute || "",
      body,
    ); // Updated generic type

  const editRailwaySubBallastDrainageAndWaterManagement = async (
    body: IApiPayload<RailwaySubBallastDrainageAndWaterManagement>, // Renamed function and updated type
  ) =>
    projectOtherApiSecondService<RailwaySubBallastDrainageAndWaterManagement>().update(
      // Updated generic type
      otherSubMenu?.apiRoute || "",
  ...
  const getPayload = (
    values: RailwaySubBallastDrainageAndWaterManagement,
  ): IApiPayload<RailwaySubBallastDrainageAndWaterManagement> => ({
    // Updated type
    data: {
      ...values,
  ...
  return (
    <CustomSideDrawer
      ...
      title={`project.other.railway-sub-ballast-drainage-and-water-management.${
        isEdit ? "edit" : "create"
      }`}
      ...
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...
            project_id:
              railwaySubBallastDrainageAndWaterManagement?.project_id ||
              projectId, // Ensure project_id is set
          }}
          createActionFunc={
            isEdit
              ? editRailwaySubBallastDrainageAndWaterManagement
              : createRailwaySubBallastDrainageAndWaterManagement
