...
export const railwayCommunicationSystemMaintenanceAndTestingColumns = (
  ...
  otherSubMenu?: DetailSubMenuItemChild
    ...
    },
    {
      ...
      headerName: t('project.other.railway-communication-system-maintenance-and-testing.details.railway_line_section_name'),
    ...
    },
    {
      ...
      headerName: t('project.other.railway-communication-system-maintenance-and-testing.details.scheduled_maintenance_activities'),
      renderCell: ({ row }: CellType) => (
    ...
    },
    {
      ...
      minWidth: 100,
      field: 'inspections',
      headerName: t('project.other.railway-communication-system-maintenance-and-testing.details.inspections'),
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.inspections ? 'Yes' : 'No'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 200,
      field: 'recent_maintenance_records_and_dates',
      headerName: t('project.other.railway-communication-system-maintenance-and-testing.details.recent_maintenance_records_and_dates'),
      renderCell: ({ row }: CellType) => (
        <Typography sx={{ color: 'text.secondary' }}>{row?.recent_maintenance_records_and_dates || 'N/A'}</Typography>
      )
    },
    {
      flex: 0.2,
      ...
      renderCell: ({ row }: CellType) => (
        <Typography sx={{ color: 'text.secondary' }}>{row?.testing_and_verification_procedures_prepared ? 'Yes' : 'No'}</Typography>
      )
    },
    {
      flex: 0.2,
      ...
      headerName: t('project.other.railway-communication-system-maintenance-and-testing.details.maintenance_contracts_or_agreements_made'),
      renderCell: ({ row }: CellType) => (
        <Typography sx={{ color: 'text.secondary' }}>{row?.maintenance_contracts_or_agreements_made || 'N/A'}</Typography>
      )
    },
    {
      flex: 0.2,
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => (
        <Typography sx={{ color: 'text.secondary' }}>
          {row.id ? <FileDrawer id={row.id} type={otherSubMenu?.fileType || 'MAINTENANCE_AND_TESTING_CONTRACTS_AGREEMENT'} /> : 'N/A'}
        </Typography>
      )
    },
    {
