...
const BridgeStructureInformationList: React.FC<
  BridgeStructureInformationListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<BridgeStructureInformation | null>(null);
  const { t } = useTranslation();

  const fetchBridgeStructureInformations = (
    params: GetRequestParam,
  ): Promise<IApiResponse<BridgeStructureInformation[]>> => {
    return projectOtherApiSecondService<BridgeStructureInformation>().getAll(
  ...
  const mapBridgeStructureInformationToDetailItems = (
    ...
    },
    {
      ...
      value: bridgeStructureInformation?.created_at
        ? formatCreatedAt(bridgeStructureInformation.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: bridgeStructureInformation?.updated_at
        ? formatCreatedAt(bridgeStructureInformation.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapBridgeStructureInformationToDetailItems(
            selectedRow as BridgeStructureInformation,
          )}
          hasReference={false}
          id={selectedRow?.id || ""}
          fileType=""
          title={t(
            "project.other.bridge-structure-information.bridge-structure-information-details",
          )}
        />
      )}
