...
const RoadSurfaceConditionDrawer = (props: RoadSurfaceConditionDrawerType) => {
  const { open, toggle, refetch, roadSurfaceCondition, projectId, otherSubMenu } = props;
  ...
  const createRoadSurfaceCondition = async (body: IApiPayload<RoadSurfaceCondition>) =>
    projectOtherApiSecondService<RoadSurfaceCondition>().create(otherSubMenu?.apiRoute || '', body);

  const editRoadSurfaceCondition = async (body: IApiPayload<RoadSurfaceCondition>) =>
    projectOtherApiSecondService<RoadSurfaceCondition>().update(otherSubMenu?.apiRoute || '', roadSurfaceCondition?.id || '', body);
  ...
  const onActionSuccess = async (response: IApiResponse<RoadSurfaceCondition>, payload: IApiPayload<RoadSurfaceCondition>) => {
    if (payload.files.length > 0) {
      uploadFile(payload.files[0], uploadableProjectFileTypes.other.roadSurfaceCondition, response.payload.id, '', '');
    }
    refetch();
    handleClose();
  };
      ...
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...
            action_taken_date: formatInitialDateDate(roadSurfaceCondition?.action_taken_date),
          ...
          createActionFunc={isEdit ? editRoadSurfaceCondition : createRoadSurfaceCondition}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RoadSurfaceCondition>) => {
            return <RoadSurfaceConditionForm file={uploadableFile} onFileChange={onFileChange} formik={formik} />;
          }}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
