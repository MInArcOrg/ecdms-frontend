...
export const railwayFasteningSystemCharacteristicColumns = (
  ...
  otherSubMenu?: DetailSubMenuItemChild
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.fastening_system_manufacturer_supplier || 'N/A'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 200,
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.fastening_system_specifications || 'N/A'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 200,
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.rail_clips_or_clamps_details || 'N/A'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 200,
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.bolts_and_nuts_specifications || 'N/A'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 200,
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => (
        <>
          ...
          />
          <RowOptions
            deletePermissionRule={{ action: 'delete', subject: 'railwayfasteningsystemcharacteristic' }}
            editPermissionRule={{ action: 'update', subject: 'railwayfasteningsystemcharacteristic' }}
            onEdit={() => onEdit(row)}
