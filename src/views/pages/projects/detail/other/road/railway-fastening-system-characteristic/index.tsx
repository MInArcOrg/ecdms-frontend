...
const RailwayFasteningSystemCharacteristicList: React.FC<
  RailwayFasteningSystemCharacteristicListProps
> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayFasteningSystemCharacteristic | null>(null);
  const { t } = useTranslation();

  const fetchRailwayFasteningSystemCharacteristic = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayFasteningSystemCharacteristic[]>> => {
    return projectOtherApiSecondService<RailwayFasteningSystemCharacteristic>().getAll(
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayFasteningSystemCharacteristic>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
    ...
    <Box>
      {showDrawer && (
        <RailwayFasteningSystemCharacteristicDrawer
          ...
          railwayFasteningSystemCharacteristic={
            selectedRow as RailwayFasteningSystemCharacteristic
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapRailwayFasteningSystemCharacteristicToDetailItems(
            selectedRow as RailwayFasteningSystemCharacteristic,
          )}
          ...
          title={t(
            "project.other.railway-fastening-system-characteristic.detail",
          )}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: railwayFasteningSystemCharacteristicColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
        ...
        ItemViewComponent={({ data }) => (
          <RailwayFasteningSystemCharacteristicCard
            ...
            railwayFasteningSystemCharacteristic={
              data as RailwayFasteningSystemCharacteristic
            }
            onEdit={handleEdit}
            refetch={refetch}
            otherSubMenu={otherSubMenu}
