...
const RailwayFasteningSystemCharacteristicDrawer = ({
  ...
  const createRailwayFasteningSystemCharacteristic = async (body: IApiPayload<RailwayFasteningSystemCharacteristic>) =>
    projectOtherApiSecondService<RailwayFasteningSystemCharacteristic>().create(otherSubMenu?.apiRoute || '', body);

  const editRailwayFasteningSystemCharacteristic = async (body: IApiPayload<RailwayFasteningSystemCharacteristic>) =>
  ...
  const getPayload = (values: RailwayFasteningSystemCharacteristic): IApiPayload<RailwayFasteningSystemCharacteristic> => {
    const files: File[] = [];
    if (uploadableFile) {
  ...
  const onActionSuccess = async (response: IApiResponse<RailwayFasteningSystemCharacteristic>) => {
    try {
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-fastening-system-characteristic.${isEdit ? 'edit' : 'create'}`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editRailwayFasteningSystemCharacteristic : createRailwayFasteningSystemCharacteristic}
