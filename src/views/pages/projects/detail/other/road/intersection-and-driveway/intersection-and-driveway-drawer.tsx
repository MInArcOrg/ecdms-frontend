...
const IntersectionDrivewayDrawer = (props: IntersectionDrivewayDrawerType) => {
  ...
  const createIntersectionDriveway = async (body: IApiPayload<IntersectionAndDriveway>) =>
    projectOtherApiSecondService<IntersectionAndDriveway>().create(otherSubMenu?.apiRoute || '', body);

  const editIntersectionDriveway = async (body: IApiPayload<IntersectionAndDriveway>) =>
    projectOtherApiSecondService<IntersectionAndDriveway>().update(otherSubMenu?.apiRoute || '', intersectionDriveway?.id || '', body);

  const getPayload = (values: IntersectionAndDriveway): IApiPayload<IntersectionAndDriveway> => ({
    data: {
      ...values,
  ...
  const onActionSuccess = async (response: IApiResponse<IntersectionAndDriveway>, payload: IApiPayload<IntersectionAndDriveway>) => {
    refetch();
    handleClose();
  };
  ...
  return (
    <CustomSideDrawer
      title={`project.other.intersection-driveway.${isEdit ? `edit-intersection-driveway` : `create-intersection-driveway`}`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editIntersectionDriveway : createIntersectionDriveway}
