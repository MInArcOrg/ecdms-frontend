...
const RailwaySleeperEnvironmentalAndOtherFactorDrawer = ({
  ...
  const [sleeperConditionPhotoFile, setSleeperConditionPhotoFile] =
    useState<File | null>(null); // New state for secondary upload
  ...
  const createRailwaySleeperEnvironmentalAndOtherFactor = async (
    body: IApiPayload<RailwaySleeperEnvironmentalAndOtherFactor>,
  ) =>
    projectOtherApiSecondService<RailwaySleeperEnvironmentalAndOtherFactor>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editRailwaySleeperEnvironmentalAndOtherFactor = async (
    body: IApiPayload<RailwaySleeperEnvironmentalAndOtherFactor>,
  ) =>
  ...
  const getPayload = (
    values: RailwaySleeperEnvironmentalAndOtherFactor,
  ): IApiPayload<RailwaySleeperEnvironmentalAndOtherFactor> => {
    const files: File[] = [];
    if (uploadableFile) {
    ...
    if (
      ...
      sleeperConditionPhotoFile !== uploadableFile
    ) {
  ...
  const onActionSuccess = async (
    ...
    try {
      ...
      if (sleeperConditionPhotoFile) {
        console.log(
          "Uploading SLEEPER_CONDITION_PHOTO file for record ID:",
          recordId,
        );
        await uploadFile(
          sleeperConditionPhotoFile,
          "SLEEPER_CONDITION_PHOTO",
          recordId,
          "",
          "",
        );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-sleeper-environmental-and-other-factor.${
        isEdit ? "edit" : "create"
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editRailwaySleeperEnvironmentalAndOtherFactor
              : createRailwaySleeperEnvironmentalAndOtherFactor
          }
