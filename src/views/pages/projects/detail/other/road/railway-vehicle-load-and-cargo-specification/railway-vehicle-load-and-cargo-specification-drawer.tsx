...
const RailwayVehicleLoadAndCargoSpecificationDrawer = ({
  ...
  const validationSchema = yup.object().shape({
    ...
    cargo_restrictions_or_special_requirements: yup.string().nullable(),
    coupling_and_uncoupling_procedures: yup.boolean().nullable(),
    remark: yup.string().nullable()
  });

  const createLoadAndCargo = async (body: IApiPayload<RailwayVehicleLoadAndCargoSpecification>) =>
    projectOtherApiSecondService<RailwayVehicleLoadAndCargoSpecification>().create(otherSubMenu?.apiRoute || '', body);

  const editLoadAndCargo = async (body: IApiPayload<RailwayVehicleLoadAndCargoSpecification>) =>
  ...
  const getPayload = (values: RailwayVehicleLoadAndCargoSpecification): IApiPayload<RailwayVehicleLoadAndCargoSpecification> => {
    return {
      data: {
        ...values,
  ...
  const onActionSuccess = async (response: IApiResponse<RailwayVehicleLoadAndCargoSpecification>) => {
    try {
      ...
      if (defaultFile) {
        await uploadFile(defaultFile, fileType, recordId, 'load_cargo_document', '');
      }
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editLoadAndCargo : createLoadAndCargo}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwayVehicleLoadAndCargoSpecification>) => (
            <RailwayVehicleLoadAndCargoSpecificationForm formik={formik} defaultFile={defaultFile} onDefaultFileChange={setDefaultFile} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
