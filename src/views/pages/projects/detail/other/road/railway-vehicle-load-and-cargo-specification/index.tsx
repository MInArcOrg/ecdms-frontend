...
const RailwayVehicleLoadAndCargoSpecificationList: React.FC<
  RailwayVehicleLoadAndCargoSpecificationListProps
> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayVehicleLoadAndCargoSpecification | null>(null);
  const { t } = useTranslation();

  const fetchLoadAndCargo = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayVehicleLoadAndCargoSpecification[]>> => {
    return projectOtherApiSecondService<RailwayVehicleLoadAndCargoSpecification>().getAll(
  ...
  const handleEdit = (
    specs: RailwayVehicleLoadAndCargoSpecification,
  ) => {
    toggleDrawer();
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayVehicleLoadAndCargoSpecification>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
  ...
  const mapLoadAndCargoToDetailItems = (
    specs: RailwayVehicleLoadAndCargoSpecification,
      ...
      {
        title: t(
          "project.other.railway-vehicle-load-and-cargo-specification.details.railway_vehicle_identification_id",
        ),
      ...
      {
        title: t(
          "project.other.railway-vehicle-load-and-cargo-specification.details.load_capacity_and_weight_limits",
        ),
        value: specs?.load_capacity_and_weight_limits || "N/A",
      },
      {
        title: t(
          "project.other.railway-vehicle-load-and-cargo-specification.details.cargo_restrictions_or_special_requirements",
        ),
        value: specs?.cargo_restrictions_or_special_requirements || "N/A",
      },
      {
        title: t(
          "project.other.railway-vehicle-load-and-cargo-specification.details.coupling_and_uncoupling_procedures",
        ),
        value: specs?.coupling_and_uncoupling_procedures
          ? t("common.yes")
          : t("common.no"),
      },
      {
        title: t(
          "project.other.railway-vehicle-load-and-cargo-specification.details.remark",
        ),
    ...
    <Box>
      {showDrawer && (
        <RailwayVehicleLoadAndCargoSpecificationDrawer
          ...
          railwayVehicleLoadAndCargoSpecification={
            selectedRow as RailwayVehicleLoadAndCargoSpecification
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapLoadAndCargoToDetailItems(
            selectedRow as RailwayVehicleLoadAndCargoSpecification,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          ...
          title={t(
            "project.other.railway-vehicle-load-and-cargo-specification.detail",
          )}
        />
      )}
      ...
      <ItemsListing
        title={t(
          "project.other.railway-vehicle-load-and-cargo-specification.title",
        )}
        pagination={pagination}
        type={ITEMS_LISTING_TYPE.table.value}
        tableProps={{
          headers: railwayVehicleLoadAndCargoSpecificationColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
        ...
        ItemViewComponent={({ data }) => (
          <RailwayVehicleLoadAndCargoSpecificationCard
            ...
            railwayVehicleLoadAndCargoSpecification={
              data as RailwayVehicleLoadAndCargoSpecification
            }
            onEdit={handleEdit}
            refetch={refetch}
            otherSubMenu={otherSubMenu}
