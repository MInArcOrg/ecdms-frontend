...
const CulvertRoadOverInformationList: React.FC<CulvertRoadOverInformationListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<CulvertRoadOverInformation | null>(null);
  const { t } = useTranslation();

  const fetchCulvertRoadOverInformation = (params: GetRequestParam): Promise<IApiResponse<CulvertRoadOverInformation[]>> => {
  ...
  const handleDelete = async (culvertRoadOverInformationId: string) => {
    await projectOtherApiSecondService<CulvertRoadOverInformation>().delete(otherSubMenu?.apiRoute || '', culvertRoadOverInformationId);
    refetch();
  };
