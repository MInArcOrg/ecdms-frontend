...
const RoadMaintenanceActivityList: React.FC<RoadMaintenanceActivityListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<RoadMaintenanceActivity | null>(null);
  ...
  const fetchRoadMaintenanceActivities = (params: GetRequestParam): Promise<IApiResponse<RoadMaintenanceActivity[]>> => {
    return projectOtherApiSecondService<RoadMaintenanceActivity>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleDelete = async (roadMaintenanceActivityId: string) => {
    await projectOtherApiSecondService<RoadMaintenanceActivity>().delete(otherSubMenu?.apiRoute || '', roadMaintenanceActivityId);
    refetch();
  };

  const handleClickDetail = (roadMaintenanceActivity: RoadMaintenanceActivity) => {
    toggleDetailDrawer();
    setSelectedRow(roadMaintenanceActivity);
  };
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapRoadMaintenanceActivityToDetailItems(selectedRow as RoadMaintenanceActivity)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={uploadableProjectFileTypes.other.maintenanceRecord}
          title={t('project.other.road-maintenance-activity.road-maintenance-activity-details')}
        />
      )}
