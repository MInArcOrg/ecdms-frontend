...
export const railwaySubBallastMaintenanceAndRenewalColumns = ( // Renamed function
  ...
  refetch: () => void
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => (
        <Typography sx={{ color: 'text.secondary' }}>{row.recent_maintenance_dates ? formatCreatedAt(row.recent_maintenance_dates) : 'N/A'}</Typography>
      )
    },
    {
      flex: 0.2,
      ...
      renderCell: ({ row }: CellType) => (
        <Typography sx={{ color: 'text.secondary' }}>{row.inspection_reports_findings || 'N/A'}</Typography>
      )
    },
    {
      flex: 0.2,
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => (
        <>
          ...
          />
          <RowOptions
            deletePermissionRule={{ action: 'delete', subject: 'railwaysubballastmaintenanceandrenewal' }} // Updated subject
            editPermissionRule={{ action: 'update', subject: 'railwaysubballastmaintenanceandrenewal' }} // Updated subject
            onEdit={() => onEdit(row)}
