...
interface RailwaySubBallastMaintenanceAndRenewalListProps { // Renamed interface
  otherSubMenu?: DetailSubMenuItemChild;
  typeId: string;
  projectId: string;
}

const RailwaySubBallastMaintenanceAndRenewalList: React.FC<RailwaySubBallastMaintenanceAndRenewalListProps> = ({ // Renamed component
  otherSubMenu,
  projectId
}) => {
  ...
  const fetchData = (params: GetRequestParam): Promise<IApiResponse<RailwaySubBallastMaintenanceAndRenewal[]>> => { // Updated generic type
    return projectOtherApiSecondService<RailwaySubBallastMaintenanceAndRenewal>().getAll(otherSubMenu?.apiRoute || '', { // Updated generic type
      ...params,
  ...
  const {
    ...
    handlePageChange,
    refetch
  } = usePaginatedFetch<RailwaySubBallastMaintenanceAndRenewal[]>({ // Updated generic type
  ...
  const handleEdit = (row: RailwaySubBallastMaintenanceAndRenewal) => { // Updated type
    setSelectedRow(row);
    setShowDrawer(true);
  };
  ...
  const handleClickDetail = (row: RailwaySubBallastMaintenanceAndRenewal) => { // Updated type
    setSelectedRow(row);
  ...
  const mapToDetailItems = (row: RailwaySubBallastMaintenanceAndRenewal): { title: string; value: string }[] => [
    ...
    {
      title: t('project.other.railway-sub-ballast-maintenance-and-renewal.details.scheduled_maintenance_activities', 'Scheduled Maintenance Activities'),
      value: row?.scheduled_maintenance_activities || 'N/A'
    },
    {
      title: t('project.other.railway-sub-ballast-maintenance-and-renewal.details.sub_ballast_renewal_history', 'Sub-Ballast Renewal History'),
    ...
    {
      title: t('project.other.railway-sub-ballast-maintenance-and-renewal.details.inspection_reports_findings', 'Inspection Reports Findings'),
