...
const RailwaySubBallastMaintenanceAndRenewalList: React.FC<
  RailwaySubBallastMaintenanceAndRenewalListProps
> = ({
  // Renamed component
  otherSubMenu,
  projectId,
  ...
  const [selectedRow, setSelectedRow] =
    useState<RailwaySubBallastMaintenanceAndRenewal | null>(null); // Updated state type
  const { t } = useTranslation();

  const fetchData = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwaySubBallastMaintenanceAndRenewal[]>> => {
    // Updated generic type
    return projectOtherApiSecondService<RailwaySubBallastMaintenanceAndRenewal>().getAll(
  ...
  const { data, isLoading, pagination, handlePageChange, refetch } =
    usePaginatedFetch<RailwaySubBallastMaintenanceAndRenewal[]>({
      // Updated generic type
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwaySubBallastMaintenanceAndRenewal>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    ); // Updated generic type
    refetch();
  ...
  const mapToDetailItems = (
    row: RailwaySubBallastMaintenanceAndRenewal,
    ...
    {
      title: t(
        "project.other.railway-sub-ballast-maintenance-and-renewal.details.project_id",
        "Project ID",
      ),
    ...
    },
    {
      ...
      value: row?.recent_maintenance_dates
        ? formatDynamicDate(row.recent_maintenance_dates)
        : "N/A",
    },
    {
      title: t(
    ...
    <Box>
      {showDrawer && (
        <RailwaySubBallastMaintenanceAndRenewalDrawer // Renamed Drawer component
          ...
          railwaySubBallastMaintenanceAndRenewal={
            selectedRow as RailwaySubBallastMaintenanceAndRenewal
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          fileType=""
          title={t(
            "project.other.railway-sub-ballast-maintenance-and-renewal.detail",
            "Sub Ballast Maintenance And Renewal Details",
      ...
      <ItemsListing
        title={t(
          "project.other.railway-sub-ballast-maintenance-and-renewal.title",
          "Sub Ballast Maintenance And Renewal Records",
        )} // Updated translation key and default
        pagination={pagination}
        type={ITEMS_LISTING_TYPE.table.value}
        tableProps={{
          headers: railwaySubBallastMaintenanceAndRenewalColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
        ...
        ItemViewComponent={(
          ...
          <RailwaySubBallastMaintenanceAndRenewalCard // Renamed Card component
            ...
            railwaySubBallastMaintenanceAndRenewal={
              itemData as RailwaySubBallastMaintenanceAndRenewal
            } // Updated prop name and type
            onEdit={handleEdit}
            refetch={refetch}
