...
export const railwaySleeperConditionAssessmentColumns = (
  ...
  refetch: () => void
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.inspection_dates ? formatDynamicDate(row?.inspection_dates) : 'N/A'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 180,
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.sleeper_stability_and_alignment || 'N/A'}</Typography>
    },
    {
      flex: 0.15,
      minWidth: 140,
