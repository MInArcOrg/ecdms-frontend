...
const RoadMaintenanceDataList: React.FC<RoadMaintenanceDataListProps> = ({ otherSubMenu, projectId, typeId }) => {
  ...
  const handleDelete = async (roadMaintenanceDataId: string) => {
    await projectOtherApiSecondService<RoadMaintenanceData>().delete(otherSubMenu?.apiRoute || '', roadMaintenanceDataId);
    refetch();
  };
  ...
  const mapRoadMaintenanceDataToDetailItems = (roadMaintenanceData: RoadMaintenanceData): { title: string; value: string }[] => [
    ...
    },
    {
      ...
      value: roadMaintenanceData?.maintenance_start_date ? formatDate(roadMaintenanceData.maintenance_start_date) : 'N/A'
    },
    {
