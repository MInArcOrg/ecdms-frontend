...
const BridgeBasicDataDrawer = (props: BridgeBasicDataDrawerType) => {
  const { open, toggle, refetch, bridgeBasicData, projectId, otherSubMenu } =
    props;
  ...
  const createBridgeBasicData = async (body: IApiPayload<BridgeBasicData>) =>
    projectOtherApiSecondService<BridgeBasicData>().create(
  ...
  const getPayload = (
    values: BridgeBasicData,
  ): IApiPayload<BridgeBasicData> => ({
    data: {
      ...values,
  ...
  const onActionSuccess = async (
    response: IApiResponse<BridgeBasicData>,
    payload: IApiPayload<BridgeBasicData>,
  ) => {
    refetch();
    handleClose();
  };
  ...
  return (
    <CustomSideDrawer
      title={`project.other.bridge-basic-data.${
        isEdit ? `edit-bridge-basic-data` : `create-bridge-basic-data`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editBridgeBasicData : createBridgeBasicData
          }
