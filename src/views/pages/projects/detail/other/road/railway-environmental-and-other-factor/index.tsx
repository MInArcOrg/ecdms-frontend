...
const RailwayEnvironmentalAndOtherFactorList: React.FC<RailwayEnvironmentalAndOtherFactorListProps> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<RailwayEnvironmentalAndOtherFactor | null>(null);
  const { t } = useTranslation();

  const fetchRailwayEnvironmentalAndOtherFactor = (
    params: GetRequestParam
  ): Promise<IApiResponse<RailwayEnvironmentalAndOtherFactor[]>> => {
    return projectOtherApiSecondService<RailwayEnvironmentalAndOtherFactor>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleEdit = (environmentalFactor: RailwayEnvironmentalAndOtherFactor) => {
    toggleDrawer();
    setSelectedRow(environmentalFactor);
  };

  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayEnvironmentalAndOtherFactor>().delete(otherSubMenu?.apiRoute || '', id);
    refetch();
  };

  const handleClickDetail = (environmentalFactor: RailwayEnvironmentalAndOtherFactor) => {
    toggleDetailDrawer();
    setSelectedRow(environmentalFactor);
  };
  ...
  const mapEnvironmentalFactorToDetailItems = (
    ...
    {
      title: t('project.other.railway-environmental-and-other-factor.details.railway_line_section_name'),
      value: environmentalFactor?.railway_line_section_name || 'N/A'
    },
    {
      title: t('project.other.railway-environmental-and-other-factor.details.environmental_compliance_measures'),
      value: environmentalFactor?.environmental_compliance_measures ? 'Yes' : 'No'
    },
    {
      title: t('project.other.railway-environmental-and-other-factor.details.environmental_impact_assessment'),
      value: environmentalFactor?.environmental_impact_assessment ? 'Yes' : 'No'
    },
    {
      title: t('project.other.railway-environmental-and-other-factor.details.data_recording_date'),
      value: environmentalFactor?.data_recording_date ? formatDate(environmentalFactor.data_recording_date) : 'N/A'
    },
    {
      title: t('project.other.railway-environmental-and-other-factor.details.remark'),
      value: environmentalFactor?.remark || 'N/A'
    },
    {
      title: t('project.other.railway-environmental-and-other-factor.details.compliance-document-file-upload'),
      value: (
        <FileDrawer
    ...
    <Box>
      {showDrawer && (
        <RailwayEnvironmentalAndOtherFactorDrawer
          ...
          open={showDrawer}
          toggle={toggleDrawer}
          railwayEnvironmentalAndOtherFactor={selectedRow as RailwayEnvironmentalAndOtherFactor}
          refetch={refetch}
          projectId={projectId}
        />
      )}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapEnvironmentalFactorToDetailItems(selectedRow as RailwayEnvironmentalAndOtherFactor)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={'RAILWAY_ENVIRONMENTAL_AND_OTHER_FACTOR'}
          title={t('project.other.railway-environmental-and-other-factor.detail')}
        />
      )}

      <ItemsListing
        title={t('project.other.railway-environmental-and-other-factor.title')}
        pagination={pagination}
        type={ITEMS_LISTING_TYPE.table.value}
        tableProps={{
          headers: railwayEnvironmentalAndOtherFactorColumns(handleClickDetail, handleEdit, handleDelete, t, refetch, otherSubMenu)
        }}
        isLoading={isLoading}
        ItemViewComponent={({ data }) => (
          <RailwayEnvironmentalAndOtherFactorCard
            onDetail={handleClickDetail}
            railwayEnvironmentalAndOtherFactor={data as RailwayEnvironmentalAndOtherFactor}
            onEdit={handleEdit}
            refetch={refetch}
            otherSubMenu={otherSubMenu}
