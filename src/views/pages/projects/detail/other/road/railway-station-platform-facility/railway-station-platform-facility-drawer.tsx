...
const RailwayStationPlatformFacilityDrawer = ({
  ...
  const createPlatformFacility = async (body: IApiPayload<RailwayStationPlatformFacility>) =>
    projectOtherApiSecondService<RailwayStationPlatformFacility>().create(otherSubMenu?.apiRoute || '', body);

  const editPlatformFacility = async (body: IApiPayload<RailwayStationPlatformFacility>) =>
  ...
  const getPayload = (values: RailwayStationPlatformFacility): IApiPayload<RailwayStationPlatformFacility> => {
    return {
      data: {
        ...values,
  ...
  const onActionSuccess = async (response: IApiResponse<RailwayStationPlatformFacility>) => {
    try {
      ...
      if (defaultFile) {
        await uploadFile(defaultFile, fileType, recordId, 'facility_document', '');
      }
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editPlatformFacility : createPlatformFacility}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwayStationPlatformFacility>) => (
            <RailwayStationPlatformFacilityForm formik={formik} defaultFile={defaultFile} onDefaultFileChange={setDefaultFile} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
