...
const RailwayFasteningSystemConditionAssessmentDrawer = ({
  ...
  const createRailwayFasteningSystemConditionAssessment = async (body: IApiPayload<RailwayFasteningSystemConditionAssessment>) =>
    projectOtherApiSecondService<RailwayFasteningSystemConditionAssessment>().create(otherSubMenu?.apiRoute || '', body);

  const editRailwayFasteningSystemConditionAssessment = async (body: IApiPayload<RailwayFasteningSystemConditionAssessment>) =>
  ...
  const getPayload = (values: RailwayFasteningSystemConditionAssessment): IApiPayload<RailwayFasteningSystemConditionAssessment> => {
    const files: File[] = [];
    if (uploadableFile) {
  ...
  const onActionSuccess = async (response: IApiResponse<RailwayFasteningSystemConditionAssessment>) => {
    try {
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-fastening-system-condition-assessment.${isEdit ? 'edit' : 'create'}`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...railwayFasteningSystemConditionAssessment,
            inspection_date: formatInitialDateDate(railwayFasteningSystemConditionAssessment.inspection_date)
          }}
          createActionFunc={isEdit ? editRailwayFasteningSystemConditionAssessment : createRailwayFasteningSystemConditionAssessment}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwayFasteningSystemConditionAssessment>) => (
            <RailwayFasteningSystemConditionAssessmentForm formik={formik} file={uploadableFile} onFileChange={setUploadableFile} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
