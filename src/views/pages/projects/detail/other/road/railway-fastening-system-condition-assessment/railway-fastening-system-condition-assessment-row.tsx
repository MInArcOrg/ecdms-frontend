...
export const railwayFasteningSystemConditionAssessmentColumns = (
  ...
  otherSubMenu?: DetailSubMenuItemChild
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row.inspection_date ? formatDynamicDate((row.inspection_date)) : 'N/A'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 200,
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.fastening_system_condition_rating || 'N/A'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 200,
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.fastening_system_stability_and_alignment || 'N/A'}</Typography>
    },
    {
      flex: 0.15,
      minWidth: 150,
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.rail_fastening_needed_quantity?.toString() || 'N/A'}</Typography>
    },
    {
      flex: 0.15,
      minWidth: 150,
      ...
      renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.rail_fastening_expected_lifespan?.toString() || 'N/A'}</Typography>
    },
    {
      flex: 0.15,
      minWidth: 150,
      ...
      renderCell: ({ row }: CellType) => (
        <Typography sx={{ color: 'text.secondary' }}>
          {typeof row.rail_fastening_availability === 'boolean'
            ? (row.rail_fastening_availability ? t('common.options.yes') : t('common.options.no'))
            : 'N/A'}
        </Typography>
      )
    },
    ...
    },
    {
      ...
      renderCell: ({ row }: CellType) => (
        <>
          ...
          />
          <RowOptions
            deletePermissionRule={{ action: 'delete', subject: 'railwayfasteningsystemconditionassessment' }}
            editPermissionRule={{ action: 'update', subject: 'railwayfasteningsystemconditionassessment' }}
            onEdit={() => onEdit(row)}
