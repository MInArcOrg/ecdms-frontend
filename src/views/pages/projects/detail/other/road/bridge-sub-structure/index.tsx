...
const BridgeSubStructureList: React.FC<BridgeSubStructureListProps> = ({
  otherSubMenu,
  projectId,
  typeId,
}) => {
  ...
  const [selectedRow, setSelectedRow] = useState<BridgeSubStructure | null>(
    null,
  );
  const { t } = useTranslation();

  const fetchBridgeSubStructures = (
    params: GetRequestParam,
  ): Promise<IApiResponse<BridgeSubStructure[]>> => {
    return projectOtherApiSecondService<BridgeSubStructure>().getAll(
  ...
  const handleDelete = async (bridgeSubStructureId: string) => {
    await projectOtherApiSecondService<BridgeSubStructure>().delete(
      otherSubMenu?.apiRoute || "",
  ...
  const mapBridgeSubStructureToDetailItems = (
    ...
    },
    {
      ...
      value: bridgeSubStructure?.created_at
        ? formatCreatedAt(bridgeSubStructure.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: bridgeSubStructure?.updated_at
        ? formatCreatedAt(bridgeSubStructure.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapBridgeSubStructureToDetailItems(
            selectedRow as BridgeSubStructure,
          )}
          hasReference={false}
          id={selectedRow?.id || ""}
          fileType=""
          title={t(
            "project.other.bridge-sub-structure.bridge-sub-structure-details",
          )}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: bridgeSubStructureColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
