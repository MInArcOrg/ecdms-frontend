...
interface RailwayBallastDrainageAndWaterManagementListProps { // Renamed interface
  otherSubMenu?: DetailSubMenuItemChild;
  typeId: string;
  projectId: string;
}

const RailwayBallastDrainageAndWaterManagementList: React.FC<RailwayBallastDrainageAndWaterManagementListProps> = ({ // Renamed component
  otherSubMenu,
  projectId,
  typeId
}) => {
  ...
  const fetchData = (params: GetRequestParam): Promise<IApiResponse<RailwayBallastDrainageAndWaterManagement[]>> => { // Updated type
    return projectOtherApiSecondService<RailwayBallastDrainageAndWaterManagement>().getAll(otherSubMenu?.apiRoute || '', { // Updated type
      ...params,
  ...
  const {
    ...
    handlePageChange,
    refetch
  } = usePaginatedFetch<RailwayBallastDrainageAndWaterManagement[]>({ // Updated type
  ...
  const handleEdit = (row: RailwayBallastDrainageAndWaterManagement) => { // Updated type
    setSelectedRow(row);
    setShowDrawer(true);
  };
  ...
  const handleClickDetail = (row: RailwayBallastDrainageAndWaterManagement) => { // Updated type
    setSelectedRow(row);
    setShowDetailDrawer(true);
  };

  const mapToDetailItems = (row: RailwayBallastDrainageAndWaterManagement): { title: string; value: string }[] => [ // Updated type
    {
