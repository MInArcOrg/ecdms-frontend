...
const RailwayFasteningSystemMaintenanceAndReplacementCard: React.FC<RailwayFasteningSystemMaintenanceAndReplacementCardProps> = ({
    ...
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t('project.other.railway-fastening-system-maintenance-and-replacement.details.railway_line_section_name')}:{' '}
            {railwayFasteningSystemMaintenanceAndReplacement.railway_line_section_name || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('project.other.railway-fastening-system-maintenance-and-replacement.details.scheduled_maintenance_activities')}:{' '}
            {railwayFasteningSystemMaintenanceAndReplacement.scheduled_maintenance_activities || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('project.other.railway-fastening-system-maintenance-and-replacement.details.recent_maintenance_records_and_dates')}:{' '}
            {railwayFasteningSystemMaintenanceAndReplacement.recent_maintenance_records_and_dates || 'N/A'}
          </Typography>
          <Typography variant="body2" color="text.secondary">
          ...
          {railwayFasteningSystemMaintenanceAndReplacement.created_at && (
            <Typography variant="body2" color="text.secondary">
              {t('common.table-columns.created-at')}: {railwayFasteningSystemMaintenanceAndReplacement.created_at}
            </Typography>
          )}
          {railwayFasteningSystemMaintenanceAndReplacement.updated_at && (
            <Typography variant="body2" color="text.secondary">
              {t('common.table-columns.updated-at')}: {railwayFasteningSystemMaintenanceAndReplacement.updated_at}
            </Typography>
          )}
        </Box>
      </CardContent>
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        {railwayFasteningSystemMaintenanceAndReplacement.id && (
          <FileDrawer id={railwayFasteningSystemMaintenanceAndReplacement.id} type={'INSPECTION_REPORTS_AND_FINDINGS'} />
        )}

        {railwayFasteningSystemMaintenanceAndReplacement.id && (
          <FileDrawer id={railwayFasteningSystemMaintenanceAndReplacement.id} type={'FASTENING SYSTEM_REPLACEMENT_HISTORY'} />
        )}
