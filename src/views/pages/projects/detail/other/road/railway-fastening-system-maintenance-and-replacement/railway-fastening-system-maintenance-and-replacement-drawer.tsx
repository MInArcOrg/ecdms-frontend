...
const RailwayFasteningSystemMaintenanceAndReplacementDrawer = ({
  ...
  const [replacementHistoryFile, setReplacementHistoryFile] =
    useState<File | null>(null);
  ...
  const createRailwayFasteningSystemMaintenanceAndReplacement = async (
    ...
    projectOtherApiSecondService<RailwayFasteningSystemMaintenanceAndReplacement>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editRailwayFasteningSystemMaintenanceAndReplacement = async (
    body: IApiPayload<RailwayFasteningSystemMaintenanceAndReplacement>,
  ) =>
  ...
  const onActionSuccess = async (
    response: IApiResponse<RailwayFasteningSystemMaintenanceAndReplacement>,
  ) => {
    try {
      ...
      if (inspectionReportsFile) {
        console.log(
          "Uploading inspection reports file for record ID:",
          recordId,
        );
        await uploadFile(
          inspectionReportsFile,
          "INSPECTION_REPORTS_AND_FINDINGS",
          recordId,
          "",
          "",
        );
      ...
      if (replacementHistoryFile) {
        console.log(
          "Uploading replacement history file for record ID:",
          recordId,
        );
        await uploadFile(
          replacementHistoryFile,
          "FASTENING_SYSTEM_REPLACEMENT_HISTORY",
          recordId,
          "",
          "",
        );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-fastening-system-maintenance-and-replacement.${
        isEdit ? "edit" : "create"
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editRailwayFasteningSystemMaintenanceAndReplacement
              : createRailwayFasteningSystemMaintenanceAndReplacement
          }
