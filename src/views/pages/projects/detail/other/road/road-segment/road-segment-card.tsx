...
const RoadSegmentCard: React.FC<RoadSegmentCardProps> = ({
  ...
  const { data: surfaceTypeData } = useQuery({
    ...
    queryFn: () =>
      projectGeneralMasterDataApiService.getOne(
        roadSegment?.surface_type_id || "",
        {},
      ),
  ...
  const { data: designStandardData } = useQuery({
    ...
    queryFn: () =>
      projectGeneralMasterDataApiService.getOne(
        roadSegment?.design_standard_id || "",
        {},
      ),
  ...
  return (
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.road-segment.details.name")}:{" "}
            {roadSegment?.name || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.road-segment.details.surface-type-id")}:{" "}
            {surfaceTypeName}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.road-segment.details.start-northing")}:{" "}
            {roadSegment?.start_northing?.toString() || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.road-segment.details.start-easting")}:{" "}
            {roadSegment?.start_easting?.toString() || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.road-segment.details.end-northing")}:{" "}
            {roadSegment?.end_northing?.toString() || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.road-segment.details.end-easting")}:{" "}
            {roadSegment?.end_easting?.toString() || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.road-segment.details.design-standard-id")}:{" "}
            {designStandardName}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.created-at")}:{" "}
            {roadSegment?.created_at
              ? formatCreatedAt(roadSegment.created_at)
              : "N/A"}
          </Typography>
        </Box>
      </CardContent>
      ...
      <CardActions sx={{ justifyContent: "flex-end" }}>
        <FileDrawer
          id={roadSegment.id}
          type={uploadableProjectFileTypes.other.roadSegment}
        />
        <ModelAction
          model="RoadSegment"
