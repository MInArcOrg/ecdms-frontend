...
const RailwayVehicleSafetyAndComplianceDrawer = ({
  ...
  const validationSchema = yup.object().shape({
    ...
    comply_with_regulatory_standards_and_certifications: yup
      .boolean()
      .nullable(),
    incident_records_number: yup
      .number()
      .integer("Must be a whole number")
      .nullable()
      .typeError("Incident count must be a number"),
    action_taken_to_accidents: yup.string().nullable(),
    remark: yup.string().nullable(),
  });

  const createSafetyAndCompliance = async (
    body: IApiPayload<RailwayVehicleSafetyAndCompliance>,
  ) =>
    projectOtherApiSecondService<RailwayVehicleSafetyAndCompliance>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editSafetyAndCompliance = async (
    body: IApiPayload<RailwayVehicleSafetyAndCompliance>,
  ) =>
  ...
  const getPayload = (
    values: RailwayVehicleSafetyAndCompliance,
  ): IApiPayload<RailwayVehicleSafetyAndCompliance> => {
    return {
      data: {
        ...values,
        project_id: projectId,
        incident_records_number: values.incident_records_number
          ? Number(values.incident_records_number)
          : null,
      },
      files: [],
    };
  ...
  const onActionSuccess = async (
    response: IApiResponse<RailwayVehicleSafetyAndCompliance>,
  ) => {
    try {
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editSafetyAndCompliance
              : createSafetyAndCompliance
          }
        ...
        >
          {(
            ...
            <RailwayVehicleSafetyAndComplianceForm
              formik={formik}
