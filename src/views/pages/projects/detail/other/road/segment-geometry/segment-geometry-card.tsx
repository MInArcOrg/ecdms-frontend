...
const SegmentGeometryCard: React.FC<SegmentGeometryCardProps> = ({
  segmentGeometry,
  refetch,
  onEdit,
  onDelete,
  ...
  // Fetch cross section type data
  const { data: crossSectionTypeData } = useQuery({
    queryKey: ["crossSectionType", segmentGeometry?.cross_section_type_id],
    queryFn: () => crossSectionTypeMasterService.getOne(segmentGeometry?.cross_section_type_id || "", {}),
    ...
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t("project.other.segment-geometry.details.carriage-way-width")}:{" "}
            {segmentGeometry?.carriage_way_width?.toString() || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.segment-geometry.details.lane-width")}: {segmentGeometry?.lane_width?.toString() || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.segment-geometry.details.shoulder-width")}:{" "}
            {segmentGeometry?.shoulder_width?.toString() || "N/A"}
          </Typography>
          <Typography variant="body2" color="text.secondary">
          ...
          <Typography variant="body2" color="text.secondary">
            ...
            {segmentGeometry?.created_at ? formatCreatedAt(segmentGeometry.created_at) : "N/A"}
          </Typography>
        </Box>
      </CardContent>
