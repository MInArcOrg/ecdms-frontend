...
const SegmentGeometryDrawer = (props: SegmentGeometryDrawerType) => {
  const { open, toggle, refetch, segmentGeometry, projectId, otherSubMenu } =
    props;

  const validationSchema = yup.object().shape({
    name: yup.string().required("Name is required"),
    cross_section_type_id: yup
      .string()
      .required("Cross Section Type is required"),
  });

  const isEdit = Boolean(segmentGeometry?.id);

  const createSegmentGeometry = async (body: IApiPayload<SegmentGeometry>) =>
    projectOtherApiSecondService<SegmentGeometry>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editSegmentGeometry = async (body: IApiPayload<SegmentGeometry>) =>
    projectOtherApiSecondService<SegmentGeometry>().update(
      otherSubMenu?.apiRoute || "",
      segmentGeometry?.id || "",
      body,
    );
  ...
  const getPayload = (
    values: SegmentGeometry,
  ): IApiPayload<SegmentGeometry> => ({
    data: {
      ...values,
  ...
  const onActionSuccess = async (
    response: IApiResponse<SegmentGeometry>,
    payload: IApiPayload<SegmentGeometry>,
  ) => {
    refetch();
    handleClose();
  };
  ...
  return (
    <CustomSideDrawer
      title={`project.other.segment-geometry.${
        isEdit ? `edit-segment-geometry` : `create-segment-geometry`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editSegmentGeometry : createSegmentGeometry
          }
