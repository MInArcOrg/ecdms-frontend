...
const RailwayVehicleSpecificationList: React.FC<
  RailwayVehicleSpecificationListProps
> = ({ otherSubMenu, projectId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayVehicleSpecification | null>(null);
  const { t } = useTranslation();

  const fetchVehicleSpecification = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayVehicleSpecification[]>> => {
    return projectOtherApiSecondService<RailwayVehicleSpecification>().getAll(
  ...
  const handleEdit = (
    specs: RailwayVehicleSpecification,
  ) => {
    toggleDrawer();
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayVehicleSpecification>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
  ...
  const mapVehicleSpecificationToDetailItems = (
    specs: RailwayVehicleSpecification,
    ...
    <Box>
      {showDrawer && (
        <RailwayVehicleSpecificationDrawer
          ...
          railwayVehicleSpecification={
            selectedRow as RailwayVehicleSpecification
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapVehicleSpecificationToDetailItems(
            selectedRow as RailwayVehicleSpecification,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          ...
          title={t(
            "project.other.railway-vehicle-specification.detail",
          )}
        />
      )}
      ...
      <ItemsListing
        title={t(
          "project.other.railway-vehicle-specification.title",
        )}
        pagination={pagination}
        type={ITEMS_LISTING_TYPE.table.value}
        tableProps={{
          headers: railwayVehicleSpecificationColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
        ...
        ItemViewComponent={({ data }) => (
          <RailwayVehicleSpecificationCard
            ...
            railwayVehicleSpecification={
              data as RailwayVehicleSpecification
            }
            onEdit={handleEdit}
            refetch={refetch}
            otherSubMenu={otherSubMenu}
