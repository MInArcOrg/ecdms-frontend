...
const RailwaySubBallastMaterialTestDrawer = ({
  ...
  const validationSchema = yup.object().shape({
    ...
    sub_ballast_material_type_id: yup
      .string()
      .required("Sub-ballast material type is required"),
  ...
  const createRailwaySubBallastMaterialTest = async (
    body: IApiPayload<RailwaySubBallastMaterialTest>,
  ) =>
    projectOtherApiSecondService<RailwaySubBallastMaterialTest>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editRailwaySubBallastMaterialTest = async (
    body: IApiPayload<RailwaySubBallastMaterialTest>,
  ) =>
  ...
  const getPayload = (
    values: RailwaySubBallastMaterialTest,
  ): IApiPayload<RailwaySubBallastMaterialTest> => ({
    data: {
      ...values,
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-sub-ballast-material-test.${
        isEdit ? "edit" : "create"
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editRailwaySubBallastMaterialTest
              : createRailwaySubBallastMaterialTest
          }
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<RailwaySubBallastMaterialTest>) => (
            <RailwaySubBallastMaterialTestForm formik={formik} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
