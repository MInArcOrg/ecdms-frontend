...
const RailwayTrackConditionAssesmentList: React.FC<
  RailwayTrackConditionAssesmentListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayTrackConditionAssessment | null>(null);
  const { t } = useTranslation();

  const fetchRailwayTrackConditionAssessment = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayTrackConditionAssessment[]>> => {
    return projectOtherApiSecondService<RailwayTrackConditionAssessment>().getAll(
  ...
  const mapAssessmentToDetailItems = (
    ...
    {
      title: t(
        "project.other.railway-track-condition-assessment.details.project-id",
      ),
...
value: assessment?.inspection_dates?.toLocaleString() || "N/A",
    ...
    {
      title: t(
        "project.other.railway-track-condition-assessment.details.track-condition-rating-id",
      ),
      value: assessment?.track_condition_rating_id || "N/A",
    },
    {
      title: t(
        "project.other.railway-track-condition-assessment.details.observed-defects-id",
      ),
      value: assessment?.observed_defects_id || "N/A",
    },
    {
      title: t(
        "project.other.railway-track-condition-assessment.details.track-settlement-irregularities",
      ),
      value: assessment?.track_settlement_irregularities || "N/A",
    },
    {
      title: t(
        "project.other.railway-track-condition-assessment.details.remark",
      ),
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapAssessmentToDetailItems(
            selectedRow as RailwayTrackConditionAssessment,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={
          ...
          title={t(
            "project.other.railway-track-condition-assessment.railway-track-condition-assessment-details",
          )}
        />
      )}
