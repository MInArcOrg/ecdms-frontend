...
const RailwayTrackDataForm: React.FC<RailwayTrackDataFormProps> = ({
  formik,
  file,
  onFileChange,
}) => {
  ...
  const { data: railwayTrackInfrastructureTypes } = useQuery({
    ...
    queryFn: () =>
      projectGeneralMasterDataApiService.getAll(
        dropDownConfig({
          filter: {
            model: projectMasterModels.railwayTrackInfrastructure.model,
          },
        }),
      ),
  });
  ...
  console.log(
    "railwayTrackInfrastructureTypes",
    railwayTrackInfrastructureTypes,
  );
  return (
        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <CustomSelectBox
              fullWidth
              label={t(
                "project.other.railway-track-data.details.track-type-id",
              )}
              name="track_type_id"
              size="small"
              sx={{ mb: 2 }}
        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <CustomSelectBox
              fullWidth
              label={t(
                "project.other.railway-track-data.details.track-gauge-id",
              )}
              name="track_gauge_id"
              size="small"
              sx={{ mb: 2 }}
        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <CustomTextBox
              fullWidth
              label={t(
                "project.other.railway-track-data.details.fastening-systems",
              )}
