...
export const railwayTrackDataColumns = (
...
): GridColDef[] => [
  {
    ...
    renderCell: ({ row }: CellType) => (
      ...
      >
        {row?.railway_track_infrastructure_type_id ||
          row?.id.slice(0, 8) + "..."}
      </Typography>
    ),
  },
  ...
  },
  {
    ...
    headerName: t(
      "project.other.railway-track-data.details.rail-type-and-size",
    ),
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.rail_type_and_size !== undefined
          ? row.rail_type_and_size
          : t("common.not-available")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.sleepers_type_and_spacing !== undefined
          ? row.sleepers_type_and_spacing
          : t("common.not-available")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.fastening_systems !== undefined
          ? row.fastening_systems
          : t("common.not-available")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.ballast_type_and_depth !== undefined
          ? row.ballast_type_and_depth
          : t("common.not-available")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.track_connection_method !== undefined
          ? row.track_connection_method
          : t("common.not-available")}
      </Typography>
    ),
  },
