...
const RailwayStationDrawer = (props: RailwayStationDrawerType) => {
  ...
  const createRailwayStation = async (body: IApiPayload<RailwayStation>) =>
    projectOtherApiService<RailwayStation>().create(model, body);

  const editRailwayStation = async (body: IApiPayload<RailwayStation>) =>
    projectOtherApiService<RailwayStation>().update(
      model,
      railwayStation?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<RailwayStation>,
    payload: IApiPayload<RailwayStation>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.railwayStation,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-station.${
        isEdit ? `edit-railway-station` : `create-railway-station`
      }`}
