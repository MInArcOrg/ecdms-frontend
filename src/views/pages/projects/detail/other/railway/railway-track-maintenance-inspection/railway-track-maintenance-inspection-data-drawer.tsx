...
const RailwayTrackMaintenanceAndInspectionDrawer = (
  props: RailwayTrackMaintenanceAndInspectionDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createRailwayTrackMaintenanceAndInspection = async (
    body: IApiPayload<RailwayTrackMaintenanceAndInspection>,
  ) =>
    projectOtherApiSecondService<RailwayTrackMaintenanceAndInspection>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editRailwayTrackMaintenanceAndInspection = async (
    body: IApiPayload<RailwayTrackMaintenanceAndInspection>,
  ) =>
  ...
  const getPayload = (values: RailwayTrackMaintenanceAndInspection) => ({
    data: {
      ...
      scheduled_maintenance_activity_id:
        values.scheduled_maintenance_activity_id,
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway-track-maintenance-and-inspection.${
        isEdit
          ? `edit-railway-track-maintenance-and-inspection`
          : `create-railway-track-maintenance-and-inspection`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editRailwayTrackMaintenanceAndInspection
              : createRailwayTrackMaintenanceAndInspection
          }
