...
const RailwayTrackMaintenanceAndInspectionList: React.FC<
  RailwayTrackMaintenanceAndInspectionListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<RailwayTrackMaintenanceAndInspection | null>(null);
  const { t } = useTranslation();

  const fetchRailwayTrackData = (
    params: GetRequestParam,
  ): Promise<IApiResponse<RailwayTrackMaintenanceAndInspection[]>> => {
    return projectOtherApiSecondService<RailwayTrackMaintenanceAndInspection>().getAll(
  ...
  const handleDelete = async (id: string) => {
    await projectOtherApiSecondService<RailwayTrackMaintenanceAndInspection>().delete(
      otherSubMenu?.apiRoute || "",
      id,
    );
    refetch();
    ...
    <Box>
      {showDrawer && (
        <RailwayTrackDataDrawer
          ...
          railwayTrackMaintenanceAndInspection={
            selectedRow as RailwayTrackMaintenanceAndInspection
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          title={t(
            "project.other.railway-track-maintenance-and-inspection.railway-track-maintenance-and-inspection-details",
          )}
        />
      )}
      ...
      <ItemsListing
        title={t(
          "project.other.railway-track-maintenance-and-inspection.title",
        )}
        pagination={pagination}
        type={ITEMS_LISTING_TYPE.table.value}
        tableProps={{
          headers: railwayTrackMaintenanceAndInspectionColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
