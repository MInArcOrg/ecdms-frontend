...
const RailwayDrawer = (props: RailwayDrawerType) => {
  ...
  const createRailway = async (body: IApiPayload<Railway>) =>
    projectOtherApiService<Railway>().create(model, body);

  const editRailway = async (body: IApiPayload<Railway>) =>
    projectOtherApiService<Railway>().update(model, railway?.id || "", body);
  ...
  const onActionSuccess = async (
    response: IApiResponse<Railway>,
    payload: IApiPayload<Railway>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.railway,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.railway.${
        isEdit ? `edit-railway` : `create-railway`
      }`}
