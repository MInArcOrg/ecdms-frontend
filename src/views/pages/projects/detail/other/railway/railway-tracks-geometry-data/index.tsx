...
const RailwayTracksGeometryDataList: React.FC<RailwayTracksGeometryDataListProps> = ({ otherSubMenu, projectId, typeId }) => {
  ...
  const handleClickDetail = (railwayTracksGeometryData: RailwayTracksGeometryData) => {
    toggleDetailDrawer();
    setSelectedRow(railwayTracksGeometryData);
  };
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapRailwayTracksGeometryDataToDetailItems(selectedRow as RailwayTracksGeometryData)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={uploadableProjectFileTypes.other.electric_grid_control_center_data}
          title={t('project.other.railway-tracks-geometry-data.details-tracks-geometry-data-details')}
        />
      )}
