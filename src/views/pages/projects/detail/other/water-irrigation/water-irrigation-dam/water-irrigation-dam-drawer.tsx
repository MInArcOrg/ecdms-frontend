...
const WaterIrrigationDamDrawer = (props: WaterIrrigationDamDrawerType) => {
  ...
  const createWaterIrrigationDam = async (
    body: IApiPayload<WaterIrrigationDam>,
  ) => projectOtherApiService<WaterIrrigationDam>().create(model, body);

  const editWaterIrrigationDam = async (
    body: IApiPayload<WaterIrrigationDam>,
  ) =>
    projectOtherApiService<WaterIrrigationDam>().update(
      model,
      waterIrrigationDam?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<WaterIrrigationDam>,
    payload: IApiPayload<WaterIrrigationDam>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.waterIrrigationDam,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.water-irrigation-dam.${
        isEdit ? `edit-water-irrigation-dam` : `create-water-irrigation-dam`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editWaterIrrigationDam : createWaterIrrigationDam
          }
