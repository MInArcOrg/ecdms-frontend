...
const ReservoirInfoDrawer = (props: ReservoirInfoDrawerType) => {
  ...
  const createReservoirInfo = async (body: IApiPayload<ReservoirInfo>) =>
    projectOtherApiService<ReservoirInfo>().create(model, body);

  const editReservoirInfo = async (body: IApiPayload<ReservoirInfo>) =>
    projectOtherApiService<ReservoirInfo>().update(
      model,
      reservoirInfo?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<ReservoirInfo>,
    payload: IApiPayload<ReservoirInfo>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.reservoirInfo,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.reservoir-info.${
        isEdit ? `edit-reservoir-info` : `create-reservoir-info`
      }`}
