...
const IrrigationCapacityDrawer = (props: IrrigationCapacityDrawerType) => {
  ...
  const createIrrigationCapacity = async (
    body: IApiPayload<IrrigationCapacity>,
  ) => projectOtherApiService<IrrigationCapacity>().create(model, body);

  const editIrrigationCapacity = async (
    body: IApiPayload<IrrigationCapacity>,
  ) =>
    projectOtherApiService<IrrigationCapacity>().update(
      model,
      irrigationCapacity?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<IrrigationCapacity>,
    payload: IApiPayload<IrrigationCapacity>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.irrigationCapacity,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.irrigation-capacity.${
        isEdit ? `edit-irrigation-capacity` : `create-irrigation-capacity`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editIrrigationCapacity : createIrrigationCapacity
          }
