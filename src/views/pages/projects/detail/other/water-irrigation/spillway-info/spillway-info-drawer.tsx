...
const SpillwayInfoDrawer = (props: SpillwayInfoDrawerType) => {
  ...
  const createSpillwayInfo = async (body: IApiPayload<SpillwayInfo>) =>
    projectOtherApiService<SpillwayInfo>().create(model, body);

  const editSpillwayInfo = async (body: IApiPayload<SpillwayInfo>) =>
    projectOtherApiService<SpillwayInfo>().update(
      model,
      spillwayInfo?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<SpillwayInfo>,
    payload: IApiPayload<SpillwayInfo>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.spillwayInfo,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.spillway-info.${
        isEdit ? `edit-spillway-info` : `create-spillway-info`
      }`}
