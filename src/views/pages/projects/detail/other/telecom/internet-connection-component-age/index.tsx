...
const InternetConnectionInfrastructureAgeList: React.FC<
  InternetConnectionInfrastructureAgeListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<InternetConnectionInfrastructureAge | null>(null);
  const { t } = useTranslation();

  const fetchInternetConnectionInfrastructureAges = (
    params: GetRequestParam,
  ): Promise<IApiResponse<InternetConnectionInfrastructureAge[]>> => {
    return projectOtherApiSecondService<InternetConnectionInfrastructureAge>().getAll(
  ...
  const handleDelete = async (
    internetConnectionInfrastructureAgeId: string,
  ) => {
  ...
  const mapInternetConnectionInfrastructureAgeToDetailItems = (
    ...
    },
    {
      ...
      value: internetConnectionInfrastructureAge?.created_at
        ? formatCreatedAt(internetConnectionInfrastructureAge.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: internetConnectionInfrastructureAge?.updated_at
        ? formatCreatedAt(internetConnectionInfrastructureAge.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      {showDrawer && (
        <InternetConnectionInfrastructureAgeDrawer
          ...
          internetConnectionInfrastructureAge={
            selectedRow as InternetConnectionInfrastructureAge
          }
          refetch={refetch}
          projectId={projectId}
        />
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapInternetConnectionInfrastructureAgeToDetailItems(
            selectedRow as InternetConnectionInfrastructureAge,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={
            uploadableProjectFileTypes.other.internetConnectionInfrastructureAge
          }
          title={t(
            "project.other.internet-connection-infrastructure-age.internet-connection-infrastructure-age-details",
          )}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: internetConnectionInfrastructureAgeColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
