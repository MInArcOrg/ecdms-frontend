...
import type { MobileNetworkCapacity } from 'src/types/project/other';
import { formatCreatedAt } from 'src/utils/formatter/date';
import ModelAction from 'src/views/components/custom/model-actions';
import RowOptions from 'src/views/shared/listing/row-options';

interface CellType {
  row: MobileNetworkCapacity;
}

export const mobileNetworkCapacityColumns = (
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.total_bandwidth !== undefined ? `${row.total_bandwidth} Mbps` : t('common.not-available')}
      </Typography>
    )
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.users_number !== undefined ? row.users_number : t('common.not-available')}
      </Typography>
    )
  },
  {
