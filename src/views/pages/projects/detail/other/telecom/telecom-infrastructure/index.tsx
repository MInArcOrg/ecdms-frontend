...
const TelecomInfrastructureList: React.FC<TelecomInfrastructureListProps> = ({
  model,
  projectId,
  typeId,
}) => {
  ...
  const [selectedRow, setSelectedRow] = useState<TelecomInfrastructure | null>(
    null,
  ); // Updated type
  ...
  ): Promise<IApiResponse<TelecomInfrastructure[]>> => {
    // Updated type
  ...
  const handleDelete = async (telecomId: string) => {
    await projectOtherApiService<TelecomInfrastructure>().delete(
      model,
      telecomId,
    );
    refetch();
  ...
  const mapTelecomInfrastructureToDetailItems = (
    ...
    {
      title: t("project.other.telecom-infrastructure.details.name"),
      value: telecomInfrastructure?.name || "N/A",
    },
    {
    ...
    {
      title: t(
        "project.other.telecom-infrastructure.details.no-of-families-coverage",
      ),
      value:
        telecomInfrastructure?.no_of_families_coverage?.toString() || "N/A",
    },
    {
      title: t("project.other.telecom-infrastructure.details.service-period"),
      value: telecomInfrastructure?.service_period
        ? formatCreatedAt(telecomInfrastructure?.service_period)
        : "N/A",
    },
    {
      title: t(
        "project.other.telecom-infrastructure.details.inauguration-date",
      ),
      value: telecomInfrastructure?.inauguration_date
        ? formatCreatedAt(telecomInfrastructure?.inauguration_date)
        : "N/A",
    },
    {
      title: t("common.table-columns.created-at"),
      value: telecomInfrastructure?.created_at
        ? formatCreatedAt(telecomInfrastructure?.created_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapTelecomInfrastructureToDetailItems(
            selectedRow as TelecomInfrastructure,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={uploadableProjectFileTypes.other.telecomInfrastructure}
          title={t(
            "project.other.telecom-infrastructure.telecom-infrastructure-details",
          )}
        />
      )}
