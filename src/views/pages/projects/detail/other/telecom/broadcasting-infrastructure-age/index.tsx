...
const BroadcastingInfrastructureAgeList: React.FC<
  BroadcastingInfrastructureAgeListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<BroadcastingInfrastructureAge | null>(null);
  ...
  const broadcastingInfrastructureMap = new Map(
    broadcastingInfrastructures?.payload.map((infra) => [
      infra.id,
      infra.broadcasting_infrastructure_type_id,
    ]) || [],
  );
  ...
  const fetchBroadcastingInfrastructureAges = (
    ...
    return projectOtherApiSecondService<BroadcastingInfrastructureAge>().getAll(
  ...
  const mapBroadcastingInfrastructureAgeToDetailItems = (
    ...
    },
    {
      ...
      value: broadcastingInfrastructureAge?.created_at
        ? formatCreatedAt(broadcastingInfrastructureAge.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: broadcastingInfrastructureAge?.updated_at
        ? formatCreatedAt(broadcastingInfrastructureAge.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      {showDrawer && (
        <BroadcastingInfrastructureAgeDrawer
          ...
          toggle={toggleDrawer}
          broadcastingInfrastructureAge={
            selectedRow as BroadcastingInfrastructureAge
          }
          refetch={refetch}
          projectId={projectId}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapBroadcastingInfrastructureAgeToDetailItems(
            selectedRow as BroadcastingInfrastructureAge,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          ...
          title={t(
            "project.other.broadcasting-infrastructure-age.broadcasting-infrastructure-age-details",
          )}
        />
      )}
