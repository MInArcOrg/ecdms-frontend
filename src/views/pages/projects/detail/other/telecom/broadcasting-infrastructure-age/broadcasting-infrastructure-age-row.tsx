...
export const broadcastingInfrastructureAgeColumns = (
  ...
  },
  {
    ...
    field: 'name',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.name?.toString() || t('common.not-available')}</Typography>
    )
  },
  {
    flex: 0.15,
  ...
  },
  {
    flex: 0.15,
    minWidth: 100,
    headerName: t('project.other.broadcasting-infrastructure-age.details.transmitters'),
    field: 'transmitters',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.transmitters !== undefined ? `${row.transmitters} ${t('common.years')}` : t('common.not-available')}
      </Typography>
    )
  },
  {
    flex: 0.15,
    minWidth: 100,
    headerName: t('project.other.broadcasting-infrastructure-age.details.towers'),
    field: 'towers',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.towers !== undefined ? `${row.towers} ${t('common.years')}` : t('common.not-available')}
      </Typography>
    )
  },
  {
