...
export const broadcastingInfrastructureColumns = (
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      ...
      >
        {broadcastingInfrastructureTypeMap.get(row?.broadcasting_infrastructure_type_id) || t('common.not-available')}
      </Typography>
    )
  },
  {
    flex: 0.15,
    minWidth: 100,
    headerName: t('project.other.broadcasting-infrastructure.details.broadcasting-network'),
  ...
  },
  {
    ...
    field: 'antennas',
    renderCell: ({ row }: CellType) => (
      <Chip size="small" label={row?.antennas ? t('common.yes') : t('common.no')} color={row?.antennas ? 'success' : 'default'} />
    )
  },
  {
    flex: 0.15,
    minWidth: 100,
    headerName: t('project.other.broadcasting-infrastructure.details.transmitters'),
    field: 'transmitters',
    renderCell: ({ row }: CellType) => (
      <Chip size="small" label={row?.transmitters ? t('common.yes') : t('common.no')} color={row?.transmitters ? 'success' : 'default'} />
    )
  },
  {
    flex: 0.15,
