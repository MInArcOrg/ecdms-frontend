...
const BroadcastingInfrastructureDrawer = (
  props: BroadcastingInfrastructureDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createBroadcastingInfrastructure = async (
    body: IApiPayload<BroadcastingInfrastructure>,
  ) =>
    projectOtherApiSecondService<BroadcastingInfrastructure>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editBroadcastingInfrastructure = async (
    body: IApiPayload<BroadcastingInfrastructure>,
  ) =>
  ...
  const getPayload = (values: BroadcastingInfrastructure) => ({
    data: {
      ...
      broadcasting_infrastructure_type_id:
        values.broadcasting_infrastructure_type_id,
    ...
    <CustomSideDrawer
      title={`project.other.broadcasting-infrastructure.${
        isEdit
          ...
          : `create-broadcasting-infrastructure`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editBroadcastingInfrastructure
              : createBroadcastingInfrastructure
          }
        ...
        >
          {(formik: FormikProps<BroadcastingInfrastructure>) => {
            return (
              <BroadcastingInfrastructureForm
                file={uploadableFile}
