...
const DataCenterComponentManufacturerList: React.FC<DataCenterComponentManufacturerListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<DataCenterComponentManufacturer | null>(null);
  const { t } = useTranslation();

  const fetchDataCenterComponentManufacturers = (params: GetRequestParam): Promise<IApiResponse<DataCenterComponentManufacturer[]>> => {
    return projectOtherApiSecondService<DataCenterComponentManufacturer>().getAll(otherSubMenu?.apiRoute || '', {
      ...params
    });
  };
  ...
  const handleEdit = (dataCenterComponentManufacturer: DataCenterComponentManufacturer) => {
    toggleDrawer();
  ...
  const handleClickDetail = (dataCenterComponentManufacturer: DataCenterComponentManufacturer) => {
    toggleDetailDrawer();
    setSelectedRow(dataCenterComponentManufacturer);
  };
    ...
    <Box>
      {showDrawer && (
        <DataCenterComponentManufacturerDrawer
          ...
          open={showDrawer}
          toggle={toggleDrawer}
          dataCenterComponentManufacturer={selectedRow as DataCenterComponentManufacturer}
          refetch={refetch}
          projectId={projectId}
        />
      )}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapDataCenterComponentManufacturerToDetailItems(selectedRow as DataCenterComponentManufacturer)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={uploadableProjectFileTypes.other.dataCenterComponentManufacturer}
          title={t('project.other.data-center-component-manufacturer.data-center-component-manufacturer-details')}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: dataCenterComponentManufacturerColumns(handleClickDetail, handleEdit, handleDelete, t, refetch)
        }}
        isLoading={isLoading}
