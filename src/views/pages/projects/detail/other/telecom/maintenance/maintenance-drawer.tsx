...
const MaintenanceDrawer = (props: MaintenanceDrawerType) => {
  ...
  const createMaintenance = async (body: IApiPayload<Maintenance>) =>
    projectOtherApiSecondService<Maintenance>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editMaintenance = async (body: IApiPayload<Maintenance>) =>
    projectOtherApiSecondService<Maintenance>().update(
      otherSubMenu?.apiRoute || "",
      maintenance?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<Maintenance>,
    payload: IApiPayload<Maintenance>,
  ) => {
  ...
  return (
    <CustomSideDrawer
      title={`project.other.maintenance.${
