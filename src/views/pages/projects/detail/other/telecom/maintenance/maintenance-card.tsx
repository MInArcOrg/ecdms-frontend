...
const MaintenanceCard: React.FC<MaintenanceCardProps> = ({ maintenance, refetch, onEdit, onDelete, onDetail }) => {
    ...
    <Card sx={{ p: 2 }}>
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Typography variant="h5" fontWeight="bold">
            ...
            >
              {t('project.other.maintenance.maintenance-record')} - {maintenance?.id.slice(0, 8) + '...'}
            </Typography>
          </Typography>
        </Box>
        ...
        <Grid container spacing={2} mt={1}>
          <Grid item xs={12}>
            <Typography variant="body2" color="text.secondary" display="flex" alignItems="center" gap={1}>
              {t('project.other.maintenance.details.maintenance-frequency')}: {renderStatusChip(maintenance?.maintenance_frequency)}
            </Typography>
          </Grid>
          <Grid item xs={12}>
            <Typography variant="body2" color="text.secondary" display="flex" alignItems="center" gap={1}>
              {t('project.other.maintenance.details.service-level-agreement')}: {renderStatusChip(maintenance?.service_level_agreement)}
            </Typography>
          </Grid>
        </Grid>
        ...
        {maintenance?.remark && (
          <Box mt={2}>
            <Typography variant="body2" color="text.secondary">
              {t('project.other.maintenance.details.remark')}: {maintenance.remark}
            </Typography>
          </Box>
        )}
        ...
        {maintenance?.created_at && (
          <Typography variant="body2" color="text.secondary" mt={2}>
            {t('common.table-columns.created-at')}: {formatCreatedAt(maintenance.created_at)}
          </Typography>
        )}
      </CardContent>
      ...
      <CardActions sx={{ flexDirection: 'column', alignItems: 'flex-start' }}>
        <Box width="100%" display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          ...
          <FileDrawer id={maintenance.id} type={uploadableProjectFileTypes.other.maintenance} />
        </Box>
        ...
        <Box width="100%" display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          ...
          <FileDrawer id={maintenance.id} type={uploadableProjectFileTypes.other.infrastructureImage} />
        </Box>
