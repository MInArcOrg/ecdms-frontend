...
const MaintenanceList: React.FC<MaintenanceListProps> = ({
  ...
  ): Promise<IApiResponse<Maintenance[]>> => {
    return projectOtherApiSecondService<Maintenance>().getAll(
  ...
  const handleDelete = async (maintenanceId: string) => {
    await projectOtherApiSecondService<Maintenance>().delete(
      otherSubMenu?.apiRoute || "",
      maintenanceId,
    );
    refetch();
