...
const DataCenterDrawer = (props: DataCenterDrawerType) => {
  ...
  const createDataCenter = async (body: IApiPayload<DataCenter>) =>
    projectOtherApiSecondService<DataCenter>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editDataCenter = async (body: IApiPayload<DataCenter>) =>
    projectOtherApiSecondService<DataCenter>().update(
      otherSubMenu?.apiRoute || "",
      dataCenter?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<DataCenter>,
    payload: IApiPayload<DataCenter>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.dataCenter,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.data-center.${
        isEdit ? `edit-data-center` : `create-data-center`
      }`}
