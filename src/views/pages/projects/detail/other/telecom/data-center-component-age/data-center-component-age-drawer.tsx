...
const DataCenterComponentAgeDrawer = (
  props: DataCenterComponentAgeDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createDataCenterComponentAge = async (
    body: IApiPayload<DataCenterComponentAge>,
  ) =>
    projectOtherApiSecondService<DataCenterComponentAge>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editDataCenterComponentAge = async (
    body: IApiPayload<DataCenterComponentAge>,
  ) =>
    projectOtherApiSecondService<DataCenterComponentAge>().update(
      otherSubMenu?.apiRoute || "",
      dataCenterComponentAge?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<DataCenterComponentAge>,
    payload: IApiPayload<DataCenterComponentAge>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.dataCenterComponentAge,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.data-center-component-age.${
        isEdit
          ? `edit-data-center-component-age`
          : `create-data-center-component-age`
      }`}
        ...
        <FormPageWrapper
          ...
          initialValues={{
            ...
            networking_equipment:
              dataCenterComponentAge?.networking_equipment || 0,
          ...
          createActionFunc={
            isEdit ? editDataCenterComponentAge : createDataCenterComponentAge
          }
