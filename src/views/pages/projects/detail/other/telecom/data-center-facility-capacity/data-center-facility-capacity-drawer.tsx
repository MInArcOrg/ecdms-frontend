...
const DataCenterFacilityCapacityDrawer = (
  props: DataCenterFacilityCapacityDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createDataCenterFacilityCapacity = async (
    body: IApiPayload<DataCenterFacilityCapacity>,
  ) =>
    projectOtherApiSecondService<DataCenterFacilityCapacity>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editDataCenterFacilityCapacity = async (
    body: IApiPayload<DataCenterFacilityCapacity>,
  ) =>
  ...
  const onActionSuccess = async (
    response: IApiResponse<DataCenterFacilityCapacity>,
    payload: IApiPayload<DataCenterFacilityCapacity>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.dataCenterFacilityCapacity,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.data-center-facility-capacity.${
        isEdit
          ? `edit-data-center-facility-capacity`
          : `create-data-center-facility-capacity`
      }`}
      ...
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...
            total_floor_area:
              dataCenterFacilityCapacity?.total_floor_area || "",
            power_capacity: dataCenterFacilityCapacity?.power_capacity || "",
            rack_space_capacity:
              dataCenterFacilityCapacity?.rack_space_capacity || "",
            cooling_capacity:
              dataCenterFacilityCapacity?.cooling_capacity || "",
            access_control: dataCenterFacilityCapacity?.access_control || false,
            surveillance_cameras:
              dataCenterFacilityCapacity?.surveillance_cameras || false,
            fire_suppression_systems:
              dataCenterFacilityCapacity?.fire_suppression_systems || false,
            intrusion_detection_systems:
              dataCenterFacilityCapacity?.intrusion_detection_systems || false,
            others: dataCenterFacilityCapacity?.others || "",
          }}
          createActionFunc={
            isEdit
              ? editDataCenterFacilityCapacity
              : createDataCenterFacilityCapacity
          }
        ...
        >
          {(formik: FormikProps<DataCenterFacilityCapacity>) => {
            return (
              <DataCenterFacilityCapacityForm
                projectId={projectId}
                file={uploadableFile}
