...
const MobileNetworkComponentAgeList: React.FC<MobileNetworkComponentAgeListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<MobileNetworkComponentAge | null>(null);
  const { t } = useTranslation();

  const fetchMobileNetworkComponentAges = (params: GetRequestParam): Promise<IApiResponse<MobileNetworkComponentAge[]>> => {
    return projectOtherApiSecondService<MobileNetworkComponentAge>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleDelete = async (mobileNetworkComponentAgeId: string) => {
    await projectOtherApiSecondService<MobileNetworkComponentAge>().delete(otherSubMenu?.apiRoute || '', mobileNetworkComponentAgeId);
    refetch();
  };

  const handleClickDetail = (mobileNetworkComponentAge: MobileNetworkComponentAge) => {
    toggleDetailDrawer();
    setSelectedRow(mobileNetworkComponentAge);
  };
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapMobileNetworkComponentAgeToDetailItems(selectedRow as MobileNetworkComponentAge)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={uploadableProjectFileTypes.other.mobileNetworkComponentAge}
          title={t('project.other.mobile-network-component-age.mobile-network-component-age-details')}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: mobileNetworkComponentAgeColumns(handleClickDetail, handleEdit, handleDelete, t, refetch)
        }}
        isLoading={isLoading}
