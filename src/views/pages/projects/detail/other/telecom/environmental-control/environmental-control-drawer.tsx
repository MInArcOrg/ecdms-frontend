...
const EnvironmentalControlDrawer = (props: EnvironmentalControlDrawerType) => {
  ...
  const createEnvironmentalControl = async (
    body: IApiPayload<EnvironmentalControl>,
  ) =>
    projectOtherApiSecondService<EnvironmentalControl>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editEnvironmentalControl = async (
    body: IApiPayload<EnvironmentalControl>,
  ) =>
    projectOtherApiSecondService<EnvironmentalControl>().update(
      otherSubMenu?.apiRoute || "",
      environmentalControl?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<EnvironmentalControl>,
    payload: IApiPayload<EnvironmentalControl>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.environmentalControl,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.environmental-control.${
        isEdit ? `edit-environmental-control` : `create-environmental-control`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editEnvironmentalControl : createEnvironmentalControl
          }
