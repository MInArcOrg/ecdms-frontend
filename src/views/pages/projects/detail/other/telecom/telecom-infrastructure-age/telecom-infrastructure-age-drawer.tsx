...
const TelecomInfrastructureAgeDrawer = (props: TelecomInfrastructureAgeDrawerType) => {
  ...
  const editTelecomInfrastructureAge = async (body: IApiPayload<TelecomInfrastructureAge>) =>
    projectOtherApiSecondService<TelecomInfrastructureAge>().update(otherSubMenu?.apiRoute || '', telecomInfrastructureAge?.id || '', body);
  ...
  const onActionSuccess = async (response: IApiResponse<TelecomInfrastructureAge>, payload: IApiPayload<TelecomInfrastructureAge>) => {
    if (payload.files.length > 0) {
      await uploadFile(payload.files[0], uploadableProjectFileTypes.other.infrastructureAge, response.payload.id, '', '');
    }
