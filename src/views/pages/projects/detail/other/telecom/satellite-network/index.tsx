...
const SatelliteNetworkList: React.FC<SatelliteNetworkListProps> = ({ otherSubMenu, projectId, typeId }) => {
  ...
  const satelliteNetworkTypeMap = new Map(satelliteNetworkTypes?.payload.map((item) => [item.id, item.title || 'N/A']) || []);
  ...
  const mapSatelliteNetworkToDetailItems = (satelliteNetwork: SatelliteNetwork): { title: string; value: string }[] => [
    ...
    },
    {
      ...
      value: satelliteNetwork?.satellite !== undefined ? (satelliteNetwork.satellite ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
      title: t('project.other.satellite-network.details.ground-stations'),
      value: satelliteNetwork?.ground_stations !== undefined ? (satelliteNetwork.ground_stations ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
      title: t('project.other.satellite-network.details.modems'),
      value: satelliteNetwork?.modems !== undefined ? (satelliteNetwork.modems ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
      title: t('project.other.satellite-network.details.routers'),
      value: satelliteNetwork?.routers !== undefined ? (satelliteNetwork.routers ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: satelliteNetworkColumns(handleClickDetail, handleEdit, handleDelete, t, refetch, satelliteNetworkTypeMap)
        }}
        isLoading={isLoading}
