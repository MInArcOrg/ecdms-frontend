...
const SatelliteNetworkList: React.FC<SatelliteNetworkListProps> = ({
  otherSubMenu,
  projectId,
  typeId,
}) => {
  ...
  const satelliteNetworkTypeMap = new Map(
    satelliteNetworkTypes?.payload.map((item) => [
      item.id,
      item.title || "N/A",
    ]) || [],
  );

  const fetchSatelliteNetworks = (
    params: GetRequestParam,
  ): Promise<IApiResponse<SatelliteNetwork[]>> => {
    return projectOtherApiSecondService<SatelliteNetwork>().getAll(
  ...
  const handleDelete = async (satelliteNetworkId: string) => {
    await projectOtherApiSecondService<SatelliteNetwork>().delete(
      otherSubMenu?.apiRoute || "",
      satelliteNetworkId,
    );
    refetch();
  ...
  const mapSatelliteNetworkToDetailItems = (
    ...
    },
    {
      ...
      value:
        satelliteNetwork?.modems !== undefined
          ? satelliteNetwork.modems
            ? t("common.yes")
            : t("common.no")
          : "N/A",
    },
    {
      ...
      value:
        satelliteNetwork?.routers !== undefined
          ? satelliteNetwork.routers
            ? t("common.yes")
            : t("common.no")
          : "N/A",
    },
    ...
    },
    {
      ...
      value: satelliteNetwork?.created_at
        ? formatCreatedAt(satelliteNetwork.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: satelliteNetwork?.updated_at
        ? formatCreatedAt(satelliteNetwork.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapSatelliteNetworkToDetailItems(
            selectedRow as SatelliteNetwork,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
