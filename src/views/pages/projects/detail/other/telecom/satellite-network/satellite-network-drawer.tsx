...
const SatelliteNetworkDrawer = (props: SatelliteNetworkDrawerType) => {
  const { open, toggle, refetch, satelliteNetwork, projectId, otherSubMenu } =
    props;
  ...
  const createSatelliteNetwork = async (body: IApiPayload<SatelliteNetwork>) =>
    projectOtherApiSecondService<SatelliteNetwork>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editSatelliteNetwork = async (body: IApiPayload<SatelliteNetwork>) =>
    projectOtherApiSecondService<SatelliteNetwork>().update(
      otherSubMenu?.apiRoute || "",
      satelliteNetwork?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<SatelliteNetwork>,
    payload: IApiPayload<SatelliteNetwork>,
  ) => {
    if (payload.files.length > 0) {
      await uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.satelliteNetwork,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.satellite-network.${
        isEdit ? `edit-satellite-network` : `create-satellite-network`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editSatelliteNetwork : createSatelliteNetwork
          }
