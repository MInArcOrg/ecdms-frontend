...
export const internetConnectionColumns = (
...
): GridColDef[] => [
  {
    ...
    headerName: t('project.other.internet-connection.details.internet-connection-type'),
    renderCell: ({ row }: CellType) => (
      ...
      >
        {internetConnectionTypeMap.get(row?.internet_connection_type_id) || t('common.not-available')}
      </Typography>
    )
  },
  {
  ...
  },
  {
    ...
    field: 'switches',
    renderCell: ({ row }: CellType) => (
      <Chip size="small" label={row?.switches ? t('common.yes') : t('common.no')} color={row?.switches ? 'success' : 'default'} />
    )
  },
  {
    flex: 0.15,
  ...
  },
  {
    ...
    field: 'cables',
    renderCell: ({ row }: CellType) => (
      <Chip size="small" label={row?.cables ? t('common.yes') : t('common.no')} color={row?.cables ? 'success' : 'default'} />
    )
  },
  {
    flex: 0.15,
