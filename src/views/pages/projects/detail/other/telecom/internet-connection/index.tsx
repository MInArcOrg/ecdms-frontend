...
const InternetConnectionList: React.FC<InternetConnectionListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<InternetConnection | null>(null);
  ...
  const internetConnectionTypeMap = new Map(internetConnectionTypes?.payload.map((item) => [item.id, item.title || 'N/A']) || []);

  const fetchInternetConnections = (params: GetRequestParam): Promise<IApiResponse<InternetConnection[]>> => {
    return projectOtherApiSecondService<InternetConnection>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const mapInternetConnectionToDetailItems = (internetConnection: InternetConnection): { title: string; value: string }[] => [
    {
      title: t('project.other.internet-connection.details.internet-connection-type'),
      value:
        internetConnectionTypeMap.get(internetConnection?.internet_connection_type_id) ||
        internetConnection?.internet_connection_type_id ||
        'N/A'
    },
    {
      title: t('project.other.internet-connection.details.routers'),
      value: internetConnection?.routers !== undefined ? (internetConnection.routers ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
      title: t('project.other.internet-connection.details.switches'),
      value: internetConnection?.switches !== undefined ? (internetConnection.switches ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
      title: t('project.other.internet-connection.details.modems'),
      value: internetConnection?.modems !== undefined ? (internetConnection.modems ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
      title: t('project.other.internet-connection.details.cables'),
      value: internetConnection?.cables !== undefined ? (internetConnection.cables ? t('common.yes') : t('common.no')) : 'N/A'
    },
    {
    ...
    },
    {
      ...
      value: internetConnection?.created_at ? formatCreatedAt(internetConnection.created_at) : 'N/A'
    },
    {
      title: t('common.table-columns.updated-at'),
      value: internetConnection?.updated_at ? formatCreatedAt(internetConnection.updated_at) : 'N/A'
    }
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapInternetConnectionToDetailItems(selectedRow as InternetConnection)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={uploadableProjectFileTypes.other.internetConnection}
          title={t('project.other.internet-connection.internet-connection-details')}
        />
      )}
