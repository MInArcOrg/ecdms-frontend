...
const MiniGridStationDistributionLineDrawer = (props: MiniGridStationDistributionLineDrawerType) => {
  const { open, toggle, refetch, miniGridStationDistributionLine, projectId, otherSubMenu, miniGridStations } = props
  ...
  const createMiniGridStationDistributionLine = async (body: IApiPayload<MiniGridStationDistributionLine>) =>
    projectOtherApiSecondService<MiniGridStationDistributionLine>().create(otherSubMenu?.apiRoute || "", body)

  const editMiniGridStationDistributionLine = async (body: IApiPayload<MiniGridStationDistributionLine>) =>
  ...
  const onActionSuccess = async (response: IApiResponse<MiniGridStationDistributionLine>, payload: IApiPayload<MiniGridStationDistributionLine>) => {
    if (payload.files.length > 0) {
      await uploadFile(payload.files[0], uploadableProjectFileTypes.other.mini_grid_station_distribution_line, response.payload.id, "", "")
    }
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={isEdit ? editMiniGridStationDistributionLine : createMiniGridStationDistributionLine}
          onActionSuccess={onActionSuccess}
          onCancel={handleClose}
        >
          {(formik: FormikProps<MiniGridStationDistributionLine>) => {
            return <MiniGridStationDistributionLineForm 
              file={uploadableFile} 
