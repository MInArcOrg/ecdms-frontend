...
const MiniGridStationDistributionLineList: React.FC<
  MiniGridStationDistributionLineListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<MiniGridStationDistributionLine | null>(null);
  ...
  const fetchMiniGridStationDistributionLines = (
    params: GetRequestParam,
  ): Promise<IApiResponse<MiniGridStationDistributionLine[]>> => {
    return projectOtherApiSecondService<MiniGridStationDistributionLine>().getAll(
  ...
  const handleDelete = async (miniGridStationDistributionLineId: string) => {
    await projectOtherApiSecondService<MiniGridStationDistributionLine>().delete(
      otherSubMenu?.apiRoute || "",
      miniGridStationDistributionLineId,
    );
    refetch();
  };
  ...
  const mapMiniGridStationDistributionLineToDetailItems = (
    ...
    {
      title: t(
        "project.other.mini-grid-station-distribution-line.details.mini-grid-station-id",
      ),
    ...
    {
      title: t(
        "project.other.mini-grid-station-distribution-line.details.transformer-type-id",
      ),
    ...
    {
      title: t(
        "project.other.mini-grid-station-distribution-line.details.transformers-number",
      ),
      value:
    ...
    {
      title: t(
        "project.other.mini-grid-station-distribution-line.details.transformers-size",
      ),
      value:
        miniGridStationDistributionLine?.transformers_size !== undefined
          ? `${miniGridStationDistributionLine.transformers_size} ${t(
              "common.kva",
            )}`
          : "N/A",
    },
    {
      title: t(
        "project.other.mini-grid-station-distribution-line.details.remark",
      ),
      value: miniGridStationDistributionLine?.remark || "N/A",
    },
    {
      title: t("common.table-columns.created-at"),
      value: miniGridStationDistributionLine?.created_at
        ? formatCreatedAt(miniGridStationDistributionLine.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: miniGridStationDistributionLine?.updated_at
        ? formatCreatedAt(miniGridStationDistributionLine.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      {showDrawer && (
        <MiniGridStationDistributionLineDrawer
          ...
          toggle={toggleDrawer}
          miniGridStationDistributionLine={
            selectedRow as MiniGridStationDistributionLine
          }
          refetch={refetch}
          projectId={projectId}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapMiniGridStationDistributionLineToDetailItems(
            selectedRow as MiniGridStationDistributionLine,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          ...
          title={t(
            "project.other.mini-grid-station-distribution-line.mini-grid-station-distribution-line-details",
          )}
        />
      )}
