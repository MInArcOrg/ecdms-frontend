...
const ElectricGridControlCenterDataList: React.FC<ElectricGridControlCenterDataListProps> = ({ otherSubMenu, projectId, typeId }) => {
  ...
  const [selectedRow, setSelectedRow] = useState<ElectricGridControlCenterData | null>(null)
  ...
  const fetchElectricGridControlCenterData = (params: GetRequestParam): Promise<IApiResponse<ElectricGridControlCenterData[]>> => {
    return projectOtherApiSecondService<ElectricGridControlCenterData>().getAll(otherSubMenu?.apiRoute || "", {})
  }
  ...
  const handleEdit = (electricGridControlCenterData: ElectricGridControlCenterData) => {
    toggleDrawer()
    setSelectedRow(electricGridControlCenterData)
  }

  const handleDelete = async (electricGridControlCenterDataId: string) => {
    await projectOtherApiSecondService<ElectricGridControlCenterData>().delete(otherSubMenu?.apiRoute || "", electricGridControlCenterDataId)
    refetch()
  }

  const handleClickDetail = (electricGridControlCenterData: ElectricGridControlCenterData) => {
    toggleDetailDrawer()
    setSelectedRow(electricGridControlCenterData)
  }
  ...
  const controlSystemTypesMap = new Map(controlSystemTypes?.payload.map((item: any) => [item.id, item.title || '']) || []);
  const communicationLinksMap = new Map(communicationLinks?.payload.map((item: any) => [item.id, item.title || '']) || []);

  const mapElectricGridControlCenterDataToDetailItems = (electricGridControlCenterData: ElectricGridControlCenterData): { title: string; value: string }[] => [
    {
    ...
    {
      title: t("project.other.electric-grid-control-center-data.details.remark"),
      value: electricGridControlCenterData?.remark || "N/A",
    },
    {
      title: t("common.table-columns.created-at"),
      value: electricGridControlCenterData?.created_at ? formatCreatedAt(electricGridControlCenterData.created_at) : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: electricGridControlCenterData?.updated_at ? formatCreatedAt(electricGridControlCenterData.updated_at) : "N/A",
    },
  ]
    ...
    <Box>
      {showDrawer && (
        <ElectricGridControlCenterDataDrawer
          ...
          open={showDrawer}
          toggle={toggleDrawer}
          electricGridControlCenterData={selectedRow as ElectricGridControlCenterData}
          refetch={refetch}
          projectId={projectId}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapElectricGridControlCenterDataToDetailItems(selectedRow as ElectricGridControlCenterData)}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={uploadableProjectFileTypes.other.electric_grid_control_center_data}
          title={t("project.other.electric-grid-control-center-data.electric-grid-control-center-data-details")}
        />
      )}
