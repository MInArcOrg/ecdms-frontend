...
export const damColumns = (
...
): GridColDef[] => [
  {
    ...
    renderCell: ({ row }: CellType) => (
      ...
      >
        {row?.dam_type_id ? damTypeMap.get(row.dam_type_id) || t('common.not-available') : row?.id.slice(0, 8) + '...'}
      </Typography>
    )
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.dam_height !== undefined ? `${row.dam_height} ${t('common.meters')}` : t('common.not-available')}
      </Typography>
    )
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.spillway_type_id ? spillwayTypeMap.get(row.spillway_type_id) || t('common.not-available') : t('common.not-available')}
      </Typography>
    )
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.turbine_type_id ? turbineTypeMap.get(row.turbine_type_id) || t('common.not-available') : t('common.not-available')}
      </Typography>
    )
  },
  {
