      ...
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Typography variant="h5" fontWeight="bold">
            ...
            >
              {dam?.dam_type_id ? damTypeMap.get(dam.dam_type_id) : dam?.id.slice(0, 8) + '...'}
            </Typography>
          </Typography>
        </Box>
        ...
        <Grid container spacing={2} mt={1}>
          {dam?.dam_height !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.dam.details.dam-height')}: {dam.dam_height} {t('common.meters')}
              </Typography>
            </Grid>
          )}
          ...
          {dam?.spillway_type_id && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.dam.details.spillway-type')}: {spillwayTypeMap.get(dam.spillway_type_id) || 'N/A'}
              </Typography>
            </Grid>
          )}
          ...
          {dam?.penstock_length !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.dam.details.penstock-length')}: {dam.penstock_length} {t('common.meters')}
              </Typography>
            </Grid>
          )}
          ...
          {dam?.turbine_type_id && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.dam.details.turbine-type')}: {turbineTypeMap.get(dam.turbine_type_id) || 'N/A'}
              </Typography>
            </Grid>
          )}
          ...
          {dam?.turbine_number !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.dam.details.turbine-number')}: {dam.turbine_number}
              </Typography>
            </Grid>
          )}
          ...
          {dam?.generator_type_id && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.dam.details.generator-type')}: {generatorTypeMap.get(dam.generator_type_id) || 'N/A'}
              </Typography>
            </Grid>
          )}
        </Grid>
        ...
        {dam?.created_at && (
          <Typography variant="body2" color="text.secondary" mt={2}>
            {t('common.table-columns.created-at')}: {formatCreatedAt(dam.created_at)}
          </Typography>
        )}
      </CardContent>
