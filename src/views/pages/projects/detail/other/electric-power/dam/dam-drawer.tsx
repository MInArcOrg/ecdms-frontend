...
const DamDrawer = (props: DamDrawerType) => {
  ...
  const createDam = async (body: IApiPayload<Dam>) => projectOtherApiSecondService<Dam>().create(otherSubMenu?.apiRoute || '', body);

  const editDam = async (body: IApiPayload<Dam>) =>
    projectOtherApiSecondService<Dam>().update(otherSubMenu?.apiRoute || '', dam?.id || '', body);
  ...
  const onActionSuccess = async (response: IApiResponse<Dam>, payload: IApiPayload<Dam>) => {
    if (payload.files.length > 0) {
      await uploadFile(payload.files[0], uploadableProjectFileTypes.other.dam, response.payload.id, '', '');
    }
  ...
  return (
    <CustomSideDrawer title={`project.other.dam.${isEdit ? `edit-dam` : `create-dam`}`} handleClose={handleClose} open={open}>
      {() => (
        <FormPageWrapper
          edit={isEdit}
        ...
        >
          {(formik: FormikProps<Dam>) => {
            return <DamForm file={uploadableFile} onFileChange={onFileChange} formik={formik} />;
          }}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
