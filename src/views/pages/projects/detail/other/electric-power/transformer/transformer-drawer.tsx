...
const TransformerDrawer = (props: TransformerDrawerType) => {
  ...
  const createTransformer = async (body: IApiPayload<Transformer>) =>
    projectOtherApiService<Transformer>().create(model, body);

  const editTransformer = async (body: IApiPayload<Transformer>) =>
    projectOtherApiService<Transformer>().update(
      model,
      transformer?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<Transformer>,
    payload: IApiPayload<Transformer>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.transformer,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.transformer.${
        isEdit ? `edit-transformer` : `create-transformer`
      }`}
