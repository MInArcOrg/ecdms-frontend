...
const MiniGridStationBackupPowerSourceList: React.FC<MiniGridStationBackupPowerSourceListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false)
  const [showDetailDrawer, setShowDetailDrawer] = useState(false)
  const [selectedRow, setSelectedRow] = useState<MiniGridStationBackupPowerSource | null>(null)
  ...
  const fetchMiniGridStationBackupPowerSources = (params: GetRequestParam): Promise<IApiResponse<MiniGridStationBackupPowerSource[]>> => {
    return projectOtherApiSecondService<MiniGridStationBackupPowerSource>().getAll(otherSubMenu?.apiRoute || "", {})
  }
  ...
  const handleEdit = (miniGridStationBackupPowerSource: MiniGridStationBackupPowerSource) => {
    toggleDrawer()
    setSelectedRow(miniGridStationBackupPowerSource)
  }

  const handleDelete = async (miniGridStationBackupPowerSourceId: string) => {
    await projectOtherApiSecondService<MiniGridStationBackupPowerSource>().delete(otherSubMenu?.apiRoute || "", miniGridStationBackupPowerSourceId)
    refetch()
  }

  const handleClickDetail = (miniGridStationBackupPowerSource: MiniGridStationBackupPowerSource) => {
    toggleDetailDrawer()
    setSelectedRow(miniGridStationBackupPowerSource)
  }
  ...
  const mapMiniGridStationBackupPowerSourceToDetailItems = (miniGridStationBackupPowerSource: MiniGridStationBackupPowerSource): { title: string; value: string }[] => [
    {
    ...
    },
    {
      ...
      value: miniGridStationBackupPowerSource?.mini_grid_station_id ? miniGridStationsMap.get(miniGridStationBackupPowerSource?.mini_grid_station_id) || miniGridStationBackupPowerSource?.mini_grid_station_id : 'N/A'
    },
    {
      title: t("project.other.mini-grid-station-backup-power-source.details.capacity"),
      value: miniGridStationBackupPowerSource?.capacity !== undefined 
    ...
    },
    {
      ...
      value: miniGridStationBackupPowerSource?.installation_year !== undefined 
    ...
    },
    {
      ...
      value: miniGridStationBackupPowerSource?.distribution_lines_total_length !== undefined 
    ...
    },
    {
      ...
      value: miniGridStationBackupPowerSource?.lifetime !== undefined 
    ...
    <Box>
      {showDrawer && (
        <MiniGridStationBackupPowerSourceDrawer
          ...
          open={showDrawer}
          toggle={toggleDrawer}
          miniGridStationBackupPowerSource={selectedRow as MiniGridStationBackupPowerSource}
          refetch={refetch}
          projectId={projectId}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapMiniGridStationBackupPowerSourceToDetailItems(selectedRow as MiniGridStationBackupPowerSource)}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={uploadableProjectFileTypes.other.mini_grid_station_backup_power_source}
          title={t("project.other.mini-grid-station-backup-power-source.mini-grid-station-backup-power-source-details")}
        />
      )}
