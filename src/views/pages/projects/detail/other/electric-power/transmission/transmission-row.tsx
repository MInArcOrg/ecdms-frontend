...
export const transmissionColumns = (
...
): GridColDef[] => [
  {
    ...
    renderCell: ({ row }: CellType) => (
      ...
      >
        {row?.transmission_voltage !== undefined
          ? `${row.transmission_voltage} ${t("common.kv")}`
          : row?.id.slice(0, 8) + "..."}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.distance_to_substation !== undefined
          ? `${row.distance_to_substation} ${t("common.kilometers")}`
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.transmission_lines_number !== undefined
          ? row.transmission_lines_number
          : t("common.not-available")}
      </Typography>
    ),
  },
