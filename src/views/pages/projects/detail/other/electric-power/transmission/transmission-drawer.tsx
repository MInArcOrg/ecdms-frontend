...
const TransmissionDrawer = (props: TransmissionDrawerType) => {
  const { open, toggle, refetch, transmission, projectId, otherSubMenu } =
    props;
  ...
  const createTransmission = async (body: IApiPayload<Transmission>) =>
    projectOtherApiSecondService<Transmission>().create(
  ...
  const onActionSuccess = async (
    response: IApiResponse<Transmission>,
    payload: IApiPayload<Transmission>,
  ) => {
    if (payload.files.length > 0) {
      await uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.transmission,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.transmission.${
        isEdit ? `edit-transmission` : `create-transmission`
      }`}
