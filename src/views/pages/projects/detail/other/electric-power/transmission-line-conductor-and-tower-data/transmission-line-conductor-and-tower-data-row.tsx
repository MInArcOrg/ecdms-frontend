...
export const transmissionLineConductorAndTowerDataColumns = (
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row?.conductor_type || t('common.not-available')}</Typography>
    )
  },
  {
    flex: 0.15,
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.strands_number !== undefined ? row.strands_number : t('common.not-available')}
      </Typography>
    )
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.conductor_size !== undefined ? `${row.conductor_size} ${t('common.mm2')}` : t('common.not-available')}
      </Typography>
    )
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.tower_height !== undefined ? `${row.tower_height} ${t('common.meters')}` : t('common.not-available')}
      </Typography>
    )
  },
  {
