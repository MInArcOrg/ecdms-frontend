...
const ReliabilityAndMaintenanceList: React.FC<
  ReliabilityAndMaintenanceListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<ReliabilityAndMaintenance | null>(null);
  ...
  const fetchReliabilityAndMaintenance = (
    params: GetRequestParam,
  ): Promise<IApiResponse<ReliabilityAndMaintenance[]>> => {
    return projectOtherApiSecondService<ReliabilityAndMaintenance>().getAll(
  ...
  const handleDelete = async (reliabilityAndMaintenanceId: string) => {
    await projectOtherApiSecondService<ReliabilityAndMaintenance>().delete(
      otherSubMenu?.apiRoute || "",
      reliabilityAndMaintenanceId,
    );
    refetch();
  };
  ...
  const mapReliabilityAndMaintenanceToDetailItems = (
    ...
    },
    {
      ...
      value:
        reliabilityAndMaintenance?.total_outage_duration !== undefined
          ? `${reliabilityAndMaintenance.total_outage_duration} ${t(
              "common.hours",
            )}`
          : "N/A",
    },
    ...
    },
    {
      ...
      value:
        reliabilityAndMaintenance?.saidi !== undefined
          ? reliabilityAndMaintenance.saidi.toString()
          : "N/A",
    },
    {
      ...
      value:
        reliabilityAndMaintenance?.saifi !== undefined
          ? reliabilityAndMaintenance.saifi.toString()
          : "N/A",
    },
    {
      title: t(
    ...
    },
    {
      ...
      value: reliabilityAndMaintenance?.created_at
        ? formatCreatedAt(reliabilityAndMaintenance.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: reliabilityAndMaintenance?.updated_at
        ? formatCreatedAt(reliabilityAndMaintenance.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapReliabilityAndMaintenanceToDetailItems(
            selectedRow as ReliabilityAndMaintenance,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={uploadableProjectFileTypes.other.reliabilityAndMaintenance}
          title={t(
            "project.other.reliability-and-maintenance.reliability-and-maintenance-details",
          )}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: reliabilityAndMaintenanceColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
