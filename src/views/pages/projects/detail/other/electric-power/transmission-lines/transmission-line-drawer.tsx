...
const TransmissionLineDrawer = (props: TransmissionLineDrawerType) => {
  ...
  const editTransmissionLine = async (body: IApiPayload<TransmissionLine>) =>
    projectOtherApiService<TransmissionLine>().update(
      model,
      transmissionLine?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<TransmissionLine>,
    payload: IApiPayload<TransmissionLine>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.transmissionLine,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.transmission-line.${
        isEdit ? `edit-transmission-line` : `create-transmission-line`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editTransmissionLine : createTransmissionLine
          }
