...
const ElectricGridControlCenterCyberSecurityDataDrawer = (props: ElectricGridControlCenterCyberSecurityDataDrawerType) => {
  ...
  const validationSchema = yup.object().shape({
    ...
    cyber_security_measures_type: yup.string().uuid().required('Cyber Security Measures Type is required'),
    cyber_security_audits_frequency: yup.string().uuid().required('Cyber Security Audits Frequency is required'),
  ...
  const onActionSuccess = async (
    ...
    if (payload.files.length > 0) {
      await uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.electric_grid_control_center_cyber_security_data,
        response.payload.id,
        '',
        ''
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.electric-grid-control-center-cyber-security-data.${
        isEdit ? `edit-electric-grid-control-center-cyber-security-data` : `create-electric-grid-control-center-cyber-security-data`
      }`}
