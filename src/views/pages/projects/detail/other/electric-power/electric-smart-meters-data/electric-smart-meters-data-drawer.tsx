...
const ElectricSmartMetersDataDrawer = (
  props: ElectricSmartMetersDataDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createElectricSmartMetersData = async (
    body: IApiPayload<ElectricSmartMetersData>,
  ) =>
    projectOtherApiSecondService<ElectricSmartMetersData>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editElectricSmartMetersData = async (
    body: IApiPayload<ElectricSmartMetersData>,
  ) =>
    projectOtherApiSecondService<ElectricSmartMetersData>().update(
      otherSubMenu?.apiRoute || "",
      electricSmartMetersData?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<ElectricSmartMetersData>,
    payload: IApiPayload<ElectricSmartMetersData>,
  ) => {
    if (payload.files.length > 0) {
      await uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.electric_smart_meters_data,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.electric-smart-meters-data.${
        isEdit
          ? `edit-electric-smart-meters-data`
          : `create-electric-smart-meters-data`
      }`}
      ...
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...electricSmartMetersData,
            facility_type:
              electricSmartMetersData?.facility_type || "Oil Immersed",
          }}
          createActionFunc={
            isEdit ? editElectricSmartMetersData : createElectricSmartMetersData
          }
