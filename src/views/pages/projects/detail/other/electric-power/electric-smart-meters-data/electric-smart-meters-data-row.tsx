...
export const electricSmartMetersDataColumns = (
  ...
  },
  {
    flex: 0.15,
    minWidth: 120,
    headerName: t("project.other.electric-smart-meters-data.details.mini-grid-station-id"),
  ...
  },
  {
    flex: 0.15,
    minWidth: 120,
    headerName: t("project.other.electric-smart-meters-data.details.facility-type"),
    field: "facility_type",
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.model_id ? smartMeterModelsMap.get(row.model_id) || row.model_id : t("common.not-available")}
      </Typography>
    ),
  },
  {
    flex: 0.15,
    minWidth: 120,
    headerName: t("project.other.electric-smart-meters-data.details.smart-meter-type-id"),
    field: "smart_meter_type_id",
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.smart_meter_type_id ? smartMeterTypesMap.get(row.smart_meter_type_id) || row.smart_meter_type_id : t("common.not-available")}
      </Typography>
    ),
  },
  {
