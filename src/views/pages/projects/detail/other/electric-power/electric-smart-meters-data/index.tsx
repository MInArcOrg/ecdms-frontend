...
const ElectricSmartMetersDataList: React.FC<
  ElectricSmartMetersDataListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<ElectricSmartMetersData | null>(null);
  ...
  const fetchElectricSmartMetersData = (
    params: GetRequestParam,
  ): Promise<IApiResponse<ElectricSmartMetersData[]>> => {
    return projectOtherApiSecondService<ElectricSmartMetersData>().getAll(
  ...
  const handleDelete = async (electricSmartMetersDataId: string) => {
    await projectOtherApiSecondService<ElectricSmartMetersData>().delete(
      otherSubMenu?.apiRoute || "",
      electricSmartMetersDataId,
    );
    refetch();
  };
  ...
  const modelMap = new Map(
    model?.payload.map((item: any) => [item.id, item.title || ""]) || [],
  );
  const smartMeterTypesMap = new Map(
    smartMeterTypes?.payload.map((item: any) => [item.id, item.title || ""]) ||
      [],
  );
  ...
  const mapElectricSmartMetersDataToDetailItems = (
    ...
    },
    {
      ...
      value: electricSmartMetersData?.model_id
    ...
    },
    {
      ...
      value: electricSmartMetersData?.smart_meter_type_id
        ? smartMeterTypesMap.get(electricSmartMetersData.smart_meter_type_id) ||
    ...
    },
    {
      ...
      value:
        electricSmartMetersData?.smart_meters_installed_number?.toString() ||
        "N/A",
    },
    {
    ...
    },
    {
      ...
      value: electricSmartMetersData?.created_at
        ? formatCreatedAt(electricSmartMetersData.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: electricSmartMetersData?.updated_at
        ? formatCreatedAt(electricSmartMetersData.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapElectricSmartMetersDataToDetailItems(
            selectedRow as ElectricSmartMetersData,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={uploadableProjectFileTypes.other.electric_smart_meters_data}
          title={t(
            "project.other.electric-smart-meters-data.electric-smart-meters-data-details",
          )}
        />
      )}
