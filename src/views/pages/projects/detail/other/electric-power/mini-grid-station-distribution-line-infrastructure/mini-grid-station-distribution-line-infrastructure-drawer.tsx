...
const MiniGridStationDistributionLineInfrastructureDrawer = (props: MiniGridStationDistributionLineInfrastructureDrawerType) => {
    const { open, toggle, refetch, miniGridStationDistributionLineInfrastructure, projectId, otherSubMenu, miniGridStations, distributionLineTypes, distributionLineMaterials } = props
    ...
    const onActionSuccess = async (response: IApiResponse<MiniGridStationDistributionLineInfrastructure>, payload: IApiPayload<MiniGridStationDistributionLineInfrastructure>) => {
        if (payload.files.length > 0) {
            await uploadFile(payload.files[0], uploadableProjectFileTypes.other.mini_grid_station_distribution_line_infrastructure, response.payload.id, "", "")
        }
    ...
    return (
        <CustomSideDrawer
            title={`project.other.mini-grid-station-distribution-line-infrastructure.${isEdit ? `edit-mini-grid-station-distribution-line-infrastructure` : `create-mini-grid-station-distribution-line-infrastructure`}`}
        ...
        >
            {() => (
                <FormPageWrapper
                    ...
                    createActionFunc={isEdit ? editMiniGridStationDistributionLineInfrastructure : createMiniGridStationDistributionLineInfrastructure}
                    onActionSuccess={onActionSuccess}
                    onCancel={handleClose}
                >
                    {(formik: FormikProps<MiniGridStationDistributionLineInfrastructure>) => {
                        return <MiniGridStationDistributionLineInfrastructureForm
                            file={uploadableFile}
