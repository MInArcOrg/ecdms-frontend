...
export const miniGridStationDistributionLineInfrastructureColumns = (
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.distribution_line_type_id
          ? distributionLineTypesMap.get(row.distribution_line_type_id) || row.distribution_line_type_id
          : t('common.not-available')}
      </Typography>
    )
  },
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.distribution_line_material_id
          ? distributionLineMaterialsMap.get(row.distribution_line_material_id) || row.distribution_line_material_id
          : t('common.not-available')}
      </Typography>
    )
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.voltage_level !== undefined ? `${row.voltage_level} ${t('common.kv')}` : t('common.not-available')}
      </Typography>
    )
  },
  {
