...
const ElectricSmartMetersRatingsDataDrawer = (
  props: ElectricSmartMetersRatingsDataDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createElectricSmartMetersRatingsData = async (
    body: IApiPayload<ElectricSmartMetersRatingsData>,
  ) =>
    projectOtherApiSecondService<ElectricSmartMetersRatingsData>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editElectricSmartMetersRatingsData = async (
    body: IApiPayload<ElectricSmartMetersRatingsData>,
  ) =>
  ...
  return (
    <CustomSideDrawer
      title={`project.other.electric-smart-meters-ratings-data.${
        isEdit
          ? `edit-electric-smart-meters-ratings-data`
          : `create-electric-smart-meters-ratings-data`
      }`}
      ...
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...electricSmartMetersRatingsData,
            active_reactive:
              electricSmartMetersRatingsData?.active_reactive || "Active",
            phase: electricSmartMetersRatingsData?.phase || "Single Phase",
          }}
          createActionFunc={
            isEdit
              ? editElectricSmartMetersRatingsData
              : createElectricSmartMetersRatingsData
          }
