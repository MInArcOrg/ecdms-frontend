...
export const solarPanelColumns = (
  ...
  },
  {
    ...
    field: 'model',
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row?.model || t('common.not-available')}</Typography>
  },
  {
    flex: 0.2,
    minWidth: 150,
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.each_solar_panel_capacity !== undefined ? `${row.each_solar_panel_capacity} ${t('common.watts')}` : t('common.not-available')}
      </Typography>
    )
  },
  {
