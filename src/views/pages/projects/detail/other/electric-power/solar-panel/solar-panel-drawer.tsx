...
const SolarPanelDrawer = (props: SolarPanelDrawerType) => {
  ...
  const createSolarPanel = async (body: IApiPayload<SolarPanel>) =>
    projectOtherApiSecondService<SolarPanel>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editSolarPanel = async (body: IApiPayload<SolarPanel>) =>
    projectOtherApiSecondService<SolarPanel>().update(
      otherSubMenu?.apiRoute || "",
      solarPanel?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<SolarPanel>,
    payload: IApiPayload<SolarPanel>,
  ) => {
    if (payload.files.length > 0) {
      await uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.solarPanel,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.solar-panel.${
        isEdit ? `edit-solar-panel` : `create-solar-panel`
      }`}
