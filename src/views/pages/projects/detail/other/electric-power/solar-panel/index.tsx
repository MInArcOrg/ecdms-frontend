...
const SolarPanelList: React.FC<SolarPanelListProps> = ({ otherSubMenu, projectId, typeId }) => {
  ...
  const solarPanelTypeMap = new Map(solarPanelTypes?.payload.map((item) => [item.id, item.title || '']) || []);

  const fetchSolarPanels = (params: GetRequestParam): Promise<IApiResponse<SolarPanel[]>> => {
    return projectOtherApiSecondService<SolarPanel>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleDelete = async (solarPanelId: string) => {
    await projectOtherApiSecondService<SolarPanel>().delete(otherSubMenu?.apiRoute || '', solarPanelId);
    refetch();
  };
  ...
  const mapSolarPanelToDetailItems = (solarPanel: SolarPanel): { title: string; value: string }[] => [
    {
    ...
    },
    {
      ...
      value: solarPanelTypeMap.get(solarPanel?.solar_panel_type_id) || solarPanel?.solar_panel_type_id || 'N/A'
    },
    {
      title: t('project.other.solar-panel.details.solar-panels-number'),
      value: solarPanel?.solar_panels_number !== undefined ? solarPanel.solar_panels_number.toString() : 'N/A'
    },
    {
      title: t('project.other.solar-panel.details.each-solar-panel-capacity'),
      value: solarPanel?.each_solar_panel_capacity !== undefined ? `${solarPanel.each_solar_panel_capacity} ${t('common.watts')}` : 'N/A'
    },
    {
    ...
    },
    {
      ...
      value: solarPanel?.inverters_number !== undefined ? solarPanel.inverters_number.toString() : 'N/A'
    },
    {
    ...
    },
    {
      ...
      value: solarPanel?.created_at ? formatCreatedAt(solarPanel.created_at) : 'N/A'
    },
    {
      title: t('common.table-columns.updated-at'),
      value: solarPanel?.updated_at ? formatCreatedAt(solarPanel.updated_at) : 'N/A'
    }
  ];
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: solarPanelColumns(handleClickDetail, handleEdit, handleDelete, t, refetch, solarPanelTypeMap)
        }}
        isLoading={isLoading}
