...
const SolarPanelCard: React.FC<SolarPanelCardProps> = ({ solarPanel, refetch, onEdit, onDelete, onDetail, solarPanelTypeMap }) => {
      ...
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Typography variant="h5" fontWeight="bold">
            ...
            >
              {solarPanel?.manufacturer ? `${solarPanel.manufacturer} - ${solarPanel.model || ''}` : solarPanel?.id.slice(0, 8) + '...'}
            </Typography>
          </Typography>
        </Box>
        ...
        <Grid container spacing={2} mt={1}>
          <Grid item xs={12}>
            <Typography variant="body2" color="text.secondary">
              {t('project.other.solar-panel.details.solar-panel-type')}: {solarPanelTypeMap.get(solarPanel?.solar_panel_type_id) || 'N/A'}
            </Typography>
          </Grid>
          ...
          {solarPanel?.each_solar_panel_capacity !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.solar-panel.details.each-solar-panel-capacity')}: {solarPanel.each_solar_panel_capacity}{' '}
                {t('common.watts')}
              </Typography>
            </Grid>
          )}
