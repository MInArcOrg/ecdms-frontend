  ...
  return (
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Grid container spacing={2} mt={1}>
          ...
          {solarPanel?.solar_panels_number !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t("project.other.solar-panel.details.solar-panels-number")}:{" "}
                {solarPanel.solar_panels_number}
              </Typography>
            </Grid>
          )}
          ...
          {solarPanel?.inverter_manufacturer && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t("project.other.solar-panel.details.inverter-manufacturer")}:{" "}
                {solarPanel.inverter_manufacturer}
              </Typography>
            </Grid>
          )}
          ...
          {solarPanel?.inverters_number !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t("project.other.solar-panel.details.inverters-number")}:{" "}
                {solarPanel.inverters_number}
              </Typography>
            </Grid>
          )}
        </Grid>
        ...
        {solarPanel?.other_equipment && (
          <Box mt={2}>
            <Typography variant="body2" color="text.secondary">
              {t("project.other.solar-panel.details.other-equipment")}:{" "}
              {solarPanel.other_equipment}
            </Typography>
          </Box>
        )}
      ...
      <CardActions sx={{ justifyContent: "space-between" }}>
        <FileDrawer
          id={solarPanel.id}
          type={uploadableProjectFileTypes.other.solarPanel}
        />
