...
const SubstationLayoutAndCommunicationDataDrawer = (props: SubstationLayoutAndCommunicationDataDrawerType) => {
  ...
  const onActionSuccess = async (response: IApiResponse<SubstationLayoutAndCommunicationData>, payload: IApiPayload<SubstationLayoutAndCommunicationData>) => {
    if (payload.files.length > 0) {
      await uploadFile(payload.files[0], uploadableProjectFileTypes.other.substation_layout_and_communication_data, response.payload.id, "", "")
    }
      ...
      {() => (
        ...
        >
          {(formik: FormikProps<SubstationLayoutAndCommunicationData>) => {
            return <SubstationLayoutAndCommunicationDataForm 
            file={uploadableFile} 
