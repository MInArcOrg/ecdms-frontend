...
const SubstationLayoutAndCommunicationDataDrawer = (
  props: SubstationLayoutAndCommunicationDataDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const validationSchema = yup.object().shape({
    ...
    substation_communication_system_id: yup
      .string()
      .required("Communication System is required"),
    scada_system: yup.boolean().nullable(),
    substation_grounding_system_id: yup
      .string()
      .required("Grounding System is required"),
  ...
  const createSubstationLayoutAndCommunicationData = async (
    body: IApiPayload<SubstationLayoutAndCommunicationData>,
  ) =>
    projectOtherApiSecondService<SubstationLayoutAndCommunicationData>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editSubstationLayoutAndCommunicationData = async (
    body: IApiPayload<SubstationLayoutAndCommunicationData>,
  ) =>
  ...
  const getPayload = (values: SubstationLayoutAndCommunicationData) => ({
    data: {
      ...
      equipped_with_standby_diesel_generator:
        values.equipped_with_standby_diesel_generator,
      substation_busbar_type: values.substation_busbar_type,
      substation_communication_system_id:
        values.substation_communication_system_id,
  ...
  return (
    <CustomSideDrawer
      title={`project.other.substation-layout-and-communication-data.${
        isEdit
          ? `edit-substation-layout-and-communication-data`
          : `create-substation-layout-and-communication-data`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editSubstationLayoutAndCommunicationData
              : createSubstationLayoutAndCommunicationData
          }
