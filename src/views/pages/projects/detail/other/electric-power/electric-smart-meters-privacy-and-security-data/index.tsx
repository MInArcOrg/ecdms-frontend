...
const ElectricSmartMetersPrivacyAndSecurityDataList: React.FC<
  ElectricSmartMetersPrivacyAndSecurityDataListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<ElectricSmartMetersPrivacyAndSecurityData | null>(null);
  ...
  const { data: customerEngagementFrequencies } = useQuery({
    ...
    queryFn: () =>
      projectGeneralMasterDataApiService.getAll({
        filter: {
          model: projectMasterModels.customerEngagementFrequency.model,
        },
      }),
  });
  ...
  const { data: customerEngagementProgramTypes } = useQuery({
    ...
    queryFn: () =>
      projectGeneralMasterDataApiService.getAll({
        filter: {
          model: projectMasterModels.customerEngagementProgramsType.model,
        },
      }),
  });

  const fetchElectricSmartMetersPrivacyAndSecurityData = (
    params: GetRequestParam,
  ): Promise<IApiResponse<ElectricSmartMetersPrivacyAndSecurityData[]>> => {
    return projectOtherApiSecondService<ElectricSmartMetersPrivacyAndSecurityData>().getAll(
  ...
  const handleDelete = async (
    electricSmartMetersPrivacyAndSecurityDataId: string,
  ) => {
    await projectOtherApiSecondService<ElectricSmartMetersPrivacyAndSecurityData>().delete(
      otherSubMenu?.apiRoute || "",
      electricSmartMetersPrivacyAndSecurityDataId,
    );
    refetch();
  };
  ...
  const privacyMeasuresTypesMap = new Map(
    privacyMeasuresTypes?.payload.map((item: any) => [
      item.id,
      item.title || "",
    ]) || [],
  );
  const customerEngagementFrequenciesMap = new Map(
    customerEngagementFrequencies?.payload.map((item: any) => [
      item.id,
      item.title || "",
    ]) || [],
  );
  const customerEngagementProgramTypesMap = new Map(
    customerEngagementProgramTypes?.payload.map((item: any) => [
      item.id,
      item.title || "",
    ]) || [],
  );
    ...
    <Box>
      {showDrawer && (
        <ElectricSmartMetersPrivacyAndSecurityDataDrawer
          ...
          toggle={toggleDrawer}
          electricSmartMetersPrivacyAndSecurityData={
            selectedRow as ElectricSmartMetersPrivacyAndSecurityData
          }
          refetch={refetch}
          projectId={projectId}
          ...
          customerEngagementFrequencies={
            customerEngagementFrequencies?.payload || []
          }
          customerEngagementProgramTypes={
            customerEngagementProgramTypes?.payload || []
          }
        />
      )}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapElectricSmartMetersPrivacyAndSecurityDataToDetailItems(
            selectedRow as ElectricSmartMetersPrivacyAndSecurityData,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          ...
          title={t(
            "project.other.electric-smart-meters-privacy-and-security-data.electric-smart-meters-privacy-and-security-data-details",
          )}
        />
      )}
      ...
      <ItemsListing
        title={t(
          "project.other.electric-smart-meters-privacy-and-security-data.title",
        )}
