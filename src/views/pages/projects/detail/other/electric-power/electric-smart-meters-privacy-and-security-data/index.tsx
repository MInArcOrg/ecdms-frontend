...
const ElectricSmartMetersPrivacyAndSecurityDataList: React.FC<ElectricSmartMetersPrivacyAndSecurityDataListProps> = ({
  otherSubMenu,
  projectId,
  typeId
}) => {
  ...
  const customerEngagementFrequenciesMap = new Map(
    customerEngagementFrequencies?.payload.map((item: any) => [item.id, item.title || '']) || []
  );
  const customerEngagementProgramTypesMap = new Map(
    customerEngagementProgramTypes?.payload.map((item: any) => [item.id, item.title || '']) || []
  );
