...
const ElectricTowerDrawer = (props: ElectricTowerDrawerType) => {
  ...
  const createElectricTower = async (body: IApiPayload<ElectricTower>) =>
    projectOtherApiService<ElectricTower>().create(model, body);

  const editElectricTower = async (body: IApiPayload<ElectricTower>) =>
    projectOtherApiService<ElectricTower>().update(
      model,
      electricTower?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<ElectricTower>,
    payload: IApiPayload<ElectricTower>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.electricTower,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.electric-tower.${
        isEdit ? `edit-electric-tower` : `create-electric-tower`
      }`}
