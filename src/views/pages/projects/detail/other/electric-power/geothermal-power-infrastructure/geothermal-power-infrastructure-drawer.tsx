...
const GeothermalPowerInfrastructureDrawer = (
  props: GeothermalPowerInfrastructureDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createGeothermalPowerInfrastructure = async (
    ...
    projectOtherApiSecondService<GeothermalPowerInfrastructure>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editGeothermalPowerInfrastructure = async (
    body: IApiPayload<GeothermalPowerInfrastructure>,
  ) =>
  ...
  return (
    <CustomSideDrawer
      title={`project.other.geothermal-power-infrastructure.${
        isEdit
          ? `edit-geothermal-power-infrastructure`
          : `create-geothermal-power-infrastructure`
      }`}
      ...
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ...
              : createGeothermalPowerInfrastructure
          }
        ...
        >
          {(formik: FormikProps<GeothermalPowerInfrastructure>) => {
            return (
              <GeothermalPowerInfrastructureForm
                file={uploadableFile}
