...
export const generatingCapacityColumns = (
  ...
  },
  {
    ...
    field: "operator",
    renderCell: ({ row }: CellType) =>
      row.operator ? row.operator : t("common.not-available"),
  },
  {
    flex: 0.15,
    minWidth: 120,
    ...
    renderCell: ({ row }: CellType) =>
      row.commission_date
        ? formatCreatedAt(row.commission_date)
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) =>
      row.turbine_type_number !== undefined
        ? row.turbine_type_number
        : t("common.not-available"),
  },
  {
    flex: 0.15,
