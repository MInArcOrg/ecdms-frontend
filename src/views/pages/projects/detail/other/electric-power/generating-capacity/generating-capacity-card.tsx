    ...
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.generating-capacity.details.operator")}:{" "}
            {generatingCapacity.operator || t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.generating-capacity.details.turbine-type-number")}
            :{" "}
            {generatingCapacity.turbine_type_number !== undefined
              ? generatingCapacity.turbine_type_number
          ...
          <Typography variant="body2" color="text.secondary">
            ...
            {generatingCapacity.commission_date
              ? formatCreatedAt(generatingCapacity.commission_date)
              : t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.created-at")}:{" "}
            {generatingCapacity.created_at
              ? formatCreatedAt(generatingCapacity.created_at)
              : t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.updated-at")}:{" "}
            {generatingCapacity.updated_at
      ...
      <CardActions sx={{ justifyContent: "flex-end" }}>
        <FileDrawer
          ...
          type={uploadableProjectFileTypes.other.generatingCapacity}
        />
        <ModelAction
