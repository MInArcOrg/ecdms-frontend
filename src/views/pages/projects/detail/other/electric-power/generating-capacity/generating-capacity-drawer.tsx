...
const GeneratingCapacityDrawer = (props: GeneratingCapacityDrawerType) => {
  ...
  const createGeneratingCapacity = async (
    body: IApiPayload<GeneratingCapacity>,
  ) => projectOtherApiService<GeneratingCapacity>().create(model, body);

  const editGeneratingCapacity = async (
    body: IApiPayload<GeneratingCapacity>,
  ) =>
    projectOtherApiService<GeneratingCapacity>().update(
      model,
      generatingCapacity?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<GeneratingCapacity>,
    payload: IApiPayload<GeneratingCapacity>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.generatingCapacity,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.generating-capacity.${
        isEdit ? `edit-generating-capacity` : `create-generating-capacity`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          initialValues={{
            ...generatingCapacity,
            commission_date: formatInitialDateDate(
              generatingCapacity?.commission_date,
            ),
          }}
          createActionFunc={
            isEdit ? editGeneratingCapacity : createGeneratingCapacity
          }
