...
const WindEnergyDrawer = (props: WindEnergyDrawerType) => {
  ...
  const createWindEnergy = async (body: IApiPayload<WindEnergy>) =>
    projectOtherApiService<WindEnergy>().create(model, body);

  const editWindEnergy = async (body: IApiPayload<WindEnergy>) =>
    projectOtherApiService<WindEnergy>().update(
      model,
      windEnergy?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<WindEnergy>,
    payload: IApiPayload<WindEnergy>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.windEnergy,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.wind-energy.${
        isEdit ? `edit-wind-energy` : `create-wind-energy`
      }`}
