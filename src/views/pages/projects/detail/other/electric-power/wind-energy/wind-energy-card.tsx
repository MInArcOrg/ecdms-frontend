  ...
  return (
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.wind-energy.details.description")}:{" "}
            {windEnergy.description || t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.wind-energy.details.specifications")}:{" "}
            {windEnergy.specifications || t("common.not-available")}
          </Typography>

          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.created-at")}:{" "}
            {windEnergy.created_at
              ? formatCreatedAt(windEnergy.created_at)
              : t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.updated-at")}:{" "}
            {windEnergy.updated_at
              ? formatCreatedAt(windEnergy.updated_at)
              : t("common.not-available")}
          </Typography>
        </Box>
      </CardContent>
      ...
      <CardActions sx={{ justifyContent: "flex-end" }}>
        <FileDrawer
          id={windEnergy.id}
          type={uploadableProjectFileTypes.other.windEnergy}
        />
        <ModelAction
          model="WindEnergy"
