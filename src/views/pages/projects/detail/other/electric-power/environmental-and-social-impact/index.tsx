...
const EnvironmentalAndSocialImpactList: React.FC<EnvironmentalAndSocialImpactListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<EnvironmentalAndSocialImpact | null>(null);
  const { t } = useTranslation();

  const fetchEnvironmentalAndSocialImpacts = (params: GetRequestParam): Promise<IApiResponse<EnvironmentalAndSocialImpact[]>> => {
    return projectOtherApiSecondService<EnvironmentalAndSocialImpact>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleEdit = (environmentalAndSocialImpact: EnvironmentalAndSocialImpact) => {
    toggleDrawer();
    setSelectedRow(environmentalAndSocialImpact);
  };

  const handleDelete = async (environmentalAndSocialImpactId: string) => {
    await projectOtherApiSecondService<EnvironmentalAndSocialImpact>().delete(otherSubMenu?.apiRoute || '', environmentalAndSocialImpactId);
    refetch();
  };

  const handleClickDetail = (environmentalAndSocialImpact: EnvironmentalAndSocialImpact) => {
    toggleDetailDrawer();
    setSelectedRow(environmentalAndSocialImpact);
  };
    ...
    <Box>
      {showDrawer && (
        <EnvironmentalAndSocialImpactDrawer
          ...
          open={showDrawer}
          toggle={toggleDrawer}
          environmentalAndSocialImpact={selectedRow as EnvironmentalAndSocialImpact}
          refetch={refetch}
          projectId={projectId}
        />
      )}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapEnvironmentalAndSocialImpactToDetailItems(selectedRow as EnvironmentalAndSocialImpact)}
          hasReference={true}
          id={selectedRow?.id || ''}
          fileType={uploadableProjectFileTypes.other.environmentalAndSocialImpact}
          title={t('project.other.environmental-and-social-impact.environmental-and-social-impact-details')}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: environmentalAndSocialImpactColumns(handleClickDetail, handleEdit, handleDelete, t, refetch)
        }}
        isLoading={isLoading}
