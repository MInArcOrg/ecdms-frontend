    ...
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.transformer-type.details.description")}:{" "}
            {transformerType.description || t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.created-at")}:{" "}
            {transformerType.created_at
              ? formatCreatedAt(transformerType.created_at)
              : t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.updated-at")}:{" "}
            {transformerType.updated_at
              ? formatCreatedAt(transformerType.updated_at)
              : t("common.not-available")}
          </Typography>
        </Box>
      </CardContent>
      ...
      <CardActions sx={{ justifyContent: "flex-end" }}>
        <FileDrawer
          ...
          type={uploadableProjectFileTypes.other.transformerType}
        />
        <ModelAction
