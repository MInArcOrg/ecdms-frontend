...
const TransformerTypeDrawer = (props: TransformerTypeDrawerType) => {
  ...
  const createTransformerType = async (body: IApiPayload<TransformerType>) =>
    projectOtherApiService<TransformerType>().create(model, body);

  const editTransformerType = async (body: IApiPayload<TransformerType>) =>
    projectOtherApiService<TransformerType>().update(
      model,
      transformerType?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<TransformerType>,
    payload: IApiPayload<TransformerType>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.transformerType,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.transformer-type.${
        isEdit ? `edit-transformer-type` : `create-transformer-type`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editTransformerType : createTransformerType
          }
