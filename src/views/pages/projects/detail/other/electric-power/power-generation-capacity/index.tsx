...
const PowerGenerationCapacityList: React.FC<
  PowerGenerationCapacityListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<PowerGenerationCapacity | null>(null);
  const { t } = useTranslation();

  const fetchPowerGenerationCapacitys = (
    params: GetRequestParam,
  ): Promise<IApiResponse<PowerGenerationCapacity[]>> => {
    return projectOtherApiSecondService<PowerGenerationCapacity>().getAll(
  ...
  const handleDelete = async (powerGenerationCapacityId: string) => {
    await projectOtherApiSecondService<PowerGenerationCapacity>().delete(
      otherSubMenu?.apiRoute || "",
      powerGenerationCapacityId,
    );
    refetch();
  };
  ...
  const mapPowerGenerationCapacityToDetailItems = (
    ...
    },
    {
      ...
      value: powerGenerationCapacity?.commissioning_date
    ...
    },
    {
      ...
      value: powerGenerationCapacity?.created_at
        ? formatCreatedAt(powerGenerationCapacity.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: powerGenerationCapacity?.updated_at
        ? formatCreatedAt(powerGenerationCapacity.updated_at)
        : "N/A",
    },
  ];
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapPowerGenerationCapacityToDetailItems(
            selectedRow as PowerGenerationCapacity,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={uploadableProjectFileTypes.other.powerGenerationCapacity}
          title={t(
            "project.other.power-generation-capacity.power-generation-capacity-details",
          )}
        />
      )}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: powerGenerationCapacityColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
