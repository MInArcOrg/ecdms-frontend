  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.capacity !== undefined ? `${row.capacity} MW` : t('common.not-available')}
      </Typography>
    )
  },
  {
    flex: 0.2,
    minWidth: 150,
    headerName: t('project.other.power-generation-capacity.details.annual-generation'),
    field: 'annual_generation',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.annual_generation !== undefined ? `${row.annual_generation} GWh` : t('common.not-available')}
      </Typography>
    )
  },
  {
    flex: 0.15,
    minWidth: 120,
    headerName: t('project.other.power-generation-capacity.details.units-number'),
    field: 'units_number',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.units_number !== undefined ? row.units_number : t('common.not-available')}
      </Typography>
    )
  },
  {
    flex: 0.2,
    minWidth: 150,
    headerName: t('project.other.power-generation-capacity.details.commissioning-date'),
    field: 'commissioning_date',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>
        {row?.commissioning_date ? formatCreatedAt(row.commissioning_date) : t('common.not-available')}
      </Typography>
    )
  },
  {
