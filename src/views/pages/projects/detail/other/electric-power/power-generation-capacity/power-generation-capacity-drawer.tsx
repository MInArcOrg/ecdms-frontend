...
const PowerGenerationCapacityDrawer = (props: PowerGenerationCapacityDrawerType) => {
  const { open, toggle, refetch, powerGenerationCapacity, projectId, otherSubMenu } = props;
  ...
  const createPowerGenerationCapacity = async (body: IApiPayload<PowerGenerationCapacity>) =>
    projectOtherApiSecondService<PowerGenerationCapacity>().create(otherSubMenu?.apiRoute || '', body);

  const editPowerGenerationCapacity = async (body: IApiPayload<PowerGenerationCapacity>) =>
    projectOtherApiSecondService<PowerGenerationCapacity>().update(otherSubMenu?.apiRoute || '', powerGenerationCapacity?.id || '', body);
  ...
  const onActionSuccess = async (response: IApiResponse<PowerGenerationCapacity>, payload: IApiPayload<PowerGenerationCapacity>) => {
    if (payload.files.length > 0) {
      await uploadFile(payload.files[0], uploadableProjectFileTypes.other.powerGenerationCapacity, response.payload.id, '', '');
    }
      ...
      {() => (
        ...
        >
          {(formik: FormikProps<PowerGenerationCapacity>) => {
            return <PowerGenerationCapacityForm file={uploadableFile} onFileChange={onFileChange} formik={formik} />;
          }}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
