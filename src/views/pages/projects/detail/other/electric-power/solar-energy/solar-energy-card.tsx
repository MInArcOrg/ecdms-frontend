  ...
  return (
    <Card sx={{ p: 2 }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={1} mt={2}>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.solar-energy.details.description")}:{" "}
            {solarEnergy.description || t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("project.other.solar-energy.details.specifications")}:{" "}
            {solarEnergy.specifications || t("common.not-available")}
          </Typography>

          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.created-at")}:{" "}
            {solarEnergy.created_at
              ? formatCreatedAt(solarEnergy.created_at)
              : t("common.not-available")}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t("common.table-columns.updated-at")}:{" "}
            {solarEnergy.updated_at
              ? formatCreatedAt(solarEnergy.updated_at)
              : t("common.not-available")}
          </Typography>
        </Box>
      </CardContent>
      ...
      <CardActions sx={{ justifyContent: "flex-end" }}>
        <FileDrawer
          id={solarEnergy.id}
          type={uploadableProjectFileTypes.other.solarEnergy}
        />
        <ModelAction
          model="SolarEnergy"
