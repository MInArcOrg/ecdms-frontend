...
const SolarEnergyDrawer = (props: SolarEnergyDrawerType) => {
  ...
  const createSolarEnergy = async (body: IApiPayload<SolarEnergy>) =>
    projectOtherApiService<SolarEnergy>().create(model, body);

  const editSolarEnergy = async (body: IApiPayload<SolarEnergy>) =>
    projectOtherApiService<SolarEnergy>().update(
      model,
      solarEnergy?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<SolarEnergy>,
    payload: IApiPayload<SolarEnergy>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.solarEnergy,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.solar-energy.${
        isEdit ? `edit-solar-energy` : `create-solar-energy`
      }`}
