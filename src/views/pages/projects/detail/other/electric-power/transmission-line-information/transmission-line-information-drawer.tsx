...
const TransmissionLineInformationDrawer = (
  props: TransmissionLineInformationDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createTransmissionLineInformation = async (
    body: IApiPayload<TransmissionLineInformation>,
  ) =>
    projectOtherApiSecondService<TransmissionLineInformation>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editTransmissionLineInformation = async (
    body: IApiPayload<TransmissionLineInformation>,
  ) =>
  ...
  return (
    <CustomSideDrawer
      title={`project.other.transmission-line-information.${
        isEdit
          ? `edit-transmission-line-information`
          : `create-transmission-line-information`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit
              ? editTransmissionLineInformation
              : createTransmissionLineInformation
          }
        ...
        >
          {(formik: FormikProps<TransmissionLineInformation>) => {
            return (
              <TransmissionLineInformationForm
                file={uploadableFile}
