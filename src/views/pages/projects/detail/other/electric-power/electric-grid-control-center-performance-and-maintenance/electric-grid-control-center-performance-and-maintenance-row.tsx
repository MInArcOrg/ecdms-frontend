...
export const electricGridControlCenterPerformanceAndMaintenanceColumns = (
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.maintenance_frequency_id
          ? maintenanceFrequenciesMap.get(row.maintenance_frequency_id) ||
            row.maintenance_frequency_id
          : t("common.not-available")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.total_system_downtime_outage_duration !== undefined
          ? row.total_system_downtime_outage_duration
          : t("common.not-available")}
      </Typography>
    ),
  },
