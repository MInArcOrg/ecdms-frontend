...
const ElectricGridControlCenterPerformanceAndMaintenanceList: React.FC<
  ElectricGridControlCenterPerformanceAndMaintenanceListProps
> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] =
    useState<ElectricGridControlCenterPerformanceAndMaintenance | null>(null);
  ...
  const fetchElectricGridControlCenterPerformanceAndMaintenance = (
    params: GetRequestParam,
  ): Promise<
    IApiResponse<ElectricGridControlCenterPerformanceAndMaintenance[]>
  > => {
    return projectOtherApiSecondService<ElectricGridControlCenterPerformanceAndMaintenance>().getAll(
  ...
  const handleDelete = async (
    electricGridControlCenterPerformanceAndMaintenanceId: string,
  ) => {
    await projectOtherApiSecondService<ElectricGridControlCenterPerformanceAndMaintenance>().delete(
      otherSubMenu?.apiRoute || "",
      electricGridControlCenterPerformanceAndMaintenanceId,
    );
    refetch();
  };
  ...
  const mapElectricGridControlCenterPerformanceAndMaintenanceToDetailItems = (
    electricGridControlCenterPerformanceAndMaintenance: ElectricGridControlCenterPerformanceAndMaintenance,
    ...
    },
    {
      ...
      value:
        electricGridControlCenterPerformanceAndMaintenance?.remark || "N/A",
    },
    {
      title: t("common.table-columns.created-at"),
      value: electricGridControlCenterPerformanceAndMaintenance?.created_at
        ? formatCreatedAt(
    ...
    },
    {
      ...
      value: electricGridControlCenterPerformanceAndMaintenance?.updated_at
        ? formatCreatedAt(
            electricGridControlCenterPerformanceAndMaintenance.updated_at,
          )
        : "N/A",
    },
  ];
    ...
    <Box>
      {showDrawer && (
        <ElectricGridControlCenterPerformanceAndMaintenanceDrawer
          ...
          toggle={toggleDrawer}
          electricGridControlCenterPerformanceAndMaintenance={
            selectedRow as ElectricGridControlCenterPerformanceAndMaintenance
          }
          refetch={refetch}
          projectId={projectId}
          electricGridControlCenterData={
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapElectricGridControlCenterPerformanceAndMaintenanceToDetailItems(
            selectedRow as ElectricGridControlCenterPerformanceAndMaintenance,
          )}
          hasReference={true}
          id={selectedRow?.id || ""}
          ...
          title={t(
            "project.other.electric-grid-control-center-performance-and-maintenance.electric-grid-control-center-performance-and-maintenance-details",
          )}
        />
      )}
      ...
      <ItemsListing
        title={t(
          "project.other.electric-grid-control-center-performance-and-maintenance.title",
        )}
