...
const ElectricDistributionTransformerList: React.FC<ElectricDistributionTransformerListProps> = ({ otherSubMenu, projectId, typeId }) => {
  const [showDrawer, setShowDrawer] = useState(false)
  const [showDetailDrawer, setShowDetailDrawer] = useState(false)
  const [selectedRow, setSelectedRow] = useState<ElectricDistributionTransformer | null>(null)
  ...
  const { data: fireExtinguishingTechnologies } = useQuery({
    ...
    queryFn: () =>
      projectGeneralMasterDataApiService.getAll({
        filter: { model: projectMasterModels.fireExtinguishingTechnology.model },
      }),
  })

  const fetchElectricDistributionTransformers = (params: GetRequestParam): Promise<IApiResponse<ElectricDistributionTransformer[]>> => {
    return projectOtherApiSecondService<ElectricDistributionTransformer>().getAll(otherSubMenu?.apiRoute || "", {})
  }
  ...
  const handleEdit = (electricDistributionTransformer: ElectricDistributionTransformer) => {
    toggleDrawer()
    setSelectedRow(electricDistributionTransformer)
  }

  const handleDelete = async (electricDistributionTransformerId: string) => {
    await projectOtherApiSecondService<ElectricDistributionTransformer>().delete(otherSubMenu?.apiRoute || "", electricDistributionTransformerId)
    refetch()
  }

  const handleClickDetail = (electricDistributionTransformer: ElectricDistributionTransformer) => {
    toggleDetailDrawer()
    setSelectedRow(electricDistributionTransformer)
  }

  // Create maps for dropdown values
  const fireExtinguishingTechnologiesMap = new Map(fireExtinguishingTechnologies?.payload.map((item: any) => [item.id, item.title || '']) || []);

  const mapElectricDistributionTransformerToDetailItems = (electricDistributionTransformer: ElectricDistributionTransformer): { title: string; value: string }[] => [
    {
    ...
    },
    {
      ...
      value: electricDistributionTransformer?.service_area !== undefined 
    ...
    <Box>
      {showDrawer && (
        <ElectricDistributionTransformerDrawer
          ...
          open={showDrawer}
          toggle={toggleDrawer}
          electricDistributionTransformer={selectedRow as ElectricDistributionTransformer}
          refetch={refetch}
          projectId={projectId}
          fireExtinguishingTechnologies={fireExtinguishingTechnologies?.payload || []}
        />
      )}
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapElectricDistributionTransformerToDetailItems(selectedRow as ElectricDistributionTransformer)}
          hasReference={true}
          id={selectedRow?.id || ""}
          fileType={uploadableProjectFileTypes.other.electric_distribution_transformer}
          title={t("project.other.electric-distribution-transformer.electric-distribution-transformer-details")}
        />
      )}
