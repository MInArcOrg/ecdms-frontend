...
export const electricDistributionTransformerColumns = (
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.fire_extinguishing_technology_id ? fireExtinguishingTechnologiesMap.get(row.fire_extinguishing_technology_id) || row.fire_extinguishing_technology_id : t("common.not-available")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.service_area !== undefined
          ? `${row.service_area} ${t("common.km2")}`
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.installation_year || t("common.not-available")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.transformers_total_number || t("common.not-available")}
      </Typography>
    ),
  },
  {
