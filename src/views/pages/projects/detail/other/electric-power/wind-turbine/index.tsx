...
const WindTurbineList: React.FC<WindTurbineListProps> = ({ otherSubMenu, projectId, typeId }) => {
  ...
  const towerTypeMap = new Map(towerTypes?.payload.map((item) => [item.id, item.title || '']) || []);
  const generatorTypeMap = new Map(generatorTypes?.payload.map((item) => [item.id, item.title || '']) || []);

  const fetchWindTurbines = (params: GetRequestParam): Promise<IApiResponse<WindTurbine[]>> => {
    return projectOtherApiSecondService<WindTurbine>().getAll(otherSubMenu?.apiRoute || '', {
      ...params,
  ...
  const handleDelete = async (windTurbineId: string) => {
    await projectOtherApiSecondService<WindTurbine>().delete(otherSubMenu?.apiRoute || '', windTurbineId);
    refetch();
  };
  ...
  const mapWindTurbineToDetailItems = (windTurbine: WindTurbine): { title: string; value: string }[] => [
    {
    ...
    },
    {
      ...
      value: windTurbine?.rotor_diameter !== undefined ? `${windTurbine.rotor_diameter} ${t('common.meters')}` : 'N/A'
    },
    {
      title: t('project.other.wind-turbine.details.hub-height'),
      value: windTurbine?.hub_height !== undefined ? `${windTurbine.hub_height} ${t('common.meters')}` : 'N/A'
    },
    {
      title: t('project.other.wind-turbine.details.tower-type'),
      value: windTurbine?.tower_type_id ? towerTypeMap.get(windTurbine.tower_type_id) || windTurbine.tower_type_id : 'N/A'
    },
    {
      title: t('project.other.wind-turbine.details.blade-length'),
      value: windTurbine?.blade_length !== undefined ? `${windTurbine.blade_length} ${t('common.meters')}` : 'N/A'
    },
    {
      title: t('project.other.wind-turbine.details.blades-number'),
      value: windTurbine?.blades_number !== undefined ? windTurbine.blades_number.toString() : 'N/A'
    },
    {
    ...
    },
    {
      ...
      value: windTurbine?.generator_type_id ? generatorTypeMap.get(windTurbine.generator_type_id) || windTurbine.generator_type_id : 'N/A'
    },
    {
      title: t('project.other.wind-turbine.details.generators-number'),
      value: windTurbine?.generators_number !== undefined ? windTurbine.generators_number.toString() : 'N/A'
    },
    {
    ...
    },
    {
      ...
      value: windTurbine?.created_at ? formatCreatedAt(windTurbine.created_at) : 'N/A'
    },
    {
      title: t('common.table-columns.updated-at'),
      value: windTurbine?.updated_at ? formatCreatedAt(windTurbine.updated_at) : 'N/A'
    }
  ];
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: windTurbineColumns(handleClickDetail, handleEdit, handleDelete, t, refetch, towerTypeMap, generatorTypeMap)
        }}
        isLoading={isLoading}
