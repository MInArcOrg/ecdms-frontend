...
const MiniGridStationCard: React.FC<MiniGridStationCardProps> = ({ miniGridStation, refetch, onEdit, onDelete, onDetail }) => {
      ...
      <CardContent>
        ...
        <Grid container spacing={2} mt={1}>
          {miniGridStation?.minigrid_size !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.mini-grid-station.details.minigrid-size')}: {miniGridStation.minigrid_size} {t('common.kw')}
              </Typography>
            </Grid>
          )}
          ...
          {miniGridStation?.battery_size !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.mini-grid-station.details.battery-size')}: {miniGridStation.battery_size} {t('common.kwh')}
              </Typography>
            </Grid>
          )}
          ...
          {miniGridStation?.inverter !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.mini-grid-station.details.inverter')}: {miniGridStation.inverter} {t('common.kw')}
              </Typography>
            </Grid>
          )}
          ...
          {miniGridStation?.system_voltage !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.mini-grid-station.details.system-voltage')}: {miniGridStation.system_voltage} {t('common.v')}
              </Typography>
            </Grid>
          )}
        </Grid>
        ...
        <Grid container spacing={2} mt={1}>
          {miniGridStation?.expected_annual_generation !== undefined && (
            <Grid item xs={6}>
              <Typography variant="body2" color="text.secondary">
                {t('project.other.mini-grid-station.details.expected-annual-generation')}: {miniGridStation.expected_annual_generation}{' '}
                {t('common.kwh')}
              </Typography>
            </Grid>
          )}
          ...
          <Grid item xs={6}>
            <Typography variant="body2" color="text.secondary">
              {t('project.other.mini-grid-station.details.diesel-generator')}: {miniGridStation.diesel_generator}
            </Typography>
          </Grid>
        </Grid>
        ...
        {miniGridStation?.owner_operator && (
          <Box mt={2}>
            <Typography variant="body2" color="text.secondary">
              {t('project.other.mini-grid-station.details.owner-operator')}: {miniGridStation.owner_operator}
            </Typography>
          </Box>
        )}
        ...
        {miniGridStation?.created_at && (
          <Typography variant="body2" color="text.secondary" mt={2}>
            {t('common.table-columns.created-at')}: {formatCreatedAt(miniGridStation.created_at)}
          </Typography>
        )}
      </CardContent>

      <CardActions sx={{ justifyContent: 'space-between' }}>
        <FileDrawer id={miniGridStation.id} type={uploadableProjectFileTypes.other.mini_grid_station} />
