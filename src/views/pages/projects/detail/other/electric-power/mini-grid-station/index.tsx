...
const MiniGridStationList: React.FC<MiniGridStationListProps> = ({
  otherSubMenu,
  projectId,
  typeId,
}) => {
  ...
  const fetchMiniGridStations = (
    params: GetRequestParam,
  ): Promise<IApiResponse<MiniGridStation[]>> => {
    return projectOtherApiSecondService<MiniGridStation>().getAll(
  ...
  const handleDelete = async (miniGridStationId: string) => {
    await projectOtherApiSecondService<MiniGridStation>().delete(
      otherSubMenu?.apiRoute || "",
      miniGridStationId,
    );
    refetch();
  ...
  const batteryTypesMap = new Map(
    batteryTypes?.payload.map((item: any) => [item.id, item.title || ""]) || [],
  );

  const mapMiniGridStationToDetailItems = (
    ...
    },
    {
      ...
      value: miniGridStation?.substation_id
        ? substationsMap.get(miniGridStation?.substation_id) ||
          miniGridStation?.substation_id
        : "N/A",
    },
    {
    ...
    },
    {
      ...
      value: miniGridStation?.battery_type_id
        ? batteryTypesMap.get(miniGridStation?.battery_type_id) ||
          miniGridStation?.battery_type_id
        : "N/A",
    },
    {
    ...
    },
    {
      ...
      value: miniGridStation?.created_at
        ? formatCreatedAt(miniGridStation.created_at)
        : "N/A",
    },
    {
      title: t("common.table-columns.updated-at"),
      value: miniGridStation?.updated_at
        ? formatCreatedAt(miniGridStation.updated_at)
        : "N/A",
    },
  ];
  ...
  return (
    <Box>
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: miniGridStationColumns(
            handleClickDetail,
            handleEdit,
            handleDelete,
            t,
