...
const HydrologicalInformationDrawer = (
  props: HydrologicalInformationDrawerType,
) => {
  const {
    open,
    toggle,
  ...
  const createHydrologicalInformation = async (
    body: IApiPayload<HydrologicalInformation>,
  ) =>
    projectOtherApiSecondService<HydrologicalInformation>().create(
      otherSubMenu?.apiRoute || "",
      body,
    );

  const editHydrologicalInformation = async (
    body: IApiPayload<HydrologicalInformation>,
  ) =>
    projectOtherApiSecondService<HydrologicalInformation>().update(
      otherSubMenu?.apiRoute || "",
      hydrologicalInformation?.id || "",
      body,
    );
  ...
  const onActionSuccess = async (
    response: IApiResponse<HydrologicalInformation>,
    payload: IApiPayload<HydrologicalInformation>,
  ) => {
    if (payload.files.length > 0) {
      uploadFile(
        payload.files[0],
        uploadableProjectFileTypes.other.hydrologicalInformation,
        response.payload.id,
        "",
        "",
      );
  ...
  return (
    <CustomSideDrawer
      title={`project.other.hydrological-information.${
        isEdit
          ? `edit-hydrological-information`
          : `create-hydrological-information`
      }`}
    ...
    >
      {() => (
        <FormPageWrapper
          ...
          createActionFunc={
            isEdit ? editHydrologicalInformation : createHydrologicalInformation
          }
