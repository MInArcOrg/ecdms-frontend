...
export const buldingDimensionDetailColumns = (
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
        ...
        row?.site_area ? `${row.site_area} sqm` : t('common.not-available')
    ),
  },
  {
    flex: 0.15,
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.ground_floor_area
          ? `${row.ground_floor_area} sqm`
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: "text.secondary" }}>
        {row?.height_above_natural_ground
          ? `${row.height_above_natural_ground} m`
          : t("common.not-available")}
      </Typography>
    ),
  },
