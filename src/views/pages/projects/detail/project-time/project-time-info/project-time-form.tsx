...
import {
  convertToGC,
  getDynamicDate,
} from "src/views/components/custom/ethio-calendar/ethio-calendar-utils";
...
const ProjectTimeForm: React.FC<ProjectTimeFormProps> = ({
  ...
  useEffect(() => {
    const siteHandoverDate = moment(
      convertDate(formik.values.site_handover_date),
    );
    const mobilizationDays = formik.values.mobilization_days_no;
    const commencementDate = siteHandoverDate.add(
      mobilizationDays || 0,
      "days",
    );
    formik.setFieldValue(
      "commencement_date",
      getDynamicDate(i18n, commencementDate.toDate()),
      false,
    );
  ...
  useEffect(() => {
    const commencementDate = moment(
      convertDate(formik.values.commencement_date),
    );
    const contractDuration = formik.values.original_contract_duration;
    const projectCompletionDate = commencementDate.add(
      contractDuration || 0,
      "days",
    );
    formik.setFieldValue(
      "project_completion_date",
      getDynamicDate(i18n, projectCompletionDate.toDate()),
      false,
    ); // Validate after the last change
  }, [
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12}>
        <CustomTextBox
          ...
          placeholder={transl(
            "project.project-time.form.original-contract-duration",
          )}
