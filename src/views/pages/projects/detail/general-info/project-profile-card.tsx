...
const ProfileCard: React.FC<ProfileCardProps> = ({ projectInfo, time }) => {
  ...
  const changeProfilePicture = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.addEventListener("load", () => {
        uploadImage(
          e.target.files![0],
          uploadablePhotoTypes.project_profile_photo,
          projectInfo.id,
        )
          .then(() => {
  ...
  const completionDate = addDays(
    parseISO(time?.contract_signing_date ?? ""),
    time?.original_contract_duration ?? 0,
  );
      ...
      >
        ...
        <Box ml={6}>
          ...
          <Typography variant="subtitle1">
            {time == null
              ? t(
                  "project.general-information.please-fill-contract-signing-date",
                )
              : `${t("project.general-information.from")} ${getDynamicDate(
                  i18n,
                  time?.contract_signing_date,
                ).toDateString()} - ${getDynamicDate(
                  i18n,
                  completionDate,
                ).toDateString()}`}
          </Typography>
