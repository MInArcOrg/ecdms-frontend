...
const EducationDrawer = (props: EducationDrawerType) => {
  const { open, toggle, refetch, education, professionalId, studyFields } =
    props;
  ...
  const createEducation = async (
    body: IApiPayload<ProfessionalEducation>,
  ...
  const onActionSuccess = async (
    response: IApiResponse<ProfessionalEducation>,
    payload: IApiPayload<ProfessionalEducation>,
  ) => {
    try {
      ...
      if (payload.files?.length) {
        ...
        await uploadFile(
          payload.files[0],
          uploadableResourceFileTypes.professionalEducation,
          educationId,
          "",
          "",
        );
