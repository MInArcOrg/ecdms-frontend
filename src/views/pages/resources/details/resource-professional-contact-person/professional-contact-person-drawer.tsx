...
const ProfessionalContactPersonDrawer = (
  props: ProfessionalContactPersonDrawerType,
) => {
  ...
  const editContactPerson = async (
    ...
    return professionalContactPersonApiService.update(
      contactPerson?.id || "",
      body,
    );
  };
  ...
  const onActionSuccess = async (
    response: IApiResponse<ProfessionalContactPerson>,
  ) => {
    refetch();
    handleClose();
  };
  ...
  return (
    <CustomSideDrawer
      title={`resources.professional.contact-person.${
        isEdit ? "edit" : "create"
      }`}
      ...
      {() => (
        ...
        >
          {(formik: FormikProps<ProfessionalContactPerson>) => (
            <ProfessionalContactPersonForm formik={formik} />
          )}
        </FormPageWrapper>
      )}
    </CustomSideDrawer>
