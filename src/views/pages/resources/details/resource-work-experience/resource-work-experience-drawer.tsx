...
const ExperienceDrawer = (props: ExperienceDrawerType) => {
  ...
  const createExperience = async (
    body: IApiPayload<ProfessionalWorkExperience>,
  ...
  const editExperience = async (
    ...
    return professionalWorkExperienceApiService.update(
      experience?.id || "",
      body,
    );
  };
  ...
  const onActionSuccess = async (
    response: IApiResponse<ProfessionalWorkExperience>,
    payload: IApiPayload<ProfessionalWorkExperience>,
  ) => {
    try {
      if (!response.payload?.id)
        throw new Error("Missing experience ID in response");
      const experienceId = response.payload.id;

      if (payload.files?.length) {
        await uploadFile(
          payload.files[0],
          uploadableResourceFileTypes.professionalWorkExperience,
          experienceId,
          "",
          "",
        );
  ...
  return (
    <CustomSideDrawer
      title={`resources.professional.work-experience.${
        isEdit ? "edit" : "create"
      }`}
