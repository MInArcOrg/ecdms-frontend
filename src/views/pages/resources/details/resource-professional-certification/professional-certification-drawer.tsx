...
const CertificationDrawer = (props: CertificationDrawerType) => {
  ...
  const editCertification = async (
    ...
    return professionalCertificationApiService.update(
      certification?.id || "",
      body,
    );
  };
  ...
  const onActionSuccess = async (
    response: IApiResponse<ProfessionalCertification>,
    payload: IApiPayload<ProfessionalCertification>,
  ) => {
    try {
      if (!response.payload?.id)
        throw new Error("Missing certification ID in response");
      const certificationId = response.payload.id;

      if (payload.files?.length) {
        await uploadFile(
          payload.files[0],
          uploadableResourceFileTypes.professionalCertification,
          certificationId,
          "",
          "",
        );
  ...
  return (
    <CustomSideDrawer
      title={`resources.professional.certification.${
        isEdit ? "edit" : "create"
      }`}
        ...
        <FormPageWrapper
          ...
          initialValues={{
            ...
            issue_date: certification?.issue_date
              ...
              : "",
            expire_date: certification?.expire_date
              ? certification.expire_date.split("T")[0]
              : "",
          }}
