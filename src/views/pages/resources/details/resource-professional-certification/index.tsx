...
const CertificationList: React.FC<CertificationListProps> = ({ professionalId }) => {
  const [showDrawer, setShowDrawer] = useState(false);
  const [showDetailDrawer, setShowDetailDrawer] = useState(false);
  const [selectedRow, setSelectedRow] = useState<ProfessionalCertification | null>(null);
  const { t } = useTranslation();

  const fetchCertifications = (params: GetRequestParam): Promise<IApiResponse<ProfessionalCertification[]>> => {
    ...
    <Box>
      ...
      {showDetailDrawer && (
        <OtherDetailSidebar
          ...
          data={mapCertificationToDetailItems(selectedRow as ProfessionalCertification)}
          id={selectedRow?.id || ''}
          hasReference={true}
      ...
      <ItemsListing
        ...
        tableProps={{
          headers: certificationColumns(handleClickDetail, handleEdit, handleDelete, t)
        }}
        isLoading={false}
