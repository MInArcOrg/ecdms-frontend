...
function ResourceStudyFieldList({ resourceId }: { resourceId: string }) {
  ...
  const [selectedRow, setSelectedRow] = useState<ResourceStudyField | null>(
    null,
  );

  const { t } = useTranslation();

  const fetchResourceStudyFields = (
    params: GetRequestParam,
  ): Promise<IApiResponse<ResourceStudyField[]>> => {
      ...
      <Container>
        <ItemsListing
          ...
          ItemViewComponent={({ data }) => (
            <ResourceStudyFieldCard
              resourceStudyField={data}
