...
const MembershipDrawer = (props: MembershipDrawerType) => {
  ...
  const createMembership = async (
    body: IApiPayload<ProfessionalMembership>,
  ...
  const onActionSuccess = async (
    response: IApiResponse<ProfessionalMembership>,
    payload: IApiPayload<ProfessionalMembership>,
  ) => {
    try {
      if (!response.payload?.id)
        throw new Error("Missing membership ID in response");
      const membershipId = response.payload.id;

      if (payload.files?.length) {
        await uploadFile(
          payload.files[0],
          uploadableResourceFileTypes.professionalAssociationMembership,
          membershipId,
          "",
          "",
        );
  ...
  return (
    <CustomSideDrawer
      title={`resources.professional.association-membership.${
        isEdit ? "edit" : "create"
      }`}
        ...
        <FormPageWrapper
          ...
          initialValues={{
            ...
            registration_date: membership?.registration_date
              ? membership.registration_date.split("T")[0]
              : "",
            end_date: membership?.end_date
              ? membership.end_date.split("T")[0]
              : "",
          }}
