...
const AddressDrawer = (props: AddressDrawerType) => {
  ...
  const createAddress = async (body: IApiPayload<ProfessionalAddress>) => professionalAddressApiService.create(body);

  const editAddress = async (body: IApiPayload<ProfessionalAddress>) => professionalAddressApiService.update(address?.id || '', body);
  ...
  return (
    <CustomSideDrawer title={`professional.address.${isEdit ? 'edit' : 'create'}`} handleClose={handleClose} open={open}>
      {() => (
        <FormPageWrapper
          edit={isEdit}
