...
function ResourceStudyLevelList({ resourceId }: { resourceId: string }) {
  ...
  const [selectedRow, setSelectedRow] = useState<ResourceStudyLevel | null>(
    null,
  );

  const { t } = useTranslation();

  const fetchResourceStudyLevels = (
    params: GetRequestParam,
  ): Promise<IApiResponse<ResourceStudyLevel[]>> => {
      ...
      <Container>
        <ItemsListing
          ...
          ItemViewComponent={({ data }) => (
            <ResourceStudyLevelCard
              resourceStudyLevel={data}
