...
import {
  getResourceStudyFields,
  postResourceStudyField,
  updateResourceStudyField
} from 'src/services/resources/studyField'
...
function StudyField({ id, isProject }) {
  ...
  const [{ data, loading, error }, refetch] = getResourceStudyFields(id, page || 0, pageSize || 10)
  const [{ loading: postLoading, error: postError }, post] = postResourceStudyField()
  const [{ loading: putLoading, error: putError }, put] = updateResourceStudyField(selectedRow?.id)
  const [{ data: fields, error: fieldsError }] = getStudyFields()
        ...
        return (
          <Box display='flex' alignItems='center' gap={2}>
            <ModelAction
              ...
              model='construtionresourcestudyfield'
  ...
  const handleSubmit = async values => {
        ...
        if (values.referenceFile.length > 0) {
          return Promise.all(
            values.referenceFile.map(async file =>
              uploadFiles(file, uploadableResourceFileTypes.studyField, selectedRow.id)
            )
          ).then(() => {
            refetch()
    ...
    return post({ data: params }).then(res => {
      if (values.referenceFile.length > 0) {
        return Promise.all(
          values.referenceFile.map(async file => uploadFiles(file, uploadableResourceFileTypes.studyField, res.data.id))
        ).then(() => {
          refetch()
    ...
    <Box display='flex' flexDirection='column'>
      ...
      <Card>
        <CardContent>
          ...
          {!error && showDetail ? (
            <CardDetail data={selectedRow} title='studyfield' fileType={uploadableResourceFileTypes.studyField} />
          ) : desktop ? (
              ...
              {data?.data?.map((study, index) => (
                <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
                  <CardView
                    ...
                    title='studyfield'
                    fileType={uploadableResourceFileTypes.studyField}
                  />
                </Grid>
              ))}
            </Grid>
