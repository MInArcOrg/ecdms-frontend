...
const SmallTeamAddress = ({ smallTeam }: { smallTeam: SmallTeam }) => {
  const { data: address, refetch } = useQuery({
    ...
    queryFn: () => addressApiService.getByModel(smallTeam.id, defaultGetRequestParam).then((response) => response.payload)
  });
  return address && <AddressOverview refetch={refetch} address={address} modelId={smallTeam.id} />;
};
