...
const EthioCalendar: FC<EthioCalendarProps> = ({
  value,
  onChange,
  placeholderText,
  disabled,
  ...
  const [currentMonth, setCurrentMonth] = useState(
    new EthiopianDate(ethiopian.year, ethiopian.month - 1, ethiopian.day),
  );
  ...
  const handlePrevMonth = () => {
    setCurrentMonth(
      (prevState: { getFullYear: () => number; getMonth: () => number }) => {
        const prevMonth1 = new EthiopianDate(
  ...
  const handleNextMonth = () => {
    setCurrentMonth(
      (prevState: { getFullYear: () => any; getMonth: () => number }) => {
        const nextMonth = new EthiopianDate(
          prevState.getFullYear(),
          prevState.getMonth() + 1,
          1,
        );
  ...
  const handleDateSelect = (data: EthiopianDate) => {
    setIsOpen(false);
    onChange(
      new EthiopianDate(data.getFullYear(), data.getMonth(), data.getDate()),
    );
  };
  ...
  const renderCalendarDays = () => {
    ...
    const previousMonthYear =
      ethiopianMonth === 0 ? ethiopianYear - 1 : ethiopianYear;
    const previousMonthDays = getEthiopianMonthDays(
      previousMonth,
      previousMonthYear,
    );

    // Calculate the number of days in the current month
    const currentMonthDays = getEthiopianMonthDays(
      ethiopianMonth,
      ethiopianYear,
    );

    // Calculate the starting day of the current month
    const startingDay = new EthiopianDate(
      ethiopianYear,
      ethiopianMonth,
      1,
    ).getDay();
    ...
    for (
      let i = startingDay === 0 ? startingDay : startingDay - 1;
      i >= 0;
      i--
    ) {
    ...
    for (let i = 1; i <= currentMonthDays; i++) {
      const currentDate = new EthiopianDate(
        ethiopianYear,
        ethiopianMonth + 1,
        i,
      );
      ...
      const isCurrentDate =
        ethiopian.year === currentDate.year &&
        ethiopian.month === currentDate.month &&
        ethiopian.day === i;
            ...
            >
              <IconButton
                sx={{
                  height: "50%",
                  backgroundColor:
                    theme.palette.mode === "dark" ? "#3D4156" : "#EFEFF0",
                  p: 0.5,
                  mt: 1,
                }}
                size="small"
            ...
            >
              <CalendarDropdownBtn
                ...
                handleChange={(month: string) => {
                  const newDate = new EthiopianDate(
                    currentMonth.getFullYear(),
                    ethiopianMonthNames.indexOf(month),
