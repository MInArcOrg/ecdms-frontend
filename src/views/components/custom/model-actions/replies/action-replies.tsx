...
const ActionReplyComponent = ({
  ...
  const { data, refetch } = useQuery({
    ...
    queryFn: () =>
      modelActionApiService.getActionReplies(String(actionData?.id), {}),
  });
  ...
  const validation = useFormik({
    ...
    onSubmit: (values) => {
      try {
        modelActionApiService.createActionReply({
          data: values as ActionReply,
          files: [],
        });
        refetch();
