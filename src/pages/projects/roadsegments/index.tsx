...
const validationSchema = Yup.object().shape({
  name: Yup.string().required('Name is required').min(3, 'Name must be at least 3 characters').max(50, 'Name cannot exceed 50 characters'),
  specifications: Yup.string().required('Specifications are required').max(100, 'Specifications cannot exceed 100 characters'),
  ...
  width: Yup.number().required('Width is required').positive('Width must be a positive number').max(5000, 'Width cannot exceed 1000 units')
});
  ...
  const handleFormSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      dispatch({
        type: 'SET_DATA',
        payload: isEdit
          ? state.roadSegments.map((segment) => (segment.id === dataToSubmit.data.id ? { ...segment, ...response.data } : segment))
              ...
              {state.roadSegments.map((segment) => (
                <TableRow key={segment.id}>
                  ...
                  <TableCell>
                    ...
                    <IconButton onClick={() => handleDelete(segment.id || '')} style={{ color: 'red' }}>
                      <GridDeleteIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
