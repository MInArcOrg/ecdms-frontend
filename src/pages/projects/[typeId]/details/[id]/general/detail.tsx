...
// Define the type for project detail data
...
function ProjectGeneralInformation() {
  ...
  const { data: projectGeneralInformation, isLoading, error, refetch } = useQuery(
    {
        ...
        >
          <Grid container spacing={3}>
            <Grid item xs={12} md={12}>
              <ProfileCard projectInfo={{ ...projectGeneralInformation?.payload, id: id as string }} time={projectGeneralInformation?.payload?.time} />
            </Grid>
          </Grid>
        </Card>
        <Card>
              ...
              <Grid item xs={12} md={6} display='flex' gap={5}>
                ...
                <Box>
                  <Typography variant='subtitle1'>
                    {projectGeneralInformation?.payload?.client != null ? projectGeneralInformation?.payload?.client : t('project.general-information.please-add-client')}
                  </Typography>
                  <Typography variant='subtitle1'>
                    {projectGeneralInformation?.payload?.contractor != null ? projectGeneralInformation?.payload?.contractor : t('project.general-information.please-add-contractor')}
                  </Typography>
                  <Typography variant='subtitle1'>
                    {projectGeneralInformation?.payload?.consultant != null ? projectGeneralInformation?.payload?.consultant : t('project.general-information.please-add-consultant')}
                  </Typography>
                </Box>
              </Grid>
              <Grid
