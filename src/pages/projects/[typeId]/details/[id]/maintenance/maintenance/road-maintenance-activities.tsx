...
const defaultMenuItem = findSubMenuItem(subMenuItems('', ''), projectMaintenanceIds.maintenance.roadMaintenance);

const RoadMaintenanceActivities = () => {
  const router = useRouter();
  const { id = '', typeId = '' } = router.query;

  const menuItem = findSubMenuItem(subMenuItems(id as string, typeId as string), projectMaintenanceIds.maintenance.roadMaintenance);
  ...
  return (
    ...
    >
      <RoadMaintenanceActivityList otherSubMenu={menuItem} typeId={String(typeId)} projectId={String(id)} />
    </ProjectLayout>
  );
};
