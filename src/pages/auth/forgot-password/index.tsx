...
const ForgotPassword = () => {
  ...
  const schema = yup.object().shape({
    email: yup
      ...
      .required(t('forgot-password.validation-messages.email.required').toString())
  });
  ...
  return (
    <AuthContainer illustrationName={`auth-v2-forgot-password-illustration`}>
      ...
      <Box sx={{ my: 6 }}>
        ...
        <Typography sx={{ color: 'text.secondary' }}>{t('forgot-password.instructions')}</Typography>
      </Box>
      ...
      <Formik initialValues={defaultValues} validationSchema={schema} onSubmit={handleSubmit}>
        {({ errors, touched, handleChange, handleBlur, isSubmitting }) => (
          <Form noValidate autoComplete="off">
            ...
            >
              <Typography sx={{ color: 'text.secondary', mr: 2 }}>{t('forgot-password.remember-password')}</Typography>
