  ...
  return (
    <AuthContainer illustrationName={`minster-logo`}>
      ...
      <Box sx={{ my: 6 }}>
        ...
        <Typography sx={{ color: 'text.secondary' }}>{t('reset-password.instructions')}</Typography>
      </Box>
      ...
      <Formik initialValues={defaultValues} validationSchema={schema} onSubmit={handleSubmit}>
        {({ errors, touched, handleChange, handleBlur, isSubmitting, isValid }) => (
          <Form noValidate autoComplete="off">
            ...
            {passwordReset && (
              <Typography sx={{ color: 'text.secondary', textAlign: 'center' }}>{t('reset-password.password-reset')}</Typography>
            )}
            <Box
              sx={{
                display: 'flex',
            ...
            >
              <Typography sx={{ color: 'text.secondary', mr: 2 }}>{t('reset-password.remember-password')}</Typography>
