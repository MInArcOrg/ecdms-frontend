...
function Index() {
  ...
  const { data, isLoading, error,refetch:refetchResource }= useQuery(
    {
      queryKey:[],
  ...
  const uploadResourceImage = async (file: string | Blob) => {
    const formData = new FormData()
    formData.append('image', file)

    if (data?.data?.image_id) {
      try {
        // const response = await uploadFile({ data: formData, method: 'PUT' })
        refetchResource()
  ...
  return (
    <Fragment>
        <ResourceLayout
          ...
          data={data?.payload}
          uploadImage={uploadResourceImage}
        >
          {activeRoute() == 0 && <div />}
          {activeRoute() == 1 && <ResourceSpecificationList resourceId={String(id)}/>}
          {/* {activeRoute() == 2 && <Brand id={id} isProject={false}/>} */}
