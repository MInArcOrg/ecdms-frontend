...
function Location() {
  ...
  const { data: fetchedCategories, isLoading: isCategoryLoading } = useQuery({
    queryKey: ['masterCategory', 'stakeholder', activeType?.id],
    queryFn: () =>
      masterCategoryApiService.getAll('stakeholder', {
        filter: { stakeholdertype_id: activeType?.id }
      }),
    enabled: !!activeType
  })
        ...
        <CardContent>
          <Grid container spacing={3} mt={3}>
            ...
            <Grid item xs={12} sm={9}>
              <LocationCard categories={categories} loading={isCategoryLoading} baseUrl={'/analytics/stakeholder-category-location-information'} />
            </Grid>
          </Grid>
        </CardContent>
      </Card>
