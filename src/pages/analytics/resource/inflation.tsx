...
const InflationAnalytics = () => {
  ...
  const baseSeed = hashString(`${resourceTitle}:${baseYear?.name || ''}:inflation`);

  const dummyInflation = {
    payload: {
      labels: yearLabels,
      data: RESOURCE_REGIONS.map((regionName, regionIndex) => {
        ...
        const trend = (profile.trendMin + rand() * (profile.trendMax - profile.trendMin)) * 1.15;
        const phase = rand() * Math.PI * 2;

        const series = yearLabels.map((_, i) => {
          const wave = Math.sin(i * 0.8 + phase) * (profile.vol * 1.1);
          const noise = (rand() - 0.5) * (profile.vol * 1.2);
          const factor = Math.max(0.65, 1 + wave + noise);
  ...
  return (
    <ResourceAnalyticsLayout>
      ...
      <Card>
        {tableView ? (
          <TableView
            years={(dummyEnabled ? dummyInflation : data)?.payload?.labels?.map((year: number, index: number) => ({ id: index, name: year }))}
            baseYear={indexOf(years, baseYear)}
            inflation={true}
            data={(dummyEnabled ? dummyInflation : data)?.payload.data}
          />
        ) : (
          <DatePickerWrapper>
            <ChartView
