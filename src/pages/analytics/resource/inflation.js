...
const Inflation = () => {
  ...
  const [{ data: resourceTypes, loading: resourceTypesLoading, error: resourceTypesError }, getTypes] =
    getResourceTypes()
  ...
  function createData(label, rest) {
    ...
    const data = rest.map((item, index) => Number((Number(item - rest[baseIndex]) / rest[baseIndex]) * 100).toFixed(0))
  ...
  return (
    <ResourceAnalyticsLayout>
      ...
      <Card>
        {tableView === true ? (
          <TableView years={years} baseYear={indexOf(years, baseYear)} reagions={labels ? labels : []} data={rows} />
        ) : (
          <DatePickerWrapper>
            <ChartView
              years={years}
