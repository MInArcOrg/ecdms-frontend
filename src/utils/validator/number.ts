...
export const nullableNumberSchema = (
  { maxIntegerDigits = 15, maxDecimalPlaces = 2 }: { maxIntegerDigits?: number; maxDecimalPlaces?: number } = {}
) =>
  limitNumberDigits(
    yup
      .number()
      ...
      .nullable(),
    { maxIntegerDigits, maxDecimalPlaces }
  );

export const nullableIntegerSchema = (
  integerMessage = 'Must be an integer',
  { maxIntegerDigits = 15 }: { maxIntegerDigits?: number } = {}
) =>
  limitNumberDigits(
    yup
      .number()
    ...
    { maxIntegerDigits, maxDecimalPlaces: 0 }
  );
