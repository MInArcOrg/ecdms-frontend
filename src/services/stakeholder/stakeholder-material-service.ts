...
const stakeholderMaterialApiService = {
  getAll: (params: GetRequestParam): Promise<IApiResponse<StakeholderMaterial[]>> =>
    buildGetRequest("/stakeholders/stakeholder-materials", params)
      .then((response: AxiosResponse<IApiResponse<StakeholderMaterial[]>>) => response.data)
      .catch((error: any) => {
        throw error
      }),

  getById: (id: string, params: GetRequestParam): Promise<IApiResponse<StakeholderMaterial>> =>
  ...
  update: (id: string, body: IApiPayload<StakeholderMaterial>): Promise<IApiResponse> =>
