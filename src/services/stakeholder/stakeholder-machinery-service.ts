...
const stakeholderMachineryApiService = {
  getAll: (params: GetRequestParam): Promise<IApiResponse<StakeholderMachinery[]>> =>
    buildGetRequest('/stakeholders/stakeholder-machineries', params)
      .then((response: AxiosResponse<IApiResponse<StakeholderMachinery[]>>) => response.data)
      .catch((error: any) => {
        throw error;
      }),

  getById: (id: string, params: GetRequestParam): Promise<IApiResponse<StakeholderMachinery>> =>
  ...
  update: (id: string, body: IApiPayload<StakeholderMachinery>): Promise<IApiResponse> =>
