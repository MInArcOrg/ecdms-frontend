...
const safetyEquipmentApiService = {
  getAll: (params: GetRequestParam): Promise<IApiResponse<SafetyEquipment[]>> =>
    buildGetRequest("/stakeholders/safety-equipments", params)
      .then((response: AxiosResponse<IApiResponse<SafetyEquipment[]>>) => response.data)
      .catch((error: any) => {
        throw error
      }),

  getById: (id: string, params: GetRequestParam): Promise<IApiResponse<SafetyEquipment>> =>
  ...
  update: (id: string, body: IApiPayload<SafetyEquipment>): Promise<IApiResponse> =>
