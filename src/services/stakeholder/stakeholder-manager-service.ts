...
const stakeholderManagerApiService = {
  getAll: (params: GetRequestParam): Promise<IApiResponse<StakeholderManager[]>> =>
    buildGetRequest('/stakeholders/stakeholder-managers', params)
      .then((response: AxiosResponse<IApiResponse<StakeholderManager[]>>) => response.data)
      .catch((error: any) => {
        throw error;
      }),

  getById: (id: string, params: GetRequestParam): Promise<IApiResponse<StakeholderManager>> =>
  ...
  update: (id: string, body: IApiPayload<StakeholderManager>): Promise<IApiResponse> =>
