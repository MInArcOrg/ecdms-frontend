...
const jointVentureCompanyApiService = {
  getAll: (params: GetRequestParam): Promise<IApiResponse<JointVentureCompany[]>> =>
    buildGetRequest('/stakeholders/joint-venture-companies', params)
      .then((response: AxiosResponse<IApiResponse<JointVentureCompany[]>>) => response.data)
      .catch((error: any) => {
        throw error;
      }),

  getById: (id: string, params: GetRequestParam): Promise<IApiResponse<JointVentureCompany>> =>
  ...
  update: (id: string, body: IApiPayload<JointVentureCompany>): Promise<IApiResponse> =>
