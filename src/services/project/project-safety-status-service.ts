...
const projectSafetyStatusApiService = {
  getAll: (params: GetRequestParam): Promise<IApiResponse<ProjectSafetyStatus[]>> =>
    buildGetRequest('/projects/project-safety-statuses', params)
      .then((response: AxiosResponse<IApiResponse<ProjectSafetyStatus[]>>) => response.data)
      .catch((error: any) => {
        throw error;
      }),

  getById: (id: string, params: GetRequestParam): Promise<IApiResponse<ProjectSafetyStatus>> =>
  ...
  update: (id: string, body: IApiPayload<ProjectSafetyStatus>): Promise<IApiResponse> =>
