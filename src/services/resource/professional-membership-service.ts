...
const professionalMembershipApiService = {
  getAll: (
    ...
    buildGetRequest("/resources/professional-association-memberships", params)
      .then(
        (response: AxiosResponse<IApiResponse<ProfessionalMembership[]>>) =>
          response.data,
      )
      .catch((error: any) => {
        throw error;
  ...
  getById: (
    ...
    buildGetRequest(
      `/resources/professional-association-memberships/${id}`,
      params,
    )
      .then(
        (response: AxiosResponse<IApiResponse<ProfessionalMembership>>) =>
  ...
  create: (body: IApiPayload<ProfessionalMembership>): Promise<IApiResponse> =>
    buildPostRequest(
      "/resources/professional-association-memberships",
      body,
      false,
    )
      .then((response: AxiosResponse<IApiResponse>) => response.data)
