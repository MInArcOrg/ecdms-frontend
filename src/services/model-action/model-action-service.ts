...
const modelActionApiService = {
  performCAActions: (
    body: { data: ModelAction; files: any[] },
    type: string,
  ): Promise<IApiResponse<ModelAction>> =>
  ...
  ): Promise<IApiResponse<AuthorizationResponse>> =>
    buildGetRequest(`/generics/model-action-data/${model_id}`, params)
      .then(
        (response: AxiosResponse<IApiResponse<AuthorizationResponse>>) =>
          response.data,
      )
      .catch((error: any) => {
        throw error;
      }),
  addCAActionNote: (body: {
    data: Note;
    files: any[];
  ...
  getActionReplies: (
    model_id: string,
    params: GetRequestParam,
  ): Promise<IApiResponse<ActionReply[]>> =>
    buildGetRequest("/generics/action-replies/" + model_id, params)
      .then(
        (response: AxiosResponse<IApiResponse<ActionReply[]>>) => response.data,
      )
      .catch((error: any) => {
        throw error;
      }),
  createActionReply: (body: {
    data: ActionReply;
    files: [];
  }): Promise<IApiResponse> =>
